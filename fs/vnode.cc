...
{
Vnode::Vnode(Superblock* sb, std::shared_ptr<Inode> inode)
    : Vnode(sb, inode, inode->rdev)
{
}

Vnode::Vnode(Superblock* sb, std::shared_ptr<Inode> inode, dev_t rdev)
{
    this->sb    = sb;
    this->inode = inode;
    this->rdev  = rdev;
...
int Vnode::link(const char* name, std::shared_ptr<Vnode> node)
{
...
std::shared_ptr<Vnode> Vnode::lookup(const char* name, int flags, mode_t mode)
{
    std::shared_ptr<Inode> retinode = inode->lookup(name, flags, mode);
    if (!retinode) {
        return std::shared_ptr<Vnode>(nullptr);
    }
    std::shared_ptr<Vnode> retvnode =
