...
//
// Defined as 0 or 1.
// Indicates whether we need to define our own implementations of inf, nan,
// snan, denorm floating point constants.
//
#if !defined(EASTL_CUSTOM_FLOAT_CONSTANTS_REQUIRED)
#if (defined(EA_COMPILER_GNUC) ||                                              \
     defined(EA_COMPILER_CLANG) && defined(__FLT_MIN__)) ||                    \
    defined(_CPPLIB_VER) // __FLT_MIN__ detects if it's really GCC/clang and not
                         // a mimic. _CPPLIB_VER (Dinkumware) covers VC++, and
                         // Microsoft platforms.
...
#define EASTL_LIMITS_MIN(T)                                                    \
    ((EASTL_LIMITS_IS_SIGNED(T) ? EASTL_LIMITS_MIN_S(T)                        \
                                : EASTL_LIMITS_MIN_U(T)))

// The max possible value of T.
#define EASTL_LIMITS_MAX_S(T)                                                  \
    ((T)(((((T)1 << (EASTL_LIMITS_DIGITS(T) - 1)) - 1) << 1) + 1))
#define EASTL_LIMITS_MAX_U(T) ((T) ~(T)0)
#define EASTL_LIMITS_MAX(T)                                                    \
    ((EASTL_LIMITS_IS_SIGNED(T) ? EASTL_LIMITS_MAX_S(T)                        \
                                : EASTL_LIMITS_MAX_U(T)))
...
#define EASTL_LIMITS_DIGITS(T)                                                 \
    ((EASTL_LIMITS_IS_SIGNED(T) ? EASTL_LIMITS_DIGITS_S(T)                     \
                                : EASTL_LIMITS_DIGITS_U(T)))

// The number of decimal digits that can be represented by T.
#define EASTL_LIMITS_DIGITS10_S(T)                                             \
    ((EASTL_LIMITS_DIGITS_S(T) * 643L) / 2136) // (643 / 2136) ~= log10(2).
#define EASTL_LIMITS_DIGITS10_U(T) ((EASTL_LIMITS_DIGITS_U(T) * 643L) / 2136)
#define EASTL_LIMITS_DIGITS10(T)                                               \
...
enum float_denorm_style {
    denorm_indeterminate = -1, /// It cannot be determined whether or not the
...
namespace Internal
{
// Defines default values for numeric_limits, which can be overridden by class
// specializations. See C++11 18.3.2.3
struct numeric_limits_base {
    // true if the type has an explicit specialization defined in the template
    // class; false if not.
    static EA_CONSTEXPR_OR_CONST bool is_specialized = false;

    // Integer types: the number of *bits* in the representation of T.
    // Floating types: the number of digits in the mantissa of T (same as
    ...
    // The number of decimal digits required to make sure that two distinct
    // values of the type have distinct decimal representations.
    ...
    // Floating types: the base of the exponent representation. Always FLT_RADIX
    // (typically 2) for float.
    ...
    // In practice the application can change this at runtime via
    ...
    // True if the type has a bounded set of representable values. Typically
    // true for all built-in numerial types (integer and floating point).
    ...
    // True if trapping (arithmetic exception generation) is implemented for
    // this type. Typically true for integer types (div by zero) and false for
    // floating point types, though in practice the application may be able to
    // change floating point to trap at runtime.
    ...
    // In practice the application can change this at runtime via
    // hardware-specific commands.
    static EA_CONSTEXPR_OR_CONST float_round_style round_style =
        round_toward_zero;

    // True if the type is floating point and follows the IEC 559 standard (IEEE
    // 754). In practice the application or OS can change this at runtime via
    // hardware-specific commands or via compiler optimizations.
...
struct numeric_limits<bool> {
    ...
    static EA_CONSTEXPR_OR_CONST int digits =
        1; // In practice bool is stores as a byte, or sometimes an int.
    static EA_CONSTEXPR_OR_CONST int digits10     = 0;
    static EA_CONSTEXPR_OR_CONST int max_digits10 = 0;
    static EA_CONSTEXPR_OR_CONST bool is_signed =
        false; // In practice bool may be implemented as signed char.
    ...
    static EA_CONSTEXPR_OR_CONST bool is_modulo     = false;
    static EA_CONSTEXPR_OR_CONST bool traps =
        true; // Should this be true or false? Given that it's implemented in
...
struct numeric_limits<char> {
    ...
    static EA_CONSTEXPR_OR_CONST int digits10 =
        EASTL_LIMITS_DIGITS10(value_type);
    static EA_CONSTEXPR_OR_CONST int max_digits10 = 0;
    static EA_CONSTEXPR_OR_CONST bool is_signed =
        EASTL_LIMITS_IS_SIGNED(value_type);
    static EA_CONSTEXPR_OR_CONST bool is_integer      = true;
    static EA_CONSTEXPR_OR_CONST bool is_exact        = true;
    static EA_CONSTEXPR_OR_CONST int radix            = 2;
    static EA_CONSTEXPR_OR_CONST int min_exponent     = 0;
...
struct numeric_limits<unsigned char> {
    ...
    static EA_CONSTEXPR_OR_CONST int digits10 =
        EASTL_LIMITS_DIGITS10_U(value_type);
    static EA_CONSTEXPR_OR_CONST int max_digits10     = 0;
    static EA_CONSTEXPR_OR_CONST bool is_signed       = false;
    static EA_CONSTEXPR_OR_CONST bool is_integer      = true;
    static EA_CONSTEXPR_OR_CONST bool is_exact        = true;
...
struct numeric_limits<signed char> {
    ...
    static EA_CONSTEXPR_OR_CONST int digits10 =
        EASTL_LIMITS_DIGITS10_S(value_type);
    static EA_CONSTEXPR_OR_CONST int max_digits10     = 0;
...
// VC++ has the option of making wchar_t simply be unsigned short. If that's
// enabled then the code below could possibly cause compile failures due to
// redundancy. The best resolution may be to use __wchar_t here for VC++ instead
// of wchar_t, as __wchar_t is always a true unique type under VC++.
...
#if EA_CHAR16_NATIVE // If char16_t is a true unique type (as called for by the
                     // C++11 Standard)...
...
#if EA_CHAR32_NATIVE // If char32_t is a true unique type (as called for by the
                     // C++11 Standard)...
...
struct numeric_limits<signed long long> {
    ...
    (defined(EA_COMPILER_GNUC) ||                                              \
     defined(EA_COMPILER_CLANG)) // If __int128_t/__uint128_t is supported...
...
struct numeric_limits<float> {
    ...
    static EA_CONSTEXPR_OR_CONST bool has_quiet_NaN =
        true; // This may be wrong for some platforms.
    static EA_CONSTEXPR_OR_CONST bool has_signaling_NaN =
        true; // This may be wrong for some platforms.
    static EA_CONSTEXPR_OR_CONST float_denorm_style has_denorm =
        denorm_present; // This may be wrong for some platforms.
    static EA_CONSTEXPR_OR_CONST bool has_denorm_loss =
        false; // This may be wrong for some platforms.
    static EA_CONSTEXPR_OR_CONST bool is_iec559 =
...
struct numeric_limits<double> {
    ...
    static EA_CONSTEXPR_OR_CONST bool is_integer     = false;
    static EA_CONSTEXPR_OR_CONST bool is_exact       = false;
    static EA_CONSTEXPR_OR_CONST int radix =
        FLT_RADIX; // FLT_RADIX applies to all floating point types.
