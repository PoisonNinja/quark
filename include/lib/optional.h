...
struct optional_base {
    ...
    explicit optional_base(in_place_t, std::initializer_list<U> il,
                           Args&&... args)
        : init_(true)
        , storage_(il, libcxx::forward<Args>(args)...)
    {
    }
...
class optional : private OptionalBase<T>
{
    ...
    {
        return libcxx::addressof(OptionalBase<T>::storage_.value_);
    }
    ...
    {
        return libcxx::move(OptionalBase<T>::storage_.value_);
    }
    OPTIONAL_MUTABLE_CONSTEXPR T& contained_val() &
    {
        return OptionalBase<T>::storage_.value_;
    ...
    void initialize(Args&&... args) noexcept(
        noexcept(T(libcxx::forward<Args>(args)...)))
    {
        assert(!OptionalBase<T>::init_);
        ::new (static_cast<void*>(dataptr())) T(libcxx::forward<Args>(args)...);
        OptionalBase<T>::init_ = true;
    }

    template <class U, class... Args>
    void initialize(std::initializer_list<U> il, Args&&... args) noexcept(
        noexcept(T(il, libcxx::forward<Args>(args)...)))
    {
        assert(!OptionalBase<T>::init_);
        ::new (static_cast<void*>(dataptr()))
            T(il, libcxx::forward<Args>(args)...);
        OptionalBase<T>::init_ = true;
    }
...
public:
    ...
    {
        if (rhs.initialized()) {
            ::new (static_cast<void*>(dataptr())) T(libcxx::move(*rhs));
            OptionalBase<T>::init_ = true;
        }
    }
            ...
            std::is_nothrow_move_constructible<T>::value)
    {
        if (initialized() == true && rhs.initialized() == false)
            ...
            initialize(libcxx::move(*rhs));
        else if (initialized() == true && rhs.initialized() == true)
            contained_val() = libcxx::move(*rhs);
        return *this;
    }

    template <class U>
    auto operator=(U&& v) -> typename std::enable_if<
        std::is_same<typename std::decay<U>::type, T>::value, optional&>::type
    {
        if (initialized()) {
            contained_val() = libcxx::forward<U>(v);
        } else {
            initialize(libcxx::forward<U>(v));
        }
        return *this;
    }
    ...
    {
        clear();
        initialize(libcxx::forward<Args>(args)...);
    }
    ...
    {
        clear();
        initialize<U, Args...>(il, libcxx::forward<Args>(args)...);
    }
    ...
    {
        if (initialized() == true && rhs.initialized() == false) {
            rhs.initialize(libcxx::move(**this));
            clear();
        } else if (initialized() == false && rhs.initialized() == true) {
            initialize(libcxx::move(*rhs));
            rhs.clear();
    ...
    OPTIONAL_MUTABLE_CONSTEXPR T&& value() &&
    {
        if (!initialized())
            throw bad_optional_access("bad optional access");
        return libcxx::move(contained_val());
    }
    ...
    constexpr T const& value() const
    {
        return initialized()
                   ? contained_val()
                   : (__throw_bad_optional_access("bad optional access"),
                      contained_val());
    }
...
public:
    ...
    {
        return ref ? *ref
                   : (__throw_bad_optional_access("bad optional access"), *ref);
    }
...
{
template <typename T>
struct hash<libcxx::optional<T>> {
    typedef typename hash<T>::result_type result_type;
    typedef libcxx::optional<T> argument_type;

    constexpr result_type operator()(argument_type const& arg) const
    {
        return arg ? libcxx::hash<T>{}(*arg) : result_type{};
    }
};
