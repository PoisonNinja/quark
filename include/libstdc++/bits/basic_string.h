...
namespace std _GLIBCXX_VISIBILITY(default)
{
    ...
    private:
        ...
        {
#if __cplusplus >= 201103L
            return std::pointer_traits<const_pointer>::pointer_to(
                *_M_local_buf);
#else
        ...
        {
            typedef
                typename iterator_traits<_InIterator>::iterator_category _Tag;
    ...
    public:
         ...
         */
        basic_string() _GLIBCXX_NOEXCEPT_IF(
        ...
        basic_string& operator=(const basic_string& __str)
        {
            ...
            if (_Alloc_traits::_S_propagate_on_copy_assign()) {
                if (!_Alloc_traits::_S_always_equal() && !_M_is_local() &&
                    ...
                    // Propagating allocator cannot free existing storage so
                    // must deallocate it before replacing current allocator.
                    if (__str.size() <= _S_local_capacity) {
                        ...
                        auto __ptr =
                            _Alloc_traits::allocate(__alloc, __len + 1);
                        _M_destroy(_M_allocated_capacity);
                        _M_data(__ptr);
                        _M_capacity(__len);
                        _M_set_length(__len);
    ...
    public:
         ...
         *  length_error is thrown.  If @a __pos is beyond end(), out_of_range
         * is thrown.  The value of the string doesn't change if an error is
         *  thrown.
         */
                                          ...
                                          size_type __n = npos)
        {
            ...
            return this->replace(
                __pos1, size_type(0),
                __sv.data() + __sv._M_check(__pos2, "basic_string::insert"),
         ...
         *  string.  In place, the value of @a __str is inserted.  If @a __pos
         * is beyond end of string, out_of_range is thrown.  If the length of
         * the result exceeds max_size(), length_error is thrown.  The value of
                ...
                _InputIterator __k2)
        {
            ...
            __glibcxx_requires_valid_range(__k1, __k2);
            typedef
                typename std::__is_integer<_InputIterator>::__type _Integral;
    ...
    public:
         ...
         *  @return  Index of start of first occurrence.
         *
         *  Starting from @a __pos, searches forward for value of @a __str
         * within this string.  If found, returns the index where it begins.  If
         ...
         * contained in @a __str within this string.  If found, returns the
         ...
         *  contained in the first @a __n characters of @a __s within this
         ...
         *  @param __svt  An object convertible to string_view to compare
         *                against.
         *  @param __pos2  A position in the string_view to start comparing
         * from.
    ...
    private:
        ...
        struct _Rep : _Rep_base {
            // Types:
            typedef
                typename _Alloc::template rebind<char>::other _Raw_bytes_alloc;
            ...
            bool _M_is_leaked() const _GLIBCXX_NOEXCEPT
            {
                ...
                // either leaked or not), so a relaxed load is enough.
            ...
            bool _M_is_shared() const _GLIBCXX_NOEXCEPT
            {
                ...
                // last but one reference concurrently with this check, so we
                // need this load to be acquire to synchronize with release
                // fetch_and_add in _M_dispose.
            ...
            void _M_dispose(const _Alloc& __a) _GLIBCXX_NOEXCEPT
            {
...
#endif
                {
                    ...
                    // - last but one decrement needs to release to synchronize
                    // with
                    //   the acquire load in _M_is_shared that will conclude
                    //   that the object is not shared anymore.
                    if (__gnu_cxx::__exchange_and_add_dispatch(
                            &this->_M_refcount, -1) <= 0) {
...
#else
            : _M_dataplus(_S_construct(size_type(), _CharT(), _Alloc()),
                          _Alloc())
        {
        }
#endif
    ...
    private:
        ...
        {
            return _S_construct_aux_2(static_cast<size_type>(__beg), __end,
                                      __a);
        }
    ...
    public:
        ...
        friend void ::_txnal_cow_string_C1_for_exceptions(void* that,
                                                          const char* s,
                                                          void* exc);
...
namespace std _GLIBCXX_VISIBILITY(default)
{
    ...
    inline string to_string(float __val)
    {
        ...
        return __gnu_cxx::__to_xstring<string>(&std::vsnprintf, __n, "%f",
                                               __val);
    }
    ...
    inline string to_string(long double __val)
    {
        ...
        return __gnu_cxx::__to_xstring<string>(&std::vsnprintf, __n, "%Lf",
                                               __val);
    }
    ...
    {
        return __gnu_cxx::__to_xstring<wstring>(
            &std::vswprintf, 4 * sizeof(long), L"%ld", __val);
    }
    ...
    inline wstring to_wstring(float __val)
    {
        const int __n =
            __gnu_cxx::__numeric_traits<float>::__max_exponent10 + 20;
        return __gnu_cxx::__to_xstring<wstring>(&std::vswprintf, __n, L"%f",
                                                __val);
    }
