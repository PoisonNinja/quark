...
namespace std _GLIBCXX_VISIBILITY(default)
{
        ...
        forward_iterator_tag)
    {
        ...
        if (__gnu_cxx::__is_null_pointer(__beg) && __beg != __end)
            __throw_logic_error(
                __N("basic_string::_S_construct null not valid"));
    ...
    template <typename _CharT, typename _Traits, typename _Alloc>
    basic_string<_CharT, _Traits, _Alloc>::basic_string(
        const basic_string& __str, size_type __pos, const _Alloc& __a)
        : _M_dataplus(
              _S_construct(
                  __str._M_data() +
    ...
    basic_string<_CharT, _Traits, _Alloc>::basic_string(const _CharT* __s,
        ...
        : _M_dataplus(
              _S_construct(
                  __s, __s ? __s + traits_type::length(__s) : __s + npos, __a),
              __a)
    {
    }
        ...
        const _Alloc& __alloc, size_type __res)
    {
        ...
        _Rep* __r =
            _Rep::_S_create(__requested_cap, this->_M_capacity, __alloc);
        if (this->_M_length)
    ...
    typename basic_string<_CharT, _Traits, _Alloc>::size_type
    basic_string<_CharT, _Traits, _Alloc>::find_first_not_of(
        _CharT __c, size_type __pos) const _GLIBCXX_NOEXCEPT
    {
        for (; __pos < this->size(); ++__pos)
        ...
        basic_string<_CharT, _Traits, _Alloc>& __str)
    {
        ...
        if (__cerb) {
            __try {
                ...
                while (
                    __extracted < __n && !_Traits::eq_int_type(__c, __eof) &&
