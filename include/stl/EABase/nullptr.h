...
#pragma once /* Some compilers (e.g. VC++) benefit significantly from using    \
                this. We've measured 3-4% build speed improvements in apps as  \
                a result. */
#endif

#if defined(EA_COMPILER_CPP11_ENABLED) && !defined(EA_COMPILER_NO_NULLPTR) &&  \
    !defined(EA_HAVE_nullptr_t_IMPL)
// The compiler supports nullptr, but the standard library doesn't implement a
// declaration for std::nullptr_t. So we provide one.
namespace std
{
...
public:
    ...
    operator bool_()
        const // An rvalue of type std::nullptr_t can be converted to an rvalue
              // of type bool; the resulting value is false.
    {
        return false;
    } // We can't use operator bool(){ return false; } because bool is
      // convertable to int which breaks other required functionality.

    // We can't enable this without generating warnings about nullptr being
    // uninitialized after being used when created without "= {}".
    // void* mSizeofVoidPtr;         // sizeof(nullptr_t) == sizeof(void*).
    // Needs to be public if nullptr_t is to be a POD.
...
#if !defined(                                                                  \
    nullptr) // If somebody hasn't already defined nullptr in a custom way...
