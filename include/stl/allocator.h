...
///
/// C++11's std::allocator (20.6.9) doesn't have a move constructor or
/// assignment operator. This is possibly because std::allocators are associated
/// with types instead of as instances. The potential non-equivalance of C++
/// std::allocator instances has been a source of some acknowledged design
/// problems. We don't implement support for move construction or assignment in
/// stl::allocator, but users can define their own allocators which do have move
/// functions and the stl containers are compatible with such allocators (i.e.
...
public:
    ...
    allocator(const char* pName = EASTL_NAME_VAL(EASTL_ALLOCATOR_DEFAULT_NAME));
...
/// dummy_allocator
///
/// Defines an allocator which does nothing. It returns NULL from allocate
/// calls.
///
...
/// To consider: This naming of this function is too similar to
/// get_default_allocator and instead should be named something like
...
///
/// Implements a default allocfreemethod which uses the default global
/// allocator. This version supports only default alignment.
///
...
///
template <typename Allocator>
void* allocate_memory(Allocator& a, size_t n, size_t alignment,
                      size_t alignmentOffset);

} // namespace stl

#ifndef EASTL_USER_DEFINED_ALLOCATOR // If the user hasn't declared that he has
                                     // defined a different allocator
...
inline void* allocator::allocate(size_t n, size_t alignment, size_t offset,
                                 int flags)
{
#if EASTL_DLL
...
#else
    return ::new (alignment, offset, pName, flags, 0, __FILE__,
                  __LINE__) char[n];
#endif
...
{
    return true; // All allocators are considered equal, as they merely use
                 // global new/delete.
}

inline bool operator!=(const allocator&, const allocator&)
{
    return false; // All allocators are considered equal, as they merely use
                  // global new/delete.
}
...
{
    return NULL; // By default we return NULL; the user must make specialization
                 // of this function in order to provide their own
                 // implementation.
}

inline EASTLAllocatorType* get_default_allocator(const EASTLAllocatorType*)
{
    return EASTLAllocatorDefault(); // For the built-in allocator
                                    // EASTLAllocatorType, we happen to already
                                    // have a function for returning the default
                                    // allocator instance, so we provide it.
}

inline void* default_allocfreemethod(size_t n, void* pBuffer,
                                     void* /*pContext*/)
{
                             ...
                             size_t alignmentOffset)
{
    ...
    if (alignment <= EASTL_ALLOCATOR_MIN_ALIGNMENT) {
        ...
        // Ensure the result is correctly aligned.  An assertion likely
        // indicates a mismatch between EASTL_ALLOCATOR_MIN_ALIGNMENT and the
        // minimum alignment of EASTLAlloc.  If there is a mismatch it may be
        // necessary to define EASTL_ALLOCATOR_MIN_ALIGNMENT to be the minimum
        // alignment of EASTLAlloc, or to increase the alignment of EASTLAlloc
        // to match EASTL_ALLOCATOR_MIN_ALIGNMENT.
        ...
        // Ensure the result is correctly aligned.  An assertion here may
        // indicate a bug in the allocator.
        EASTL_ASSERT((reinterpret_cast<size_t>(result) & ~(alignment - 1)) ==
                     reinterpret_cast<size_t>(result));
    }
    return result;
