...
//
// Identifies if the standard library provides a built-in aligned version of
// malloc. Defined as 0 or 1, depending on the standard library or platform
// availability. None of the viable C functions provides for an aligned malloc
// with offset, so we don't consider that supported in any case.
//
...
// http://pubs.opengroup.org/onlinepubs/000095399/functions/posix_memalign.html
// VC++ _aligned_malloc
// http://msdn.microsoft.com/en-us/library/8z34s9c6%28VS.80%29.aspx This is not
// suitable, since it has a limitation that you need to free via _aligned_free.
//
#if !defined EASTL_ALIGNED_MALLOC_AVAILABLE
#if defined(EA_PLATFORM_POSIX) && !defined(EA_PLATFORM_APPLE)
// memalign is more consistently available than posix_memalign, though its
// location isn't consistent across platforms and compiler libraries. Typically
// it's declared in one of three headers: stdlib.h, malloc.h, or malloc/malloc.h
...
public:
                   ...
                   int /*flags*/ = 0)
    {
        ...
        if ((alignmentOffset % alignment) ==
            0) // We check for (offset % alignmnent == 0) instead of (offset ==
               // 0) because any block which is aligned on e.g. 64 also is
               // aligned at an offset of 64 by definition.
            return memalign(alignment, n); // memalign is more consistently
                                           // available than posix_memalign.
#else
        if ((alignment <= EASTL_SYSTEM_ALLOCATOR_MIN_ALIGNMENT) &&
            ((alignmentOffset % alignment) == 0))
            return malloc(n);
#endif
        return NULL;
    }
