...
///////////////////////////////////////////////////////////////////////////////
// This file implements the stl::any which is part of the C++ standard STL
// library specification.  
//
// stl::any is a type-safe container for single values of any type.  Our
...
namespace stl
{
			...
			static void* handler_func(storage_operation op, const any* pThis, any* pOther)
			{
				switch (op)
				{
					...
					case storage_operation::MOVE:
					{
						EASTL_ASSERT(pThis);
						EASTL_ASSERT(pOther);
						construct(pOther->m_storage, stl::move(*(T*)(&pThis->m_storage.internal_storage)));
						destroy(const_cast<any&>(*pThis));
					}
					break;
			...
			static void* handler_func(storage_operation op, const any* pThis, any* pOther)
			{
				switch (op)
				{
					...
					case storage_operation::MOVE:
					{
						EASTL_ASSERT(pThis);
						EASTL_ASSERT(pOther);
						construct(pOther->m_storage, stl::move(*(T*)(pThis->m_storage.external_storage)));
						destroy(const_cast<any&>(*pThis));
					}
					break;
	...
	public:
		...
		{ 
			if(other.m_handler)
			{
				...
				m_handler = stl::move(other.m_handler);
		...
		any(ValueType&& value,
		    typename stl::enable_if<!stl::is_same<typename stl::decay<ValueType>::type, any>::value>::type* = 0)
		{
			...
			storage_handler<DecayedValueType>::construct(m_storage, stl::forward<ValueType>(value));
		...
		explicit any(in_place_type_t<T>, Args&&... args) 
		{
			...
			static_assert(stl::is_constructible<T, Args...>::value, "T must be constructible with Args...");

			StorageHandlerT::construct_inplace(m_storage, stl::forward<Args>(args)...);
		...
		explicit any(in_place_type_t<T>,
		             ...
		             Args&&... args,
		             typename stl::enable_if<stl::is_constructible<T, std::initializer_list<U>&, Args...>::value,
		                                       void>::type* = 0)
		{
			typedef storage_handler<decay_t<T>> StorageHandlerT;

			StorageHandlerT::construct_inplace(m_storage, il, stl::forward<Args>(args)...);
			m_handler = &StorageHandlerT::handler_func;
		}
		...
		any& operator=(ValueType&& value)
		{
			...
			any(stl::forward<ValueType>(value)).swap(*this);
			return *this;
		}
        ...
        // 20.7.3.3, modifiers
			...
			template <class NT, class U, class... Args>
		    typename stl::enable_if<stl::is_constructible<NT, std::initializer_list<U>&, Args...>::value, void>::type
		...
		void swap(any& other) EA_NOEXCEPT 
		{
			...
			{
				stl::swap(m_handler, other.m_handler);
	...
	{
		static_assert(stl::is_reference<ValueType>::value || stl::is_copy_constructible<ValueType>::value,
	...
	// value will be different -- completely breaking the validation mechanism.  Due to the fact that stl::any uses
	...
	//
		...
		{
			return any(stl::in_place<T>, stl::forward<Args>(args)...);
		}

		template <class T, class U, class... Args>
		inline any make_any(std::initializer_list<U> il, Args&&... args)
		{
			return any(stl::in_place<T>, il, stl::forward<Args>(args)...);
		}
    #endif
