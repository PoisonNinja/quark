...
namespace stl
{
	...
	///
		...
		#define EASTL_BITVECTOR_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " bitvector" // Unless the user overrides something, this is "stl bitvector".
	#endif
	...
	template <typename Allocator = EASTLAllocatorType, 
			  ...
			  typename Container = stl::vector<Element, Allocator> >
	class bitvector
	{
	public:
		...
		if(pCurrent >= pStart)
		{
			if(nExtraBits == 0)
			{
				if(pCurrent == pEnd && mReference)
					return stl::isf_valid | stl::isf_current;
				else if(pCurrent < pEnd)
					return stl::isf_valid | stl::isf_current | stl::isf_can_dereference;
			}
...
*/
	...
	bitvector<Allocator, Element, Container>::insert(const_iterator position, value_type value)
	{
		iterator iPosition(position.get_reference_type()); // This is just a non-const version of position.
			...
			if(EASTL_UNLIKELY(validate_iterator(iPosition) & stl::isf_valid) == 0)
	...
	*/
	...
	bitvector<Allocator, Element, Container>::erase(const_iterator position)
	{
		...
		#if EASTL_ASSERT_ENABLED
			if(EASTL_UNLIKELY(validate_iterator(iPosition) & stl::isf_can_dereference) == 0)
		...
		EASTL_ASSERT(validate_iterator(iPosition) & stl::isf_valid);
		return iPosition;
	}
	...
	bitvector<Allocator, Element, Container>::erase(const_iterator first, const_iterator last)
	{
		...
		#if EASTL_ASSERT_ENABLED
			if(EASTL_UNLIKELY(validate_iterator(iLast) & stl::isf_valid) == 0)
				EASTL_FAIL_MSG("bitvector::erase -- invalid iterator");
		#endif
		...
		{
			#if EASTL_ASSERT_ENABLED
				if(EASTL_UNLIKELY(validate_iterator(iFirst) & stl::isf_can_dereference) == 0)
					EASTL_FAIL_MSG("bitvector::erase -- invalid iterator");
			#endif
			...
			// Verify that no reallocation occurred.
			#if EASTL_ASSERT_ENABLED
				if(EASTL_UNLIKELY(validate_iterator(iFirst) & stl::isf_valid) == 0)
					EASTL_FAIL_MSG("bitvector::erase -- invalid iterator");
			#endif
		}
	...
	void bitvector<Allocator, Element, Container>::swap(this_type& rhs)
	{
		...
		stl::swap(mFreeBitCount, rhs.mFreeBitCount);
	}
