...
namespace stl
{
	...
	///
		...
		#define EASTL_RING_BUFFER_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " ring_buffer" // Unless the user overrides something, this is "stl ring_buffer".
	#endif
	...
	///
	template <typename T, typename Container = stl::vector<T>, typename Allocator = typename Container::allocator_type>
	class ring_buffer
	{
	public:
	...
	{
		typedef typename stl::iterator_traits<container_iterator>::iterator_category IC;
		increment(n, IC());
		return *this;
	}
		...
		: c(x.c)
	{
		...
		stl::advance(mBegin, stl::distance(const_cast<this_type&>(x).c.begin(), x.mBegin)); // We can do a simple distance algorithm here, as there will be no wraparound.
		stl::advance(mEnd,   stl::distance(const_cast<this_type&>(x).c.begin(), x.mEnd));
	}
	...
	void ring_buffer<T, Container, Allocator>::swap(this_type& x)
	{
		if(&x != this)
		{
			const difference_type dBegin  = stl::distance(c.begin(), mBegin); // We can do a simple distance algorithm here, as there will be no wraparound.
			const difference_type dEnd    = stl::distance(c.begin(), mEnd);

			const difference_type dxBegin = stl::distance(x.c.begin(), x.mBegin);
			const difference_type dxEnd   = stl::distance(x.c.begin(), x.mEnd);

			stl::swap(c, x.c);
			stl::swap(mSize, x.mSize);

			mBegin = c.begin();
			stl::advance(mBegin, dxBegin); // We can do a simple advance algorithm here, as there will be no wraparound.

			mEnd = c.begin();
			stl::advance(mEnd, dxEnd);

			x.mBegin = x.c.begin();
			stl::advance(x.mBegin, dBegin);

			x.mEnd = x.c.begin();
			stl::advance(x.mEnd, dEnd);
		}
	}
	...
	{
		// We could alternatively just use stl::distance() here, but we happen to
	...
	*/
	...
	ring_buffer<T, Container, Allocator>::DoGetSize(EASTL_ITC_NS::random_access_iterator_tag) const
	{
		...
		//     return stl::distance(mBegin, mEnd);
		//
		...
		//
		const difference_type dBegin = stl::distance(const_cast<Container&>(c).begin(), mBegin); // const_cast here solves a little compiler 
		const difference_type dEnd   = stl::distance(const_cast<Container&>(c).begin(), mEnd);   // argument matching problem.
	...
	*/
	...
	void ring_buffer<T, Container, Allocator>::resize(size_type n)
	{
		...
		if(n > cap) // If we need to grow in capacity...
		{
			...
			stl::copy(begin(), end(), cTemp.get().begin());
			stl::swap(c, cTemp.get());

			mBegin = c.begin();
			mEnd   = mBegin;
			stl::advance(mEnd, n); // We can do a simple advance algorithm on this because we know that mEnd will not wrap around.
		}
		else // We could do a check here for n != size(), but that would be costly and people don't usually resize things to their same size.
		{
			...
			// stl::advance(mEnd, n); // We *cannot* use this because there may be wraparound involved.
	...
	void ring_buffer<T, Container, Allocator>::set_capacity(size_type n)
	{
		...
		if(n != capacity)    // If we need to change capacity...
		{
			...
			{
				stl::advance(itCopyBegin, mSize - n);
				mSize = n;
			}

			stl::copy(itCopyBegin, end(), cTemp.get().begin());  // The begin-end range may in fact be larger than n, in which case values will be overwritten.
			stl::swap(c, cTemp.get());

			mBegin = c.begin();
			mEnd   = mBegin;
			stl::advance(mEnd, mSize); // We can do a simple advance algorithm on this because we know that mEnd will not wrap around.
		}
	}
	...
	ring_buffer<T, Container, Allocator>::insert(const_iterator position, const value_type& value)
	{
		...
		stl::copy_backward(itPosition, beforeEnd, end());
		*itPosition = value;
	...
	ring_buffer<T, Container, Allocator>::erase(const_iterator first, const_iterator last)
	{
		...
		typename iterator::difference_type d = stl::distance(itFirst, itLast);

		stl::copy(itLast, end(), itFirst);
	...
	inline bool ring_buffer<T, Container, Allocator>::validate() const
	{
		if(!c.validate())  // This requires that the container implement the validate function. That pretty much 
			return false;  // means that the container is an stl container and not a std STL container. 
