...
#include <stl/heap.h>
#include <stl/sort.h>             // For backwards compatibility due to sorts moved from here to sort.h.
...
namespace stl
{
	...
	inline void selection_sort(ForwardIterator first, ForwardIterator last)
	{
		...
		stl::selection_sort<ForwardIterator, Less>(first, last, Less());
	}
	...
	{
		typedef stl::less<typename stl::iterator_traits<BidirectionalIterator>::value_type> Less;

		stl::shaker_sort<BidirectionalIterator, Less>(first, last, Less());
	}
	...
	///  bucket_sort(intArray.begin(), intArray.end(), bucketArray, stl::hash_use_self<int>());
	///
	template <typename T>
	...
	void bucket_sort(ForwardIterator first, ForwardIterator last, ContainerArray& bucketArray, HashFunction hash /*= hash_use_self*/)
	{
		...
		for(typename ContainerArray::const_iterator iBucket = bucketArray.begin(); iBucket != bucketArray.end(); ++iBucket)
			first = stl::copy((*iBucket).begin(), (*iBucket).end(), first);
	}
