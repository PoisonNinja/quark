...
#include <stl/internal/fixed_pool.h>
...
namespace stl
{
	...
	///
		...
		#define EASTL_FIXED_VECTOR_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " fixed_vector" // Unless the user overrides something, this is "stl fixed_vector".
	#endif
	...
	///    fixedVector.clear();
	///
	template <typename T, size_t nodeCount, bool bEnableOverflow = true, typename OverflowAllocator = typename stl::type_select<bEnableOverflow, EASTLAllocatorType, EASTLDummyAllocatorType>::type>
	...
	inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::swap(this_type& x)
	{
		...
		{                                                                                                        // then we can do a fast pointer swap instead of content swap.
			...
			stl::swap(internalCapacityPtr(), x.internalCapacityPtr());
		}
		else
		{
	...
	inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::set_capacity(size_type n)
	{
		...
		if(n != nPrevCapacity)  // If the request results in a capacity change...
		{
			if(can_overflow() && (((uintptr_t)mpBegin != (uintptr_t)mBuffer.buffer) || (n > kMaxSize))) // If we are or would be using dynamically allocated memory instead of our fixed-size member buffer...
			{
				...
				stl::uninitialized_move_ptr(mpBegin, pCopyEnd, pNewData); // Move [mpBegin, pCopyEnd) to p.
	...
	{
		DoPushBackMove(typename type_select<bEnableOverflow, true_type, false_type>::type(), stl::move(value));
	}
	...
	{
		base_type::push_back(stl::move(value)); // This will call vector::push_back(value_type &&), and possibly swap value with *mpEnd.
	}
	...
	inline void fixed_vector<T, nodeCount, bEnableOverflow, OverflowAllocator>::DoPushBackMove(false_type, value_type&& value)
	{
		...
		::new((void*)mpEnd++) value_type(stl::move(value)); // This will call the value_type(value_type&&) constructor, and possibly swap value with *mpEnd.
	}
