...
bool validate_not_equal_to(const T& a, const T& b, Compare compare)
{
    return compare(a, b) ==
           compare(b,
                   a); // We want the not equal comparison results to be equal.
}
...
///
/// Example usage:
///     hash_set<const char*, hash<const char*>, str_equal_to<const char*> >
///     stringHashSet;
///
/// Note:
...
/// the comparison function. But str_equal_to tests an array of something
/// itself.
///
/// To consider: Update this code to use existing word-based comparison
/// optimizations, such as that used in the EAStdC Strcmp function.
///
template <typename T>
...
struct str_less : public binary_function<T, T, bool> {
    bool operator()(T a, T b) const
    {
        while (
            static_cast<
                typename make_unsigned<typename remove_pointer<T>::type>::type>(
                *a) ==
            static_cast<
                typename make_unsigned<typename remove_pointer<T>::type>::type>(
                *b)) {
        ...
        typename make_unsigned<char>::type aValueU =
            static_cast<typename make_unsigned<char>::type>(aValue);
        typename make_unsigned<char>::type bValueU =
            static_cast<typename make_unsigned<char>::type>(bValue);

        return aValueU < bValueU;

        // return (static_cast<typename make_unsigned<typename
        // remove_pointer<T>::type>::type>(*a) <
        //        static_cast<typename make_unsigned<typename
        //        remove_pointer<T>::type>::type>(*b));
    }
};
...
struct equal_to_2 : public binary_function<T, U, bool> {
    ...
    EA_CPP14_CONSTEXPR bool operator()(const U& b, const T& a)
        const // If you are getting a 'operator() already defined' error related
              // to on this line while compiling a
    {
        return b == a;
    } // hashtable class (e.g. hash_map), it's likely that you are using
      // hashtable::find_as when you should
};    // be using hashtable::find instead. The problem is that (const T, U)
      // collide. To do: make this work.
...
public:
    ...
    EA_CPP14_CONSTEXPR bool
    operator()(const typename Predicate::argument_type& a) const
    {
        return !mPredicate(a);
    }
};
...
template <typename Predicate>
inline EA_CPP14_CONSTEXPR unary_negate<Predicate>
...
public:
    ...
    EA_CPP14_CONSTEXPR bool
    operator()(const typename Predicate::first_argument_type& a,
...
template <typename Predicate>
inline EA_CPP14_CONSTEXPR binary_negate<Predicate>
not2(const Predicate& predicate)
{
...
public:
    ...
    typename Operation1::result_type
    operator()(typename Operation2::argument_type& x,
               typename Operation3::argument_type& y) const
    {
        return op1(op2(x), op3(y));
    }
};
...
///
/// Example usage:
///     ptrdiff_t Rand(ptrdiff_t n) { return rand() % n; } // Note: The C rand
///     function is poor and slow. pointer_to_unary_function<ptrdiff_t,
///     ptrdiff_t> randInstance(Rand); random_shuffle(pArrayBegin, pArrayEnd,
///     randInstance);
///
...
///
/// Example usage (actually, you don't need to use ptr_fun here, but it works
/// anyway):
///    int factorial(int x) { return (x > 1) ? (x * factorial(x - 1)) : x; }
///    transform(pIntArrayBegin, pIntArrayEnd, pIntArrayBegin,
///    ptr_fun(factorial));
///
template <typename Arg, typename Result>
...
///    transform(pIntArray1Begin, pIntArray1End, pIntArray2Begin,
///    pIntArray1Begin, ptr_fun(multiply));
///
template <typename Arg1, typename Arg2, typename Result>
...
/// The C++ standard is in error and this has been recognized by the defect
/// group.
///
template <typename Result, typename T>
...
template <typename Result, typename T>
inline const_mem_fun_t<Result, T> mem_fun(Result (T::*MemberFunction)() const)
{
...
///
template <typename Result, typename T>
inline mem_fun_ref_t<Result, T> mem_fun_ref(Result (T::*MemberFunction)())
{
...
template <typename Result, typename T>
inline const_mem_fun_ref_t<Result, T> mem_fun_ref(Result (T::*MemberFunction)()
                                                      const)
{
...
// The type name is not specified but it does have mandated functions that
...
template <typename F>
struct not_fn_ret {
    explicit not_fn_ret(F&& f)
        : mDecayF(stl::forward<F>(f))
    {
    }
    ...
    auto operator()(Args&&... args) & -> decltype(
        !stl::declval<stl::invoke_result_t<stl::decay_t<F>&, Args...>>())
    {
    ...
    auto operator()(Args&&... args) const& -> decltype(
        !stl::declval<stl::invoke_result_t<stl::decay_t<F> const&, Args...>>())
    {
        return !stl::invoke(mDecayF, stl::forward<Args>(args)...);
    }
...
/// not_fn
///
/// Creates an implementation specified functor that returns the complement of
/// the callable object it was passed. not_fn is intended to replace the
/// C++03-era negators stl::not1 and stl::not2.
///
/// http://en.cppreference.com/w/cpp/utility/functional/not_fn
///
/// Example usage:
...
template <typename T>
struct hash<T*> // Note that we use the pointer as-is and don't divide by
                // sizeof(T*). This is because the table is of a prime size and
                // this division doesn't benefit distribution.
{
...
//    - The hash function is significantly more efficient if it can make this
//    assumption.
...
//      special hash customized for such strings that's better than what we
//      provide.
///////////////////////////////////////////////////////////////////////////
...
struct hash<char8_t*> {
    size_t operator()(const char8_t* p) const
    {
        uint32_t c,
            result =
                2166136261U; // FNV1 hash. Perhaps the best string hash.
                             // Intentionally uint32_t instead of size_t, so the
                             // behavior is the same regardless of size.
        while ((c = (uint8_t)*p++) !=
               0) // Using '!=' disables compiler warnings.
...
struct hash<const char8_t*> {
    size_t operator()(const char8_t* p) const
    {
        uint32_t c,
            result =
                2166136261U; // Intentionally uint32_t instead of size_t, so the
                             // behavior is the same regardless of size.
