...
namespace stl
{
	...
	///
		...
		#define EASTL_HASH_MAP_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " hash_map" // Unless the user overrides something, this is "stl hash_map".
	#endif
	...
	///
		...
		#define EASTL_HASH_MULTIMAP_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " hash_multimap" // Unless the user overrides something, this is "stl hash_multimap".
	#endif
	...
	///
	template <typename Key, typename T, typename Hash = stl::hash<Key>, typename Predicate = stl::equal_to<Key>, 
			  typename Allocator = EASTLAllocatorType, bool bCacheHashCode = false>
	class hash_map
		: public hashtable<Key, stl::pair<const Key, T>, Allocator, stl::use_first<stl::pair<const Key, T> >, Predicate,
							Hash, mod_range_hashing, default_ranged_hash, prime_rehash_policy, bCacheHashCode, true, true>
	{
	public:
		typedef hashtable<Key, stl::pair<const Key, T>, Allocator, 
						  stl::use_first<stl::pair<const Key, T> >, 
	...
	public:
		...
		explicit hash_map(const allocator_type& allocator = EASTL_HASH_MAP_DEFAULT_ALLOCATOR)
			: base_type(0, Hash(), mod_range_hashing(), default_ranged_hash(), 
						Predicate(), stl::use_first<stl::pair<const Key, T> >(), allocator)
		{
			// Empty
		}
		...
		{
			return base_type::DoInsertKey(true_type(), stl::move(key));
		}
		...
		mapped_type& operator[](key_type&& key)
		{
			...
			return (*base_type::DoInsertKey(true_type(), stl::move(key)).first).second;
		}
	...
	public:
		...
		insert_return_type insert(key_type&& key)
		{
			return base_type::DoInsertKey(false_type(), stl::move(key));
		}
