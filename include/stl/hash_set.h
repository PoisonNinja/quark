...
#include <stl/internal/hashtable.h>
...
namespace stl
{
	...
	///
		...
		#define EASTL_HASH_SET_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " hash_set" // Unless the user overrides something, this is "stl hash_set".
	#endif
	...
	///
		...
		#define EASTL_HASH_MULTISET_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " hash_multiset" // Unless the user overrides something, this is "stl hash_multiset".
	#endif
	...
	///
	template <typename Value, typename Hash = stl::hash<Value>, typename Predicate = stl::equal_to<Value>, 
			  typename Allocator = EASTLAllocatorType, bool bCacheHashCode = false>
	class hash_set
		: public hashtable<Value, Value, Allocator, stl::use_self<Value>, Predicate,
						   Hash, mod_range_hashing, default_ranged_hash, 
						   prime_rehash_policy, bCacheHashCode, false, true>
	{
	public:
		typedef hashtable<Value, Value, Allocator, stl::use_self<Value>, Predicate, 
						  Hash, mod_range_hashing, default_ranged_hash,
	...
	public:
		...
		explicit hash_set(const allocator_type& allocator = EASTL_HASH_SET_DEFAULT_ALLOCATOR)
			: base_type(0, Hash(), mod_range_hashing(), default_ranged_hash(), Predicate(), stl::use_self<Value>(), allocator)
		{
			// Empty
		}
		...
		explicit hash_set(size_type nBucketCount, const Hash& hashFunction = Hash(), const Predicate& predicate = Predicate(), 
			...
			: base_type(nBucketCount, hashFunction, mod_range_hashing(), default_ranged_hash(), predicate, stl::use_self<Value>(), allocator)
		{
			// Empty
		}
		...
		hash_set(std::initializer_list<value_type> ilist, size_type nBucketCount = 0, const Hash& hashFunction = Hash(), 
			...
			: base_type(ilist.begin(), ilist.end(), nBucketCount, hashFunction, mod_range_hashing(), default_ranged_hash(), predicate, stl::use_self<Value>(), allocator)
		{
			// Empty
		}
		...
		hash_set(FowardIterator first, FowardIterator last, size_type nBucketCount = 0, const Hash& hashFunction = Hash(), 
				 const Predicate& predicate = Predicate(), const allocator_type& allocator = EASTL_HASH_SET_DEFAULT_ALLOCATOR)
			: base_type(first, last, nBucketCount, hashFunction, mod_range_hashing(), default_ranged_hash(), predicate, stl::use_self<Value>(), allocator)
		{
			// Empty
		}
		...
		this_type& operator=(std::initializer_list<value_type> ilist)
		{
			...
			{
				return static_cast<this_type&>(base_type::operator=(stl::move(x)));
			}
		#endif
						   ...
						   const hash_multiset<Value, Hash, Predicate, Allocator, bCacheHashCode>& b)
	{
		...
		typedef typename stl::iterator_traits<const_iterator>::difference_type difference_type;
		...
		stl::pair<const_iterator, const_iterator> aRange;
		stl::pair<const_iterator, const_iterator> bRange;
		...
		for(const_iterator ai = a.begin(), aiEnd = a.end(); ai != aiEnd; ai = aRange.second) // For each element in a...
		{
			...
			const difference_type aDistance = stl::distance(aRange.first, aRange.second);
			const difference_type bDistance = stl::distance(bRange.first, bRange.second);
			...
			else
			{
				...
				if(!stl::is_permutation(aRange.first, aRange.second, bRange.first))
					return false;
			}
		}
