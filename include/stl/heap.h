...
namespace stl
{
	...
	inline void promote_heap_impl(RandomAccessIterator first, Distance topPosition, Distance position, T value)
	{
		...
		{
			*(first + position) = stl::forward<ValueType>(*(first + parentPosition)); // Swap the node with its parent.
			position = parentPosition;
		}

		*(first + position) = stl::forward<ValueType>(value);
	}
	...
	{
		typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
		promote_heap_impl<RandomAccessIterator, Distance, T&&, value_type>(first, topPosition, position, stl::forward<T>(value));
	}
	...
	{
		typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
		promote_heap_impl<RandomAccessIterator, Distance, T&&, Compare, value_type>(first, topPosition, position, stl::forward<T>(value), compare);
	}
	...
	void adjust_heap_impl(RandomAccessIterator first, Distance topPosition, Distance heapSize, Distance position, T value)
	{
		...
		for(; childPosition < heapSize; childPosition = (2 * childPosition) + 2)
		{
			if(*(first + childPosition) < *(first + (childPosition - 1))) // Choose the larger of the two children.
				--childPosition;
			*(first + position) = stl::forward<ValueType>(*(first + childPosition)); // Swap positions with this child.
			position = childPosition;
		}

		if(childPosition == heapSize) // If we are at the very last index of the bottom...
		{
			*(first + position) = stl::forward<ValueType>(*(first + (childPosition - 1)));
			position = childPosition - 1;
		}

		stl::promote_heap<RandomAccessIterator, Distance, T>(first, topPosition, position, stl::forward<ValueType>(value));
	}
	...
	{
		typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
		adjust_heap_impl<RandomAccessIterator, Distance, const T&, const value_type>(first, topPosition, heapSize, position, stl::forward<const T&>(value));
	}
	...
	{
		typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
		adjust_heap_impl<RandomAccessIterator, Distance, T&&, value_type>(first, topPosition, heapSize, position, stl::forward<T>(value));
	}
	...
	void adjust_heap_impl(RandomAccessIterator first, Distance topPosition, Distance heapSize, Distance position, T value, Compare compare)
	{
		...
		stl::promote_heap<RandomAccessIterator, Distance, T, Compare>(first, topPosition, position, stl::forward<ValueType>(value), compare);
	}
	...
	{
		typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
		adjust_heap_impl<RandomAccessIterator, Distance, const T&, Compare, const value_type>(first, topPosition, heapSize, position, stl::forward<const T&>(value), compare);
	}
	...
	{
		typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
		adjust_heap_impl<RandomAccessIterator, Distance, T&&, Compare, value_type>(first, topPosition, heapSize, position, stl::forward<T>(value), compare);
	}
	...
	inline void push_heap(RandomAccessIterator first, RandomAccessIterator last)
	{
		...
		const value_type tempBottom(stl::forward<value_type>(*(last - 1)));

		stl::promote_heap<RandomAccessIterator, difference_type, value_type>
						   (first, (difference_type)0, (difference_type)(last - first - 1), stl::forward<const value_type>(tempBottom));
	}
	...
	inline void push_heap(RandomAccessIterator first, RandomAccessIterator last, Compare compare)
	{
		...
		stl::promote_heap<RandomAccessIterator, difference_type, value_type, Compare>
	...
	inline void pop_heap(RandomAccessIterator first, RandomAccessIterator last)
	{
		...
		value_type tempBottom(stl::forward<value_type>(*(last - 1)));
		*(last - 1) = stl::forward<value_type>(*first);
		stl::adjust_heap<RandomAccessIterator, difference_type, value_type>
						  (first, (difference_type)0, (difference_type)(last - first - 1), 0, stl::forward<value_type>(tempBottom));
	}
	...
	inline void pop_heap(RandomAccessIterator first, RandomAccessIterator last, Compare compare)
	{
		...
		*(last - 1) = stl::forward<value_type>(*first);
		stl::adjust_heap<RandomAccessIterator, difference_type, value_type, Compare>
						  (first, (difference_type)0, (difference_type)(last - first - 1), 0, stl::forward<value_type>(tempBottom), compare);
	}
	...
	void make_heap(RandomAccessIterator first, RandomAccessIterator last)
	{
		...
		if(heapSize >= 2) // If there is anything to do... (we need this check because otherwise the math fails below).
		{
			...
			do{
				--parentPosition;
				value_type temp(stl::forward<value_type>(*(first + parentPosition)));
				stl::adjust_heap<RandomAccessIterator, difference_type, value_type>
								  (first, parentPosition, heapSize, parentPosition, stl::forward<value_type>(temp));
	...
	void make_heap(RandomAccessIterator first, RandomAccessIterator last, Compare compare)
	{
		...
		if(heapSize >= 2) // If there is anything to do... (we need this check because otherwise the math fails below).
		{
			...
			do{
				...
				value_type temp(stl::forward<value_type>(*(first + parentPosition)));
				stl::adjust_heap<RandomAccessIterator, difference_type, value_type, Compare>
								  (first, parentPosition, heapSize, parentPosition, stl::forward<value_type>(temp), compare);
			} while(parentPosition != 0);
		}
	}
	...
	inline void sort_heap(RandomAccessIterator first, RandomAccessIterator last)
	{
		for(; (last - first) > 1; --last) // We simply use the heap to sort itself.
			stl::pop_heap<RandomAccessIterator>(first, last);
	}
	...
	{
		for(; (last - first) > 1; --last) // We simply use the heap to sort itself.
			stl::pop_heap<RandomAccessIterator, Compare>(first, last, compare);
	}
	...
	inline void change_heap(RandomAccessIterator first, Distance heapSize, Distance position)
	{
		...
		stl::remove_heap<RandomAccessIterator, Distance>(first, heapSize, position);
	...
	inline void change_heap(RandomAccessIterator first, Distance heapSize, Distance position, Compare compare)
	{
		...
		stl::remove_heap<RandomAccessIterator, Distance, Compare>(first, heapSize, position, compare);
	...
	{
		return (stl::is_heap_until(first, last) == last);
	}
	...
	{
		return (stl::is_heap_until(first, last, compare) == last);
	}
