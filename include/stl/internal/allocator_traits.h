...
namespace stl
{
	namespace Internal
	{
		...
		private:
			...
			template <class U> static stl::yes_type test(typename U::value_type* = 0);
		public:
		...
		{
			template <class U> static stl::no_type test(...);
			template <class U> static stl::yes_type test(typename U::pointer* = 0);
		}

		template <class T>
		struct has_pointer_type
			: public integral_constant<bool, sizeof(has_pointer_type_imp::test<T>(0)) == sizeof(stl::yes_type)>
		{
		};
		...
		private:
			template <class U> static stl::no_type test(...);
			template <class U> static stl::yes_type test(typename U::const_pointer* = 0);
		public:
		...
		private:
			template <class U> static stl::no_type test(...);
			template <class U> static stl::yes_type test(typename U::void_pointer* = 0);
		public:
			static const bool value = sizeof(test<Pointer>(0)) == sizeof(stl::yes_type);
		};
		...
		private:
			template <class U> static stl::no_type test(...);
			template <class U> static stl::yes_type test(typename U::const_void_pointer* = 0);
		public:
			static const bool value = sizeof(test<Pointer>(0)) == sizeof(stl::yes_type);
		};
        ...
        decltype(stl::declval<Alloc>().construct(stl::declval<T*>(), stl::declval<Args>()...), stl::true_type())
        has_construct_test(Alloc&& a, T* p, Args&&... args);

        template <class Alloc, class Pointer, class... Args>
        stl::false_type has_construct_test(const Alloc& a, Pointer&& p, Args&&... args);

        template <class Alloc, class Pointer, class... Args>
        struct has_construct
            : public stl::integral_constant< bool,
                  stl::is_same<decltype(has_construct_test(stl::declval<Alloc>(), stl::declval<Pointer>(),
				  stl::declval<Args>()...)),
                          stl::true_type>::value>
        {
        };
        ...
        decltype(stl::declval<Alloc>().construct(stl::declval<T*>(), stl::declval<Arg0>()), stl::true_type())
        has_construct_test(Alloc&& a, T* p, Arg0&& args);

        template <class Alloc, class Pointer, class Arg0>
        stl::false_type has_construct_test(const Alloc& a, Pointer&& p, Arg0&& args);
		...
		// has_destroy
        template <class Alloc, class Pointer>
        auto has_destroy_test(Alloc&& a, Pointer&& p) -> decltype(a.destroy(p), stl::true_type());

        template <class Alloc, class Pointer>
        auto has_destroy_test(const Alloc& a, Pointer&& p) -> stl::false_type;

        template <class Alloc, class Pointer>
        struct has_destroy
            : public stl::integral_constant< bool,
                  is_same<decltype(has_destroy_test(stl::declval<Alloc>(), stl::declval<Pointer>())), stl::true_type>::value>
        {
        };
		...
		// has_max_size
        template <class Alloc>
        auto has_max_size_test(Alloc&& a) -> decltype(a.max_size(), stl::true_type());

        template <class Alloc>
        auto has_max_size_test(const volatile Alloc& a) -> stl::false_type;

        template <class Alloc>
        struct has_max_size
            : public stl::integral_constant<bool,
                                is_same<decltype(has_max_size_test(stl::declval<Alloc&>())), stl::true_type>::value>
        {
        };
	...
	//
	// stl::allocator_traits supplies a uniform interface to all allocator types.
	//
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // stl::allocator_traits is not a standards conforming implementation. Enough of the standard was implemented to
    // make the stl::function implementation possible.  We must revisit this implementation before rolling out its
    // usage fully in stl::containers.
	//
	// NOTE: We do not recommend users directly code against stl::allocator_traits until we have completed a full standards comforming implementation.
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    template <class Alloc>
	struct allocator_traits
	{
	    ...
	    static size_type internal_max_size(false_type, const allocator_type&) { return (stl::numeric_limits<size_type>::max)(); }  // additional parenthesis disables the windows max macro from expanding.
	    ...
	    static void internal_construct(stl::true_type, allocator_type& a, T* p, Args&&... args)
	    {
		    a.construct(p, stl::forward<Args>(args)...);
	    }

	    template <class T, class... Args>
	    static void internal_construct(false_type, allocator_type&, T* p, Args&&... args)
	    {
		    ::new ((void*)p) T(stl::forward<Args>(args)...);
	    }

	    template <class T, class... Args>
		static void construct(allocator_type& a, T* p, Args&&... args)
		{
			internal_construct(Internal::has_construct<allocator_type, T*, Args...>(), a, p, stl::forward<Args>(args)...);
		}
	    ...
	    template <class T>
	    static void internal_destroy(stl::true_type, allocator_type& a, T* p) { a.destroy(p); }

	    template <class T>
	    static void internal_destroy(stl::false_type, allocator_type&, T* p) { p->~T(); }
