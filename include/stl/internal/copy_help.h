...
namespace stl
{
	...
	struct move_and_copy_helper<EASTL_ITC_NS::random_access_iterator_tag, false, false>
	{
		...
		static OutputIterator move_or_copy(InputIterator first, InputIterator last, OutputIterator result)
		{
			typedef typename stl::iterator_traits<InputIterator>::difference_type difference_type;
	...
	{
		typedef typename stl::iterator_traits<InputIterator>::iterator_category  IIC;
		typedef typename stl::iterator_traits<OutputIterator>::iterator_category OIC;
		typedef typename stl::iterator_traits<InputIterator>::value_type         value_type_input;
		typedef typename stl::iterator_traits<OutputIterator>::value_type        value_type_output;

		const bool canBeMemmoved = stl::is_trivially_copyable<value_type_output>::value && 
								   stl::is_same<value_type_input, value_type_output>::value && 
								  (stl::is_pointer<InputIterator>::value  || stl::is_same<IIC, stl::contiguous_iterator_tag>::value) && 
								  (stl::is_pointer<OutputIterator>::value || stl::is_same<OIC, stl::contiguous_iterator_tag>::value);

		return stl::move_and_copy_helper<IIC, isMove, canBeMemmoved>::move_or_copy(first, last, result); // Need to chose based on the input iterator tag and not the output iterator tag, because containers accept input ranges of iterator types different than self.
	}
	...
	{
		return OutputIterator(stl::move_and_copy_chooser<isMove>(stl::unwrap_iterator(first), stl::unwrap_iterator(last), stl::unwrap_iterator(result))); // Have to convert to OutputIterator because result.base() could be a T*
	}
	...
	///
	/// Example usage:
	///     stl::move(myArray.begin(), myArray.end(), myDestArray.begin());
	///
	...
	{
		return stl::move_and_copy_unwrapper<true>(stl::unwrap_iterator(first), stl::unwrap_iterator(last), result);
	}
	...
	{
		const bool isMove = stl::is_move_iterator<InputIterator>::value; EA_UNUSED(isMove);

		return stl::move_and_copy_unwrapper<isMove>(stl::unwrap_iterator(first), stl::unwrap_iterator(last), result);
	}
} // namespace stl
