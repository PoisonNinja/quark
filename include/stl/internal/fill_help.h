...
namespace stl
{
	...
	{
		stl::fill_imp< is_scalar<T>::value >::do_fill(first, last, value);
		...
		//const bool bEasyCopy = stl::type_and<stl::has_trivial_constructor<T>::value, 
		//                                       stl::integral_constant<bool, (sizeof(T) <= 16)>::value;
		//stl::fill_imp<bEasyCopy>::do_fill(first, last, value);
	...
	struct fill_n_imp<true>
	{
		...
		static OutputIterator do_fill(OutputIterator first, Size n, const T& value)
		{
			typedef typename stl::iterator_traits<OutputIterator>::value_type value_type;
	...
	{
		return stl::fill_n_imp<is_scalar<T>::value>::do_fill(first, n, value);
	}
