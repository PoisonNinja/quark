...
namespace stl
{
	...
	///
		...
		#define EASTL_HASHTABLE_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " hashtable" // Unless the user overrides something, this is "stl hashtable".
	#endif
	...
	/// EASTL_MACRO_SWAP
	///
	/// Use EASTL_MACRO_SWAP because GCC (at least v4.6-4.8) has a bug where it fails to compile stl::swap(mpBucketArray, x.mpBucketArray).
	///
	...
	namespace Internal
	{
		...
		{
		private:
			template <class U> static stl::no_type test(...);
			template <class U> static stl::yes_type test(decltype(U::mnHashCode)* = 0);
		public:
			static const bool value = sizeof(test<T>(0)) == sizeof(stl::yes_type);
		};
	}
	...
	#define ENABLE_IF_HAS_HASHCODE(T, RT) typename stl::enable_if<Internal::has_hashcode_member<T>::value, RT>::type*
	#define ENABLE_IF_HASHCODE_EASTLSIZET(T, RT) typename stl::enable_if<stl::is_convertible<T, eastl_size_t>::value, RT>::type
	#define ENABLE_IF_TRUETYPE(T) typename stl::enable_if<T::value>::type*
	#define DISABLE_IF_TRUETYPE(T) typename stl::enable_if<!T::value>::type*
	...
	{
		typedef typename stl::iterator_traits<Iterator>::iterator_category IC;
	...
	protected:
		...
		{
			stl::swap(mExtractKey, x.mExtractKey);
			stl::swap(mEqual,      x.mEqual);
			stl::swap(mRangedHash, x.mRangedHash);
		}
	...
	protected:
		...
		void base_swap(hash_code_base& x)
		{
			stl::swap(mExtractKey, x.mExtractKey);
			stl::swap(mEqual,      x.mEqual);
			stl::swap(m_h1,        x.m_h1);
			stl::swap(m_h2,        x.m_h2);
		}
	...
	public:
		...
		typedef typename type_select<bUniqueKeys, stl::pair<iterator, bool>, iterator>::type      insert_return_type;
		...
		enum
		{
			// This enumeration is deprecated in favor of stl::kHashtableAllocFlagBuckets.
			kAllocFlagBuckets = stl::kHashtableAllocFlagBuckets                  // Flag to allocator which indicates that we are allocating buckets and not nodes.
		};
	...
	public:
	   ...
	   ~hashtable();
		...
		size_type bucket_size(size_type n) const EA_NOEXCEPT
			{ return (size_type)stl::distance(begin(n), end(n)); }
	  ...
	  //iterator                               insert(const_iterator hint, node_type&& nh);
		...
		template <class P,
		          class = typename stl::enable_if_t<
					#if EASTL_ENABLE_PAIR_FIRST_ELEMENT_CONSTRUCTOR
		              !stl::is_same_v<stl::decay_t<P>, key_type> &&
					#endif
		              !stl::is_literal_type_v<P> &&
		              stl::is_constructible_v<value_type, P&&>>>
		...
		template <class M> stl::pair<iterator, bool> insert_or_assign(const key_type& k, M&& obj);
		template <class M> stl::pair<iterator, bool> insert_or_assign(key_type&& k, M&& obj);
		...
		stl::pair<iterator, iterator> find_range_by_hash(hash_code_t c);
		stl::pair<const_iterator, const_iterator> find_range_by_hash(hash_code_t c) const;

		size_type count(const key_type& k) const EA_NOEXCEPT;

		stl::pair<iterator, iterator>             equal_range(const key_type& k);
		stl::pair<const_iterator, const_iterator> equal_range(const key_type& k) const;
	...
	protected:
		...
		stl::pair<iterator, bool> DoInsertValue(BoolConstantT, Args&&... args);
		...
		template <typename BoolConstantT>
		stl::pair<iterator, bool> DoInsertValueExtra(BoolConstantT,
													   const key_type& k,
													   hash_code_t c,
													   node_type* pNodeNew,
													   value_type&& value,
		...
		template <typename BoolConstantT>
		stl::pair<iterator, bool> DoInsertValue(BoolConstantT,
												  value_type&& value,
												  ENABLE_IF_TRUETYPE(BoolConstantT) = nullptr);
		...
		stl::pair<iterator, bool> DoInsertKey(true_type, const key_type& key);
		iterator                    DoInsertKey(false_type, const key_type& key);
		stl::pair<iterator, bool> DoInsertKey(true_type, key_type&& key);
			...
			mAllocator(allocator)
	{
		if(nBucketCount < 2)
		{
			const size_type nElementCount = (size_type)stl::ht_distance(first, last);
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateNodeFromKey(const key_type& key)
	{
		...
		EASTL_ASSERT_MSG(pNode != nullptr, "the behaviour of stl::allocators that return nullptr is not defined.");

		#if EASTL_EXCEPTIONS_ENABLED
			try
			{
		#endif
				::new(stl::addressof(pNode->mValue)) value_type(pair_first_construct, key);
				pNode->mpNext = NULL;
				return pNode;
		#if EASTL_EXCEPTIONS_ENABLED
			}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateNodeFromKey(key_type&& key)
	{
		...
		EASTL_ASSERT_MSG(pNode != nullptr, "the behaviour of stl::allocators that return nullptr is not defined.");
			...
			{
		#endif
				::new(stl::addressof(pNode->mValue)) value_type(pair_first_construct, stl::move(key));
				pNode->mpNext = NULL;
				return pNode;
		#if EASTL_EXCEPTIONS_ENABLED
			}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateBuckets(size_type n)
	{
		...
		//stl::fill(pBucketArray, pBucketArray + n, (node_type*)NULL);
	...
	void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::swap(this_type& x)
	{
		...
		stl::swap(mRehashPolicy, x.mRehashPolicy);
		EASTL_MACRO_SWAP(node_type**, mpBucketArray, x.mpBucketArray);
		stl::swap(mnBucketCount, x.mnBucketCount);
		stl::swap(mnElementCount, x.mnElementCount);

		if (mAllocator != x.mAllocator) // If allocators are not equivalent...
		{
			stl::swap(mAllocator, x.mAllocator);
		}
	}
	...
	inline typename H::iterator hashtable_find(H& hashTable, U u)
		{ return hashTable.find_as(u, stl::hash<U>(), stl::equal_to_2<const typename H::key_type, U>()); }
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::find_as(const U& other)
		{ return stl::hashtable_find(*this, other); }
		...
		//{ return find_as(other, stl::hash<U>(), stl::equal_to_2<const key_type, U>()); }
	...
	template <typename K, typename V, typename A, typename EK, typename Eq, 
			  typename H1, typename H2, typename H, typename RP, bool bC, bool bM, bool bU>
	stl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::const_iterator,
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::find_range_by_hash(hash_code_t c) const
	{
		...
		if (pNodeStart)
		{
			stl::pair<const_iterator, const_iterator> pair(const_iterator(pNodeStart, mpBucketArray + start), 
															 const_iterator(pNodeStart, mpBucketArray + start));
			pair.second.increment_bucket();
			return pair;
		}

		return stl::pair<const_iterator, const_iterator>(const_iterator(mpBucketArray + mnBucketCount),
	...
	template <typename K, typename V, typename A, typename EK, typename Eq, 
			  typename H1, typename H2, typename H, typename RP, bool bC, bool bM, bool bU>
	stl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator,
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::find_range_by_hash(hash_code_t c)
	{
		...
		if (pNodeStart)
		{
			stl::pair<iterator, iterator> pair(iterator(pNodeStart, mpBucketArray + start), 
		...
		return stl::pair<iterator, iterator>(iterator(mpBucketArray + mnBucketCount),
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::equal_range(const key_type& k)
	{
		...
		if(pNode)
		{
			...
			return stl::pair<iterator, iterator>(first, last);
		}

		return stl::pair<iterator, iterator>(iterator(mpBucketArray + mnBucketCount),  // iterator(mpBucketArray + mnBucketCount) == end()
											   iterator(mpBucketArray + mnBucketCount));
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::equal_range(const key_type& k) const
	{
		...
		if(pNode)
		{
			...
			return stl::pair<const_iterator, const_iterator>(first, last);
		}

		return stl::pair<const_iterator, const_iterator>(const_iterator(mpBucketArray + mnBucketCount),  // iterator(mpBucketArray + mnBucketCount) == end()
														   const_iterator(mpBucketArray + mnBucketCount));
	}
	...
	stl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator, bool>
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(BoolConstantT, Args&&... args) // true_type means bUniqueKeys is true.
	{
		...
		node_type* const  pNodeNew = DoAllocateNode(stl::forward<Args>(args)...);
		...
		{
			const stl::pair<bool, uint32_t> bRehash = mRehashPolicy.GetRehashRequired((uint32_t)mnBucketCount, (uint32_t)mnElementCount, (uint32_t)1);
			...
			set_code(pNodeNew, c); // This is a no-op for most hashtables.
				...
				try
				{
					...
					return stl::pair<iterator, bool>(iterator(pNodeNew, mpBucketArray + n), true);
			#if EASTL_EXCEPTIONS_ENABLED
				}
				catch(...)
				{
		...
		return stl::pair<iterator, bool>(iterator(pNode, mpBucketArray + n), false);
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(BoolConstantT, Args&&... args) // false_type means bUniqueKeys is false.
	{
		...
		node_type*        pNodeNew = DoAllocateNode(stl::forward<Args>(args)...);
		const key_type&   k        = mExtractKey(pNodeNew->mValue);
		const hash_code_t c        = get_hash_code(k);
		const size_type   n        = (size_type)bucket_index(k, c, (uint32_t)mnBucketCount);
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateNode(Args&&... args)
	{
		...
		EASTL_ASSERT_MSG(pNode != nullptr, "the behaviour of stl::allocators that return nullptr is not defined.");
			...
			{
		#endif
				::new(stl::addressof(pNode->mValue)) value_type(stl::forward<Args>(args)...);
				pNode->mpNext = NULL;
				return pNode;
		#if EASTL_EXCEPTIONS_ENABLED
			}
		...
		hash_code_t c, node_type* pNodeNew, value_type&& value, ENABLE_IF_TRUETYPE(BoolConstantT)) // true_type means bUniqueKeys is true.
	{
		...
		if(pNode == NULL) // If value is not present... add it.
		{
			...
			if(pNodeNew)
			{
				::new(stl::addressof(pNodeNew->mValue)) value_type(stl::move(value)); // It's expected that pNodeNew was allocated with allocate_uninitialized_node.
				#if EASTL_EXCEPTIONS_ENABLED
					nodeAllocated = false;
				#endif
			}
			...
			{
				pNodeNew = DoAllocateNode(stl::move(value));
				#if EASTL_EXCEPTIONS_ENABLED
					nodeAllocated = true;
				#endif
			}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(BoolConstantT, value_type&& value, ENABLE_IF_TRUETYPE(BoolConstantT)) // true_type means bUniqueKeys is true.
	{
		...
		return DoInsertValueExtra(true_type(), k, c, NULL, stl::move(value));
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(BoolConstantT, value_type&& value, DISABLE_IF_TRUETYPE(BoolConstantT)) // false_type means bUniqueKeys is false.
	{
		...
		return DoInsertValueExtra(false_type(), k, c, NULL, stl::move(value));
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateNode(value_type&& value)
	{
		...
		EASTL_ASSERT_MSG(pNode != nullptr, "the behaviour of stl::allocators that return nullptr is not defined.");
			...
			{
		#endif
				::new(stl::addressof(pNode->mValue)) value_type(stl::move(value));
				pNode->mpNext = NULL;
				return pNode;
		#if EASTL_EXCEPTIONS_ENABLED
			}
			...
			ENABLE_IF_TRUETYPE(BoolConstantT)) // true_type means bUniqueKeys is true.
	{
		...
		if(pNode == NULL) // If value is not present... add it.
		{
			...
			if(pNodeNew)
			{
				::new(stl::addressof(pNodeNew->mValue)) value_type(value); // It's expected that pNodeNew was allocated with allocate_uninitialized_node.
				#if EASTL_EXCEPTIONS_ENABLED
					nodeAllocated = false;
				#endif
			}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateNode(const value_type& value)
	{
		...
		EASTL_ASSERT_MSG(pNode != nullptr, "the behaviour of stl::allocators that return nullptr is not defined.");
			...
			{
		#endif
				::new(stl::addressof(pNode->mValue)) value_type(value);
				pNode->mpNext = NULL;
				return pNode;
		#if EASTL_EXCEPTIONS_ENABLED
			}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertKey(true_type, key_type&& key) // true_type means bUniqueKeys is true.
	{
		...
		if(pNode == NULL)
		{
			...
			// Allocate the new node before doing the rehash so that we don't
			// do a rehash if the allocation throws.
			node_type* const pNodeNew = DoAllocateNodeFromKey(stl::move(key));
			set_code(pNodeNew, c); // This is a no-op for most hashtables.
	...
	{
		return DoInsertValue(has_unique_keys_type(), stl::forward<Args>(args)...); // Need to use forward instead of move because Args&& is a "universal reference" instead of an rvalue reference.
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::emplace_hint(const_iterator, Args&&... args)
	{
		...
		insert_return_type result = DoInsertValue(has_unique_keys_type(), stl::forward<Args>(args)...);
	...
	template <typename K, typename V, typename A, typename EK, typename Eq,
	          typename H1, typename H2, typename H, typename RP, bool bC, bool bM, bool bU>
	template <class... Args>
	// inline stl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator, bool>
	...
	{
		return DoInsertValue(has_unique_keys_type(), piecewise_construct, forward_as_tuple(stl::move(key)),
		                     forward_as_tuple(forward<Args>(args)...));
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::try_emplace(const_iterator, key_type&& key, Args&&... args)
	{
		insert_return_type result =
		    DoInsertValue(has_unique_keys_type(), value_type(piecewise_construct, forward_as_tuple(stl::move(key)),
		                                                     forward_as_tuple(forward<Args>(args)...)));
	...
	{
		return DoInsertValue(has_unique_keys_type(), stl::move(otherValue));
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(hash_code_t c, node_type* pNodeNew, P&& otherValue)
	{
		...
		value_type value(stl::forward<P>(otherValue)); // Need to use forward instead of move because P&& is a "universal reference" instead of an rvalue reference.
		const key_type& k = mExtractKey(value);
		return DoInsertValueExtra(has_unique_keys_type(), k, c, pNodeNew, stl::move(value));
	}
	...
	{
		// We currently ignore the iterator argument as a hint.
		insert_return_type result = DoInsertValue(has_unique_keys_type(), value_type(stl::move(value)));
		return DoGetResultIterator(has_unique_keys_type(), result);
	}
	...
	{
		return emplace(stl::forward<P>(otherValue));
	}
	...
	{
		const uint32_t nElementAdd = (uint32_t)stl::ht_distance(first, last);
		const stl::pair<bool, uint32_t> bRehash = mRehashPolicy.GetRehashRequired((uint32_t)mnBucketCount, (uint32_t)mnElementCount, nElementAdd);
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert_or_assign(const key_type& k, M&& obj)
	{
		...
		if(iter == end())
		{
			return insert(value_type(piecewise_construct, forward_as_tuple(k), forward_as_tuple(stl::forward<M>(obj))));
		}
		else
		{
			iter->second = stl::forward<M>(obj);
			return {iter, false};
		}
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert_or_assign(key_type&& k, M&& obj)
	{
		...
		if(iter == end())
		{
			return insert(value_type(piecewise_construct, forward_as_tuple(stl::move(k)), forward_as_tuple(stl::forward<M>(obj))));
		}
		else
		{
			iter->second = stl::forward<M>(obj);
	...
	{
		return insert_or_assign(k, stl::forward<M>(obj)).first; // we ignore the iterator hint
	}
	...
	{
		return insert_or_assign(stl::move(k), stl::forward<M>(obj)).first; // we ignore the iterator hint
	}
