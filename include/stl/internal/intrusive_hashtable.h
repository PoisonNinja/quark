...
namespace stl
{
	...
	public:
		...
		typedef typename type_select<bConstIterators, stl::use_self<Value>, 
												 stl::use_intrusive_key<Value, key_type> >::type  extract_key;
	...
	protected:
		stl::pair<iterator, bool> DoInsertValue(value_type&, true_type);  // true_type means bUniqueKeys is true.
	...
	void intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::swap(this_type& x)
	{
		for(size_t i = 0; i < kBucketCount; i++)
			stl::swap(mBucketArray[i], x.mBucketArray[i]);

		stl::swap(mnElementCount, x.mnElementCount);
		stl::swap(mHash,          x.mHash);
	...
	intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::find_as(const U& other)
		{ return stl::intrusive_hashtable_find(*this, other); }
	...
	template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM, bool bU>
	stl::pair<typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::iterator,
	...
	intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::equal_range(const key_type& k)
	{
		...
		return stl::pair<iterator, iterator>(iterator(mBucketArray + kBucketCount),
	...
	template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM, bool bU>
	stl::pair<typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::const_iterator,
	...
	intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::equal_range(const key_type& k) const
	{
		...
		return stl::pair<const_iterator, const_iterator>(const_iterator(const_cast<node_type**>(mBucketArray) + kBucketCount),
	...
	template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM, bool bU>
	stl::pair<typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::iterator, bool>
	intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::DoInsertValue(value_type& value, true_type) // true_type means bUniqueKeys is true.
	{
		...
		if(pNode == NULL)
		{
			...
			return stl::pair<iterator, bool>(iterator(&value, mBucketArray + n), true);
		}

		return stl::pair<iterator, bool>(iterator(pNode, mBucketArray + n), false);
	}
