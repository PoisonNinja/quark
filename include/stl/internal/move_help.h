...
// such cases. For example, stl::move (same as std::move) can be treated
// as a no-op under C++03, though with the consequence that move functionality
// isn't taken advantage of.


/// EASTL_MOVE
/// Acts like stl::move when possible. Same as C++11 std::move.
///
/// EASTL_MOVE_INLINE
/// Acts like stl::move but is implemented inline instead of a function call.
/// This allows code to be faster in debug builds in particular.
/// Depends on C++ compiler decltype support or a similar extension.
///
/// EASTL_FORWARD
/// Acts like stl::forward when possible. Same as C++11 std::forward.
///
/// EASTL_FORWARD_INLINE
/// Acts like stl::forward but is implemented inline instead of a function call.
/// This allows code to be faster in debug builds in particular.
///
#if EASTL_MOVE_SEMANTICS_ENABLED
	#define EASTL_MOVE(x)              stl::move(x)
	#if !defined(EA_COMPILER_NO_DECLTYPE)
		#define EASTL_MOVE_INLINE(x)   static_cast<typename stl::remove_reference<decltype(x)>::type&&>(x)
	#elif defined(__GNUC__)
		#define EASTL_MOVE_INLINE(x)   static_cast<typename stl::remove_reference<__typeof__(x)>::type&&>(x)
	#else
		#define EASTL_MOVE_INLINE(x)   stl::move(x)
	#endif

	#define EASTL_FORWARD(T, x)        stl::forward<T>(x)
	#define EASTL_FORWARD_INLINE(T, x) stl::forward<T>(x)  // Need to investigate how to properly make a macro for this. (stl::is_reference<T>::value ? static_cast<T&&>(static_cast<T&>(x)) : static_cast<T&&>(x))
#else
...
/// EASTL_MOVE_RANGE
/// Acts like the stl::move algorithm when possible. Same as C++11 std::move.
...
///
#if EASTL_MOVE_SEMANTICS_ENABLED
	#define EASTL_MOVE_RANGE(first, last, result)             stl::move(first, last, result)
	#define EASTL_MOVE_BACKWARD_RANGE(first, last, resultEnd) stl::move_backward(first, last, resultEnd)
#else
	#define EASTL_MOVE_RANGE(first, last, result)             stl::copy(first, last, result)
	#define EASTL_MOVE_BACKWARD_RANGE(first, last, result)    stl::copy_backward(first, last, result)
#endif
...
namespace stl
{
		...
		//         { foo(stl::forward<T>(arg)); } 
		//
		//     template <class... Args>
		//     void WrapperFunction(Args&&... args)
		//         { foo(stl::forward<Args>(args)...); } 
		//
		// See the C++ Standard, section 20.2.3
		// http://en.cppreference.com/w/cpp/utility/forward
		//
		template <typename T>
		EA_CPP14_CONSTEXPR T&& forward(typename stl::remove_reference<T>::type& x) EA_NOEXCEPT
		{
		...
		//
		template <typename T>
		EA_CPP14_CONSTEXPR typename stl::remove_reference<T>::type&&
		...
		//
			...
			template <typename T> 
			EA_CPP14_CONSTEXPR typename stl::conditional<!stl::is_nothrow_move_constructible<T>::value && 
															stl::is_copy_constructible<T>::value, const T&, T&&>::type 
