...
namespace stl
{
	...
	///
		...
		#define EASTL_RBTREE_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " rbtree" // Unless the user overrides something, this is "stl rbtree".
	#endif
	...
	/// will be the same as each other and ExtractKey will be stl::use_self.
	///
	...
	/// other and ExtractKey will be stl::use_self.
	///
	...
	struct rb_base<Key, Pair, Compare, stl::use_first<Pair>, true, RBTree>
	{
		typedef stl::use_first<Pair> extract_key;
	...
	///
	template <typename Key, typename Pair, typename Compare, typename RBTree>
	struct rb_base<Key, Pair, Compare, stl::use_first<Pair>, false, RBTree>
	{
		typedef stl::use_first<Pair> extract_key;
	...
	/// the node is just the value. ExtractKey will be either stl::use_first (map and multimap)
	/// or stl::use_self (set and multiset).
	///
	...
	public:
		...
		typedef typename type_select<bUniqueKeys, stl::pair<iterator, bool>, iterator>::type  insert_return_type;  // map/set::insert return a pair, multimap/multiset::iterator return an iterator.
	...
	public:
		...
		template <class... Args> stl::pair<iterator, bool> try_emplace(const key_type& k, Args&&... args);
		template <class... Args> stl::pair<iterator, bool> try_emplace(key_type&& k, Args&&... args);
		...
		template <class P, class = typename stl::enable_if<stl::is_constructible<value_type, P&&>::value>::type> 
	...
	protected:
		...
		template <class... Args>
		stl::pair<iterator, bool> DoInsertValue(true_type, Args&&... args);

		template <class... Args>
		iterator DoInsertValue(false_type, Args&&... args);

		stl::pair<iterator, bool> DoInsertValue(true_type, value_type&& value);
	...
	rbtree<K, V, C, A, E, bM, bU>::operator=(std::initializer_list<value_type> ilist)
	{
		...
		for(typename std::initializer_list<value_type>::iterator it = ilist.begin(), itEnd = ilist.end(); it != itEnd; ++it)
			DoInsertValue(has_unique_keys_type(), stl::move(*it));
	...
	void rbtree<K, V, C, A, E, bM, bU>::swap(this_type& x)
	{
		if(mAllocator == x.mAllocator) // If allocators are equivalent...
		{
			...
			stl::swap(mnSize,              x.mnSize);
			stl::swap(base_type::mCompare, x.mCompare);
			...
			{
				stl::swap(mAnchor.mpNodeRight,  x.mAnchor.mpNodeRight);
				stl::swap(mAnchor.mpNodeLeft,   x.mAnchor.mpNodeLeft);
				stl::swap(mAnchor.mpNodeParent, x.mAnchor.mpNodeParent);
	...
	{
		return DoInsertValue(has_unique_keys_type(), stl::forward<Args>(args)...);
	}
	...
	{
		return DoInsertValueHint(has_unique_keys_type(), position, stl::forward<Args>(args)...);
	}

	template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
	template <class... Args>
	inline stl::pair<typename rbtree<K, V, C, A, E, bM, bU>::iterator, bool>
	...
	{
		return DoInsertValue(has_unique_keys_type(), piecewise_construct, forward_as_tuple(stl::move(key)), forward_as_tuple(forward<Args>(args)...));
	}
	...
	{
		return DoInsertValueHint(has_unique_keys_type(), position, value_type(stl::move(value)));
	}
	...
	template <typename K, typename V, typename C, typename A, typename E, bool bM, bool bU>
	template <class M>
	stl::pair<typename rbtree<K, V, C, A, E, bM, bU>::iterator, bool>
	...
	rbtree<K, V, C, A, E, bM, bU>::insert_or_assign(const_iterator hint, const key_type& k, M&& obj)
	{
		...
		if(iter == end())
		{
			return insert(hint, value_type(piecewise_construct, forward_as_tuple(k), forward_as_tuple(stl::forward<M>(obj))));
		}
		else
		{
			iter->second = stl::forward<M>(obj);
	...
	rbtree<K, V, C, A, E, bM, bU>::insert_or_assign(const_iterator hint, key_type&& k, M&& obj)
	{
		...
		if(iter == end())
		{
			return insert(hint, value_type(piecewise_construct, forward_as_tuple(stl::move(k)), forward_as_tuple(stl::forward<M>(obj))));
		}
		else
		{
			iter->second = stl::forward<M>(obj);
	...
	rbtree<K, V, C, A, E, bM, bU>::DoInsertValue(true_type, value_type&& value)
	{
		...
		if(canInsert)
		{
			const iterator itResult(DoInsertValueImpl(pPosition, false, key, stl::move(value)));
	...
	rbtree<K, V, C, A, E, bM, bU>::DoInsertValue(false_type, value_type&& value)
	{
		...
		return DoInsertValueImpl(pPosition, false, key, stl::move(value));
	}
	...
	rbtree<K, V, C, A, E, bM, bU>::DoInsertValue(true_type, Args&&... args) // true_type means keys are unique.
	{
		...
		node_type* pNodeNew = DoCreateNode(stl::forward<Args>(args)...); // Note that pNodeNew->mpLeft, mpRight, mpParent, will be uninitialized.
	...
	rbtree<K, V, C, A, E, bM, bU>::DoInsertValue(false_type, Args&&... args) // false_type means keys are not unique.
	{
		...
		// To do: Change this so that we call DoCreateNode(stl::forward<Args>(args)...) here and use the value from the resulting pNode to get the 
		// key, and make DoInsertValueImpl take that node as an argument. That way there is no value created on the stack.

		node_type* const pNodeNew = DoCreateNode(stl::forward<Args>(args)...); // Note that pNodeNew->mpLeft, mpRight, mpParent, will be uninitialized.
		const key_type& key = extract_key{}(pNodeNew->mValue);
	...
	{
		return (a.size() == b.size()) && stl::equal(a.begin(), a.end(), b.begin());
	}
