...
namespace stl
{
	...
	namespace Internal
	{
		...
		public:
			typedef decltype(test<typename stl::remove_reference<Deleter>::type>(0)) type;
		};
		...
		template <typename P1, typename P2, bool = stl::is_same<typename stl::remove_cv<typename pointer_traits<P1>::element_type>::type,
																  typename stl::remove_cv<typename pointer_traits<P2>::element_type>::type>::value>
		struct is_array_cv_convertible_impl 
			: public stl::is_convertible<P1, P2> {};  // Return true if P1 is convertible to P2.

		template <typename P1, typename P2>
		struct is_array_cv_convertible_impl<P1, P2, false> 
			: public stl::false_type {};              // P1's underlying type is not the same as P2's, so it can't be converted, even if P2 refers to a subclass of P1. Parent == Child, but Parent[] != Child[]

		template <typename P1, typename P2, bool = stl::is_scalar<P1>::value && !stl::is_pointer<P1>::value>
		struct is_array_cv_convertible
			: public is_array_cv_convertible_impl<P1, P2> {};

		template <typename P1, typename P2>
		struct is_array_cv_convertible<P1, P2, true>
			: public stl::false_type {};              // P1 is scalar not a pointer, so it can't be converted to a pointer.
		...
		///////////////////////////////////////////////////////////////////////
			...
			struct is_derived : public stl::integral_constant<bool, stl::is_base_of<Base, Derived>::value && !stl::is_same<typename stl::remove_cv<Base>::type, typename stl::remove_cv<Derived>::type>::value> {};
		#else
			...
			struct is_derived : public stl::integral_constant<bool, !stl::is_same<typename stl::remove_cv<Base>::type, typename stl::remove_cv<Derived>::type>::value> {};
		#endif
		...
		struct is_safe_array_conversion : public stl::integral_constant<bool, stl::is_convertible<U_pointer, T_pointer>::value &&
																				stl::is_array<U>::value && 
																				(!stl::is_pointer<U_pointer>::value || !is_pointer<T_pointer>::value || !Internal::is_derived<T, typename stl::remove_extent<U>::type>::value)> {};
	...
	struct default_delete
	{
		...
		default_delete(const default_delete<U>&, typename stl::enable_if<is_convertible<U*, T*>::value>::type* = 0) EA_NOEXCEPT {}
	...
	struct default_delete<T[]> // Specialization for arrays.
	{
		...
		default_delete(const default_delete<U[]>&, typename stl::enable_if<Internal::is_array_cv_convertible<U*, T*>::value>::type* = 0) EA_NOEXCEPT {}
