...
namespace stl
{
	...
	struct has_void_arg<A0, Args...>
		{ static const bool value = (stl::is_void<A0>::value || stl::has_void_arg<Args...>::value); };
	...
	// stl::is_null_pointer<nullptr>::value == true
	// stl::is_null_pointer<std::nullptr_t>::value == true
	// stl::is_null_pointer<void*>::value == false
	// stl::is_null_pointer<NULL>::value == [cannot compile]
	//
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct is_null_pointer : public stl::is_same<typename stl::remove_cv<T>::type, decltype(nullptr)> {}; // A C++11 compiler defines nullptr, but you need a C++11 standard library to declare std::nullptr_t. So it's safer to compare against decltype(nullptr) than to use std::nullptr_t, because we may have a C++11 compiler but C++98 library (happens with Apple frequently).
	#else
		#define EASTL_TYPE_TRAIT_is_null_pointer_CONFORMANCE 1

		template <typename T> 
		struct is_null_pointer : public stl::is_same<typename stl::remove_cv<T>::type, std::nullptr_t> {};
	#endif
	...
	template <typename T>
	struct is_integral : public stl::is_integral_helper<typename stl::remove_cv<T>::type>{};
	...
	template <typename T>
	struct is_floating_point : public stl::is_floating_point_helper<typename stl::remove_cv<T>::type>{};
	...
	template <typename T>
	struct is_hat_type : public stl::is_hat_type_helper<T> {};
