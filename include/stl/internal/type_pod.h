...
namespace stl
{
	...
	///////////////////////////////////////////////////////////////////////
		...
		struct is_empty_helper : public stl::false_type{};

		template <typename T>
		struct is_empty_helper<T, true> : public stl::integral_constant<bool,
		...
		{
			typedef typename stl::remove_cv<T>::type _T;
			typedef stl::is_empty_helper<_T, stl::is_class<_T>::value> type;
		};

		template <typename T> 
		struct is_empty : public stl::is_empty_helper2<T>::type {};
	#endif
	...
	///////////////////////////////////////////////////////////////////////
		...
		struct is_pod : public stl::integral_constant<bool, (__has_trivial_constructor(T) && __is_pod(T) && !stl::is_hat_type<T>::value) || stl::is_void<T>::value || stl::is_scalar<T>::value>{};
		...
		template <typename T> 
		struct is_pod : public stl::integral_constant<bool, __is_pod(T) || stl::is_void<T>::value || stl::is_scalar<T>::value>{};
	#else
		...
		struct is_pod : public stl::integral_constant<bool, stl::is_void<T>::value || stl::is_scalar<typename stl::remove_all_extents<T>::type>::value>{};
	#endif
	...
	#define EASTL_DECLARE_IS_POD(T, isPod)                                                                   \
		namespace stl {                                                                                    \
			template <> struct is_pod<T>                : public stl::integral_constant<bool, isPod>  { }; \
			template <> struct is_pod<const T>          : public stl::integral_constant<bool, isPod>  { }; \
			template <> struct is_pod<volatile T>       : public stl::integral_constant<bool, isPod>  { }; \
			template <> struct is_pod<const volatile T> : public stl::integral_constant<bool, isPod>  { }; \
		}

	// Old style macro, for bacwards compatibility:
	#define EASTL_DECLARE_POD(T) namespace stl{ template <> struct is_pod<T> : public true_type{}; template <> struct is_pod<const T> : public true_type{}; }
	...
	//
		...
		template <typename T> 
		struct is_standard_layout : public stl::integral_constant<bool, __is_standard_layout(T) || stl::is_void<T>::value || stl::is_scalar<T>::value>{};
	#else
		#define EASTL_TYPE_TRAIT_is_standard_layout_CONFORMANCE 0    // is_standard_layout is not conforming. Can return false negatives.

		template <typename T> // There's not much we can do here without some compiler extension.
		struct is_standard_layout : public stl::integral_constant<bool, is_void<T>::value || is_scalar<T>::value>{};
	#endif

	#define EASTL_DECLARE_IS_STANDARD_LAYOUT(T, isStandardLayout)                                                    \
		namespace stl {                                                                                            \
			template <> struct is_standard_layout<T>                : public stl::integral_constant<bool, isStandardLayout>  { }; \
			template <> struct is_standard_layout<const T>          : public stl::integral_constant<bool, isStandardLayout>  { }; \
			template <> struct is_standard_layout<volatile T>       : public stl::integral_constant<bool, isStandardLayout>  { }; \
			template <> struct is_standard_layout<const volatile T> : public stl::integral_constant<bool, isStandardLayout>  { }; \
		}

	// Old style macro, for bacwards compatibility:
	#define EASTL_DECLARE_STANDARD_LAYOUT(T) namespace stl{ template <> struct is_standard_layout<T> : public true_type{}; template <> struct is_standard_layout<const T> : public true_type{}; }
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct has_trivial_constructor : public stl::integral_constant<bool, (__has_trivial_constructor(T) || stl::is_pod<T>::value) && !stl::is_hat_type<T>::value>{};
	#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_CLANG))
		#define EASTL_TYPE_TRAIT_has_trivial_constructor_CONFORMANCE 1    // has_trivial_constructor is conforming.

		template <typename T> 
		struct has_trivial_constructor : public stl::integral_constant<bool, __has_trivial_constructor(T) || stl::is_pod<T>::value>{};
	#else
		...
		template <typename T> 
		struct has_trivial_constructor : public stl::is_pod<T> {};
	#endif

	#define EASTL_DECLARE_HAS_TRIVIAL_CONSTRUCTOR(T, hasTrivialConstructor)                                                     \
		namespace stl {                                                                                                       \
			template <> struct has_trivial_constructor<T>  : public stl::integral_constant<bool, hasTrivialConstructor>  { }; \
		}

	// Old style macro, for bacwards compatibility:
	#define EASTL_DECLARE_TRIVIAL_CONSTRUCTOR(T) namespace stl{ template <> struct has_trivial_constructor<T> : public true_type{}; template <> struct has_trivial_constructor<const T> : public true_type{}; }
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct has_trivial_copy : public stl::integral_constant<bool, (__has_trivial_copy(T) || stl::is_pod<T>::value) && !stl::is_volatile<T>::value && !stl::is_hat_type<T>::value>{};
	#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && (defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_CLANG))
		#define EASTL_TYPE_TRAIT_has_trivial_copy_CONFORMANCE 1    // has_trivial_copy is conforming.

		template <typename T> 
		struct has_trivial_copy : public stl::integral_constant<bool, (__has_trivial_copy(T) || stl::is_pod<T>::value) && (!stl::is_volatile<T>::value && !stl::is_reference<T>::value)>{};
	#else
		...
		template <typename T> 
		struct has_trivial_copy : public stl::integral_constant<bool, stl::is_pod<T>::value && !stl::is_volatile<T>::value>{};
	#endif

	#define EASTL_DECLARE_HAS_TRIVIAL_COPY(T, hasTrivialCopy)                                                    \
		namespace stl {                                                                                        \
			template <> struct has_trivial_copy<T> : public stl::integral_constant<bool, hasTrivialCopy>  { }; \
		}

	// Old style macro, for bacwards compatibility:
	#define EASTL_DECLARE_TRIVIAL_COPY(T) namespace stl{ template <> struct has_trivial_copy<T> : public true_type{}; template <> struct has_trivial_copy<const T> : public true_type{}; }
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct has_trivial_assign : public integral_constant<bool, (__has_trivial_assign(T) || stl::is_pod<T>::value) && !stl::is_const<T>::value && !stl::is_volatile<T>::value && !stl::is_hat_type<T>::value>{};
	#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_CLANG))
		#define EASTL_TYPE_TRAIT_has_trivial_assign_CONFORMANCE 1    // has_trivial_assign is conforming.

		template <typename T> 
		struct has_trivial_assign : public integral_constant<bool, (__has_trivial_assign(T) || stl::is_pod<T>::value) && !stl::is_const<T>::value && !stl::is_volatile<T>::value>{};
	#else
	...
	#define EASTL_DECLARE_HAS_TRIVIAL_ASSIGN(T, hasTrivialAssign)                                                    \
		namespace stl {                                                                                            \
			template <> struct has_trivial_assign<T> : public stl::integral_constant<bool, hasTrivialAssign>  { }; \
		}

	// Old style macro, for bacwards compatibility:
	#define EASTL_DECLARE_TRIVIAL_ASSIGN(T) namespace stl{ template <> struct has_trivial_assign<T> : public true_type{}; template <> struct has_trivial_assign<const T> : public true_type{}; }
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct has_trivial_destructor : public stl::integral_constant<bool, (__has_trivial_destructor(T) || stl::is_pod<T>::value) && !stl::is_hat_type<T>::value>{};
	#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_CLANG))
		#define EASTL_TYPE_TRAIT_has_trivial_destructor_CONFORMANCE 1    // has_trivial_destructor is conforming.

		template <typename T> 
		struct has_trivial_destructor : public stl::integral_constant<bool, __has_trivial_destructor(T) || stl::is_pod<T>::value>{};
	#else
		#define EASTL_TYPE_TRAIT_has_trivial_destructor_CONFORMANCE 0  // is_pod is not fully conforming. Can return false negatives.

		// With current compilers, this is all we can do.
		template <typename T> 
		struct has_trivial_destructor : public stl::is_pod<T>{};
	#endif

	#define EASTL_DECLARE_HAS_TRIVIAL_DESTRUCTOR(T, hasTrivialDestructor)                                                    \
		namespace stl {                                                                                                    \
			template <> struct has_trivial_destructor<T> : public stl::integral_constant<bool, hasTrivialDestructor>  { }; \
		}

	// Old style macro, for bacwards compatibility:
	#define EASTL_DECLARE_TRIVIAL_DESTRUCTOR(T) namespace stl{ template <> struct has_trivial_destructor<T> : public true_type{}; template <> struct has_trivial_destructor<const T> : public true_type{}; }
	...
	// With current compilers, this is all we can do.
	template <typename T> 
	struct has_trivial_relocate : public stl::integral_constant<bool, stl::is_pod<T>::value && !stl::is_volatile<T>::value>{};

	#define EASTL_DECLARE_TRIVIAL_RELOCATE(T) namespace stl{ template <> struct has_trivial_relocate<T> : public true_type{}; template <> struct has_trivial_relocate<const T> : public true_type{}; }
	...
	///////////////////////////////////////////////////////////////////////
		...
		struct has_nothrow_constructor 
			: public stl::integral_constant<bool, __has_nothrow_constructor(T)>{};
		...
		struct has_nothrow_constructor : public stl::integral_constant<bool, __has_nothrow_constructor(T) || stl::is_scalar<typename stl::remove_all_extents<T>::type>::value || stl::is_reference<T>::value>{};
		...
		struct has_nothrow_constructor // To do: Improve this to include other types that can work.
			{ static const bool value = stl::is_scalar<typename stl::remove_all_extents<T>::type>::value || stl::is_reference<T>::value; };
	#endif

	#define EASTL_DECLARE_HAS_NOTHROW_CONSTRUCTOR(T, hasNothrowConstructor)                                                    \
		namespace stl {                                                                                                      \
			template <> struct has_nothrow_constructor<T> : public stl::integral_constant<bool, hasNothrowConstructor>  { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct has_nothrow_copy : public stl::integral_constant<bool, __has_nothrow_copy(T)>{};
		...
		template <typename T> 
		struct has_nothrow_copy : public stl::integral_constant<bool, __has_nothrow_copy(T) || stl::is_scalar<typename stl::remove_all_extents<T>::type>::value || stl::is_reference<T>::value>{};
	...
	#define EASTL_DECLARE_HAS_NOTHROW_COPY(T, hasNothrowCopy)                                                    \
		namespace stl {                                                                                        \
			template <> struct has_nothrow_copy<T> : public stl::integral_constant<bool, hasNothrowCopy>  { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct has_nothrow_assign : public stl::integral_constant<bool, __has_nothrow_assign(T)>{};
		...
		template <typename T> // This is mistakenly returning true for an unbounded array of scalar type. 
		struct has_nothrow_assign : public stl::integral_constant<bool, __has_nothrow_assign(T) || stl::is_scalar<typename stl::remove_all_extents<T>::type>::value || stl::is_reference<T>::value>{};
	#else
	...
	#define EASTL_DECLARE_HAS_NOTHROW_ASSIGN(T, hasNothrowAssign)                                                    \
		namespace stl {                                                                                            \
			template <> struct has_nothrow_assign<T> : public stl::integral_constant<bool, hasNothrowAssign>  { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct has_virtual_destructor : public stl::integral_constant<bool, __has_virtual_destructor(T)>{};
	#else
		...
		template <typename T>
		struct has_virtual_destructor : public stl::false_type{};
	#endif

	#define EASTL_DECLARE_HAS_VIRTUAL_DESTRUCTOR(T, hasVirtualDestructor)                                                                   \
		namespace stl {                                                                                                                   \
			template <> struct has_virtual_destructor<T>                : public stl::integral_constant<bool, hasVirtualDestructor>  { }; \
			template <> struct has_virtual_destructor<const T>          : public stl::integral_constant<bool, hasVirtualDestructor>  { }; \
			template <> struct has_virtual_destructor<volatile T>       : public stl::integral_constant<bool, hasVirtualDestructor>  { }; \
			template <> struct has_virtual_destructor<const volatile T> : public stl::integral_constant<bool, hasVirtualDestructor>  { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct is_literal_type : public stl::integral_constant<bool, __is_literal(T)>{};
		...
		template <typename T> 
		struct is_literal_type : public stl::integral_constant<bool, __is_literal_type(T)>{};
		...
		struct is_literal_type : public stl::integral_constant<bool, stl::is_scalar<typename stl::remove_reference<typename stl::remove_all_extents<T>::type>::type>::value>{};
	#endif
	...
	///////////////////////////////////////////////////////////////////////
		...
		template<typename T, bool = !stl::is_object<T>::value>
		class is_abstract_helper
		{
			template<typename>
			static stl::yes_type test(...);

			template<typename T1>
			static stl::no_type test(T1(*)[1]);  // The following: 'typedef SomeAbstractClass (*SomeFunctionType)[1];' is invalid (can't have an array of abstract types) and thus doesn't choose this path.
	
		public:
			static const bool value = (sizeof(test<T>(NULL)) == sizeof(stl::yes_type));
		};
	...
	#define EASTL_DECLARE_IS_ABSTRACT(T, isAbstract)                                                                   \
		namespace stl {                                                                                              \
			template <> struct is_abstract<T>                : public stl::integral_constant<bool, isAbstract>  { }; \
			template <> struct is_abstract<const T>          : public stl::integral_constant<bool, isAbstract>  { }; \
			template <> struct is_abstract<volatile T>       : public stl::integral_constant<bool, isAbstract>  { }; \
			template <> struct is_abstract<const volatile T> : public stl::integral_constant<bool, isAbstract>  { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T> 
		struct is_trivially_copyable : public integral_constant<bool, (__has_trivial_copy(T) || stl::is_pod<typename stl::remove_all_extents<T>::type>::value) && (!stl::is_void<T>::value && !stl::is_volatile<T>::value && !stl::is_reference<T>::value)>{};
	#else
		#define EASTL_TYPE_TRAIT_is_trivially_copyable_CONFORMANCE 0  // Generates false negatives.

		template <typename T>
		struct is_trivially_copyable { static const bool value = stl::is_scalar<typename stl::remove_all_extents<T>::type>::value; };
	#endif

	#define EASTL_DECLARE_IS_TRIVIALLY_COPYABLE(T, isTriviallyCopyable)                                                                   \
		namespace stl {                                                                                                                 \
			template <> struct is_trivially_copyable<T>                : public stl::integral_constant<bool, isTriviallyCopyable>  { }; \
			template <> struct is_trivially_copyable<const T>          : public stl::integral_constant<bool, isTriviallyCopyable>  { }; \
			template <> struct is_trivially_copyable<volatile T>       : public stl::integral_constant<bool, isTriviallyCopyable>  { }; \
			template <> struct is_trivially_copyable<const volatile T> : public stl::integral_constant<bool, isTriviallyCopyable>  { }; \
		}
	...
	#define EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE 1
		...
		// chicken-and-egg problem. To do: Resolve this, probably by putting stl::move somewhere else.
		template <typename T>
		inline typename stl::remove_reference<T>::type&& move_internal(T&& x) EA_NOEXCEPT
		{ return ((typename stl::remove_reference<T>::type&&)x); }

		template <typename T, class ...Args>
		typename first_type_select<stl::true_type, decltype(stl::move_internal(T(stl::declval<Args>()...)))>::type is(T&&, Args&& ...);

		template <typename T>
		struct can_construct_scalar_helper
		{
			static stl::true_type can(T);
			static stl::false_type can(...);
		};

		template <typename ...Args>
		stl::false_type is(argument_sink, Args&& ...);
		...
		struct is_constructible_helper_2    // argument_sink will catch all T that is not constructible from the Args and denote false_type
			: public stl::identity<decltype(is(stl::declval<T>(), stl::declval<Args>()...))>::type {};

		template <typename T>
		struct is_constructible_helper_2<true, T>
			: public stl::is_scalar<T> {};
		...
		struct is_constructible_helper_2<true, T, Arg0>
			: public stl::identity<decltype(can_construct_scalar_helper<T>::can(stl::declval<Arg0>()))>::type {};
		...
		struct is_constructible_helper_2<true, T, Arg0, Args...>
			: public stl::false_type {};

		template <bool, typename T, typename... Args>
		struct is_constructible_helper_1
			: public is_constructible_helper_2<stl::is_scalar<T>::value || stl::is_reference<T>::value, T, Args...> {};
		...
		struct is_constructible
			: public is_constructible_helper_1<(stl::is_abstract<typename stl::remove_all_extents<T>::type>::value || 
												stl::is_array_of_unknown_bounds<T>::value                            ||
												stl::is_function<typename stl::remove_all_extents<T>::type>::value || 
												stl::has_void_arg<T, Args...>::value), 
												T, Args...> {};
		...
		struct is_constructible_helper_2<false, Array[N]>
			: public stl::is_constructible<typename stl::remove_all_extents<Array>::type> {};
	...
	#define EASTL_DECLARE_IS_CONSTRUCTIBLE(T, U, isConstructible)                                                     \
		namespace stl {                                                                                             \
			template <> struct is_constructible<T, U>  : public stl::integral_constant<bool, isConstructible>  { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		// has __is_trivially_constructible(T) or stl::has_trivial_copy<T>::value, depending on
		...
		// If the compiler has this trait built-in (which ideally all compilers would have since it's necessary for full conformance) use it.
			...
			template <typename T, typename Arg0 = stl::unused>
			struct is_trivially_constructible 
				: public stl::false_type {};

			template <typename T>
			struct is_trivially_constructible<T, stl::unused>
				: public stl::integral_constant<bool, __is_trivially_constructible(T)> {};
			...
			template <typename T>
			struct is_trivially_constructible<T, stl::unused>
				: public stl::integral_constant<bool, stl::is_constructible<T>::value && stl::has_trivial_constructor<typename stl::remove_all_extents<T>::type>::value> {};

			template <typename T>
			struct is_trivially_constructible<T, T>
				: public stl::integral_constant<bool, stl::is_constructible<T>::value && stl::has_trivial_copy<T>::value> {};
		...
		// If the compiler has this trait built-in (which ideally all compilers would have since it's necessary for full conformance) use it.
			...
			template <typename T, typename... Args>
			struct is_trivially_constructible
				: public stl::integral_constant<bool, stl::is_constructible<T, Args...>::value && __is_trivially_constructible(T, Args...)> {};
	...
	#define EASTL_DECLARE_IS_TRIVIALLY_CONSTRUCTIBLE(T, isTriviallyConstructible)                                                                   \
		namespace stl {                                                                                                                           \
			template <> struct is_trivially_constructible<T>                : public stl::integral_constant<bool, isTriviallyConstructible>  { }; \
			template <> struct is_trivially_constructible<const T>          : public stl::integral_constant<bool, isTriviallyConstructible>  { }; \
			template <> struct is_trivially_constructible<volatile T>       : public stl::integral_constant<bool, isTriviallyConstructible>  { }; \
			template <> struct is_trivially_constructible<const volatile T> : public stl::integral_constant<bool, isTriviallyConstructible>  { }; \
		}
	...
	struct is_trivially_default_constructible 
		: public stl::is_trivially_constructible<T> {};
	...
	#define EASTL_TYPE_TRAIT_is_trivial_CONFORMANCE ((EASTL_TYPE_TRAIT_is_trivially_default_constructible_CONFORMANCE && EASTL_TYPE_TRAIT_is_trivially_copyable_CONFORMANCE) ? 1 : 0)
		...
		struct is_trivial_helper
			: public stl::integral_constant<bool, stl::is_trivially_copyable<T>::value && stl::is_trivially_default_constructible<T>::value>{};
	...
	///////////////////////////////////////////////////////////////////////
		...
		struct is_nothrow_constructible<T>
			: public stl::integral_constant<bool, stl::has_nothrow_constructor<T>::value> {};

		template <typename T>
		struct is_nothrow_constructible<T, T>
			: public stl::integral_constant<bool, stl::has_nothrow_copy<T>::value> {};
		...
		struct is_nothrow_constructible_helper_noexcept_wrapper
			{ static const bool value = noexcept(T(stl::declval<Args>()...)); };
		...
		struct is_nothrow_constructible_helper<true, T, Args...>
			: public stl::integral_constant<bool, is_nothrow_constructible_helper_noexcept_wrapper<T, Args...>::value> {};

		template<typename T, typename Arg>
		struct is_nothrow_constructible_helper<true, T, Arg>
			: public stl::integral_constant<bool, noexcept(T(stl::declval<Arg>()))> {};

		template<typename T>
		struct is_nothrow_constructible_helper<true, T>
			: public stl::integral_constant<bool, noexcept(T())> {};
		...
		template <typename T, typename... Args>
		struct is_nothrow_constructible
			: public stl::is_nothrow_constructible_helper<stl::is_constructible<T, Args...>::value, T, Args...> {};

		template <typename T, size_t N>
		struct is_nothrow_constructible<T[N]>
			: public stl::is_nothrow_constructible_helper<stl::is_constructible<T>::value, T> {};
	#endif

	#define EASTL_DECLARE_IS_NOTHROW_CONSTRUCTIBLE(T, isNothrowConstructible)                                                    \
		namespace stl{                                                                                                         \
			template <> struct is_nothrow_constructible<T> : public stl::integral_constant<bool, isNothrowConstructible>  { }; \
		}
	...
	struct is_copy_constructible
		: public stl::is_constructible<T, typename stl::add_lvalue_reference<typename stl::add_const<T>::type>::type> {};
	...
	struct is_trivially_copy_constructible
		: public stl::is_trivially_constructible<T, typename stl::add_lvalue_reference<typename stl::add_const<T>::type>::type> {};
	...
	struct is_nothrow_copy_constructible
		: public is_nothrow_constructible<T, typename stl::add_lvalue_reference<typename stl::add_const<T>::type>::type> {};
	...
	struct is_move_constructible
		: public stl::is_constructible<T, typename stl::add_rvalue_reference<T>::type> {};
	...
	struct is_trivially_move_constructible
		: public stl::is_trivially_constructible<T, typename stl::add_rvalue_reference<T>::type> {};

	#define EASTL_DECLARE_IS_TRIVIALLY_MOVE_CONSTRUCTIBLE(T, isTrivallyMoveConstructible)                                                      \
		namespace stl{                                                                                                                       \
			template <> struct is_trivially_move_constructible<T>  : public stl::integral_constant<bool, isTriviallyMoveConstructible>  { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		{
			typedef typename stl::remove_reference<T>::type T1;
			typedef typename stl::remove_reference<U>::type U1;

			// Do not decay the types, as a decayed array (pointer) can be assigned but an array cannot.
			static const bool eitherIsArray = (stl::is_array<T1>::value || stl::is_array<U1>::value);
			static const bool isArithmetic  = (stl::is_arithmetic<T1>::value && stl::is_arithmetic<U1>::value);
			static const bool isSameScalar  = (stl::is_same<T1, U1>::value && stl::is_scalar<T1>::value && stl::is_scalar<U1>::value);
			static const bool isTConst      = stl::is_const<T1>::value;
		...
		struct is_assignable_helper
		{
			...
			static decltype(stl::declval<T1>() = stl::declval<U1>(), stl::yes_type()) is(int);

			static const bool value = (sizeof(is<T, U>(0)) == sizeof(stl::yes_type));
		};
	#endif

	template<typename T, typename U>
	struct is_assignable : 
		public stl::integral_constant<bool, stl::is_assignable_helper<T, U>::value> {};
	...
	#define EASTL_DECLARE_IS_ASSIGNABLE(T, U, isAssignable)                                                    \
		namespace stl {                                                                                      \
			template <> struct is_assignable<T, U> : public stl::integral_constant<bool, isAssignable>  { }; \
		}
	...
	//
	// This is an stl extension function which is like is_assignable but
	...
	struct is_lvalue_assignable
		: public stl::is_assignable<typename stl::add_lvalue_reference<T>::type,
									  typename stl::add_lvalue_reference<typename stl::add_const<U>::type>::type> {};

	#define EASTL_DECLARE_IS_LVALUE_ASSIGNABLE(T, U, isLvalueAssignable)                                                    \
		namespace stl {                                                                                                   \
			template <> struct is_lvalue_assignable<T, U> : public stl::integral_constant<bool, isLvalueAssignable>  { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		struct is_trivially_assignable
			: stl::integral_constant<bool, __is_trivially_assignable(T, U)> {};
		...
		template <typename T, typename U>
		struct is_trivially_assignable_helper<true, T, U> : stl::integral_constant<bool, __is_trivially_assignable(T, U)>{};
		...
		template <typename T, typename U>
		struct is_trivially_assignable
			: stl::integral_constant<bool, is_trivially_assignable_helper< stl::is_assignable<T, U>::value, T, U >::value> {};
		...
		template <typename T, typename U>
		struct is_trivially_assignable
			: stl::integral_constant<bool, stl::is_assignable<T, U>::value && 
									   (stl::is_pod<typename stl::remove_reference<T>::type>::value || __has_trivial_assign(typename stl::remove_reference<T>::type))> {};
	#else
		...
		struct is_trivially_assignable<T&, T>
			: public stl::integral_constant<bool, stl::is_scalar<T>::value> {};
	...
	#define EASTL_DECLARE_IS_TRIVIALLY_ASSIGNABLE(T, U, isTriviallyAssignable)                                                   \
		namespace stl {                                                                                                        \
			template <> struct is_trivially_assignable<T, U> : public stl::integral_constant<bool, isTriviallyAssignable> { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		struct is_nothrow_assignable
			: stl::integral_constant<bool, __is_nothrow_assignable(T, U)> {};
		...
		struct is_nothrow_assignable<T&, T>
			: public stl::integral_constant<bool, stl::has_nothrow_assign<T>::value> {};
		...
		struct is_nothrow_assignable_helper<true, T, U> // Set to true if the assignment (same as is_assignable) cannot generate an exception.
			: public stl::integral_constant<bool, noexcept(stl::declval<T>() = stl::declval<U>()) >
		{
		};

		template <typename T, typename U>
		struct is_nothrow_assignable
			: public stl::is_nothrow_assignable_helper<stl::is_assignable<T, U>::value, T, U>
		{
		};
	#endif
	...
	#define EASTL_DECLARE_IS_NOTHROW_ASSIGNABLE(T, isNothrowAssignable)                                                                   \
		namespace stl{                                                                                                                  \
			template <> struct is_nothrow_assignable<T>                : public stl::integral_constant<bool, isNothrowAssignable>  { }; \
			template <> struct is_nothrow_assignable<const T>          : public stl::integral_constant<bool, isNothrowAssignable>  { }; \
			template <> struct is_nothrow_assignable<volatile T>       : public stl::integral_constant<bool, isNothrowAssignable>  { }; \
			template <> struct is_nothrow_assignable<const volatile T> : public stl::integral_constant<bool, isNothrowAssignable>  { }; \
		}
	...
	struct is_copy_assignable
		: public stl::is_assignable<typename stl::add_lvalue_reference<T>::type,
									  typename stl::add_lvalue_reference<typename stl::add_const<T>::type>::type> {};
	...
	struct is_trivially_copy_assignable
		: public stl::is_trivially_assignable<typename stl::add_lvalue_reference<T>::type,
												typename stl::add_lvalue_reference<typename stl::add_const<T>::type>::type> {};
#else
	template <typename T>
	struct is_trivially_copy_assignable
		: public integral_constant<bool,
			stl::is_scalar<T>::value || stl::is_pod<T>::value || stl::is_trivially_assignable<typename stl::add_lvalue_reference<T>::type, typename stl::add_lvalue_reference<typename stl::add_const<T>::type>::type>::value
		> {};
#endif

	#define EASTL_DECLARE_IS_TRIVIALLY_COPY_ASSIGNABLE(T, isTriviallyCopyAssignable)                                                    \
		namespace stl {                                                                                                               \
			template <> struct is_trivially_copy_assignable<T> : public stl::integral_constant<bool, isTriviallyCopyAssignable>  { }; \
		}
	...
	struct is_nothrow_copy_assignable
		: public stl::is_nothrow_assignable<typename stl::add_lvalue_reference<T>::type,
											  typename stl::add_lvalue_reference<typename stl::add_const<T>::type>::type> {};
	...
	struct is_move_assignable
		: public stl::is_assignable<typename stl::add_lvalue_reference<T>::type,
									  typename stl::add_rvalue_reference<T>::type> {};

	#define EASTL_DECLARE_IS_MOVE_ASSIGNABLE(T, isMoveAssignable)                                                                   \
		namespace stl{                                                                                                            \
			template <> struct is_move_assignable<T>                : public stl::integral_constant<bool, isMoveAssignable>  { }; \
			template <> struct is_move_assignable<const T>          : public stl::integral_constant<bool, isMoveAssignable>  { }; \
			template <> struct is_move_assignable<volatile T>       : public stl::integral_constant<bool, isMoveAssignable>  { }; \
			template <> struct is_move_assignable<const volatile T> : public stl::integral_constant<bool, isMoveAssignable>  { }; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		template <typename T>
		struct is_destructible
			: public stl::integral_constant<bool, !stl::is_array_of_unknown_bounds<T>::value && 
													!stl::is_void<T>::value                    && 
													!stl::is_function<T>::value                && 
													!stl::is_abstract<T>::value> {};
	#else
		#define EASTL_TYPE_TRAIT_is_destructible_CONFORMANCE 1
		...
		template <typename>
		stl::false_type destructible_test_function(...);
		
		template <typename T, typename U = decltype(stl::declval<stl::destructible_test_helper<T> >().~destructible_test_helper<T>())>
		stl::true_type destructible_test_function(int);

		template <typename T, bool = stl::is_array_of_unknown_bounds<T>::value || // Exclude these types from being considered destructible.
									 stl::is_void<T>::value                    || 
									 stl::is_function<T>::value                || 
									 stl::is_abstract<T>::value>
		struct is_destructible_helper
			: public stl::identity<decltype(stl::destructible_test_function<T>(0))>::type {}; // Need to wrap decltype with identity because some compilers otherwise don't like the bare decltype usage.
	...
	#define EASTL_DECLARE_IS_DESTRUCTIBLE(T, isDestructible)												                \
		namespace stl{                                                                                                    \
			template <> struct is_destructible<T>                : public stl::integral_constant<bool, isDestructible>{}; \
			template <> struct is_destructible<const T>          : public stl::integral_constant<bool, isDestructible>{}; \
			template <> struct is_destructible<volatile T>       : public stl::integral_constant<bool, isDestructible>{}; \
			template <> struct is_destructible<const volatile T> : public stl::integral_constant<bool, isDestructible>{}; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		struct is_trivially_destructible // Can't use just __has_trivial_destructor(T) because some compilers give it slightly different meaning, and are just plain broken, such as VC++'s __has_trivial_destructor, which says false for fundamental types.
			: public integral_constant<bool, stl::is_destructible<T>::value && ((__has_trivial_destructor(T) && !stl::is_hat_type<T>::value)|| stl::is_scalar<typename stl::remove_all_extents<T>::type>::value)> {};
		...
		struct is_trivially_destructible_helper
			: public integral_constant<bool, (stl::is_pod<T>::value || stl::is_scalar<T>::value || stl::is_reference<T>::value) && !stl::is_void<T>::value> {};

		template <typename T> 
		struct is_trivially_destructible
			: public stl::is_trivially_destructible_helper<typename stl::remove_all_extents<T>::type> {};
	#endif

	#define EASTL_DECLARE_IS_TRIVIALLY_DESTRUCTIBLE(T, isTriviallyDestructible)                                                       \
		namespace stl{                                                                                                              \
			template <> struct is_trivially_destructible<T>                : public stl::integral_constant<bool, isTriviallyDestructible>{}; \
			template <> struct is_trivially_destructible<const T>          : public stl::integral_constant<bool, isTriviallyDestructible>{}; \
			template <> struct is_trivially_destructible<volatile T>       : public stl::integral_constant<bool, isTriviallyDestructible>{}; \
			template <> struct is_trivially_destructible<const volatile T> : public stl::integral_constant<bool, isTriviallyDestructible>{}; \
		}
	...
	///////////////////////////////////////////////////////////////////////
		...
		struct is_nothrow_destructible_helper
			: public stl::integral_constant<bool, stl::is_scalar<T>::value || stl::is_reference<T>::value> {};

		template <typename T> 
		struct is_nothrow_destructible
			: public stl::is_nothrow_destructible_helper<typename stl::remove_all_extents<T>::type> {};
		...
		struct is_nothrow_destructible_helper_noexcept_wrapper
			{ static const bool value = noexcept(stl::declval<T&>().~T()); };
		...
		struct is_nothrow_destructible_helper<T, true>     // If the expression T::~T is a noexcept expression then it's nothrow.
			: public stl::integral_constant<bool, is_nothrow_destructible_helper_noexcept_wrapper<T>::value > {};

		template <typename T>
		struct is_nothrow_destructible                      // A type needs to at least be destructible before it could be nothrow destructible.
			: public stl::is_nothrow_destructible_helper<T, stl::is_destructible<T>::value> {};
		...
		struct is_nothrow_destructible<T[N]>                // To consider: Replace this with a remove_all_extents pathway.
			: public stl::is_nothrow_destructible<T> {};
	...
	struct is_nothrow_default_constructible
		: public stl::is_nothrow_constructible<T> {};
	...
	struct is_nothrow_move_constructible
	    : public stl::is_nothrow_constructible<T, typename stl::add_rvalue_reference<T>::type> {};
