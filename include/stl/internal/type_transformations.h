...
namespace stl
{
	...
	template <typename T, bool = stl::is_const<T>::value || stl::is_reference<T>::value || stl::is_function<T>::value>
	...
	template <typename T>
	struct  add_const
		{ typedef typename stl::add_const_helper<T>::type type; };
	...
	template <typename T, bool = stl::is_volatile<T>::value || stl::is_reference<T>::value || stl::is_function<T>::value>
	...
	template <typename T> struct add_volatile
		{ typedef typename stl::add_volatile_helper<T>::type type; };
	...
	{ typedef typename stl::make_signed<T>::type type; };
	...
	template<class T>
	struct add_pointer { typedef typename stl::remove_reference<T>::type* type; };
	...
	template<typename T, size_t N> struct remove_all_extents<T[N]> { typedef typename stl::remove_all_extents<T>::type type; };
	template<typename T>           struct remove_all_extents<T[]>  { typedef typename stl::remove_all_extents<T>::type type; };
	...
	#if defined(EA_COMPILER_NO_TEMPLATE_ALIASES)
		#define EASTL_ALIGNED_STORAGE_T(N, Align) typename stl::aligned_storage_t<N, Align>::type
	#else
		template <size_t N, size_t Align = EASTL_ALIGN_OF(double)>
		using aligned_storage_t = typename aligned_storage<N, Align>::type;
		#define EASTL_ALIGNED_STORAGE_T(N, Align) stl::aligned_storage_t<N, Align>
	#endif
	...
	///////////////////////////////////////////////////////////////////////
		...
		{
			static const size_t size0           = stl::static_max<minSize, sizeof(Type0)>::value;
			static const size_t size1           = stl::static_max<size0,   sizeof(Type1)>::value;
			static const size_t size2           = stl::static_max<size1,   sizeof(Type2)>::value;
			static const size_t size            = stl::static_max<size2,   sizeof(Type3)>::value;

			static const size_t alignment0      = stl::static_max<EA_ALIGN_OF(Type0), EA_ALIGN_OF(Type1)>::value;
			static const size_t alignment1      = stl::static_max<alignment0,         EA_ALIGN_OF(Type2)>::value;
			static const size_t alignment_value = stl::static_max<alignment1,         EA_ALIGN_OF(Type3)>::value;

			typedef typename stl::aligned_storage<size, alignment_value>::type type;
		};
		...
		struct aligned_union
		{
			static const size_t size            = stl::static_max<minSize, sizeof(Type0), sizeof(TypeN)...>::value;
			static const size_t alignment_value = stl::static_max<EA_ALIGN_OF(Type0), EA_ALIGN_OF(TypeN)...>::value;
