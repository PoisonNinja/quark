...
#include <stl/internal/memory_base.h>
#include <stl/internal/generic_iterator.h>
#include <stl/internal/pair_fwd_decls.h>
#include <stl/internal/functional_base.h>
#include <stl/internal/allocator_traits_fwd_decls.h>
...
namespace stl
{
	...
	///
		...
		#define EASTL_TEMP_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " temp" // Unless the user overrides something, this is "stl temp".
	#endif
	...
	/// 
	template <typename T>
	stl::pair<T*, ptrdiff_t> get_temporary_buffer(ptrdiff_t n, size_t alignment = 1, size_t alignmentOffset = 0, const char* pName = EASTL_TEMP_DEFAULT_NAME)
	{
		EASTLAllocatorType allocator(*EASTLAllocatorDefault(), pName);
		return stl::pair<T*, ptrdiff_t>(static_cast<T*>(EASTLAllocAligned(allocator, n * sizeof(T), alignment, alignmentOffset)), n);
	}
	...
	public:
		...
		typedef typename stl::aligned_storage<sizeof(value_type), 
						 stl::alignment_of<value_type>::value>::type storage_type;
		...
		{
			if(!mpValue)
				mpValue = new(&mStorage) value_type(stl::forward<Args>(args)...);
		} 
	...
	public:
		...
		{
			::new(stl::addressof(*mIterator)) T(value);
			return *this;
		}
	...
	namespace Internal
	{
		...
		struct uninitialized_relocate_impl
		{
			...
			{
				typedef typename stl::iterator_traits<ForwardIterator>::value_type value_type;
					...
					try
					{
						...
						for(; first != last; ++first, ++dest)
							::new((void*)stl::addressof(*dest)) value_type(*first);
				#if EASTL_EXCEPTIONS_ENABLED
					}
					catch(...)
					{
	...
	inline ForwardIteratorDest uninitialized_relocate_start(ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
	{
		...
		typedef typename stl::iterator_traits<ForwardIterator>::value_type value_type_input;
		typedef typename stl::iterator_traits<ForwardIteratorDest>::value_type value_type_output;
	...
	inline ForwardIteratorDest uninitialized_relocate(ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
	{
		...
		stl::uninitialized_relocate_commit(first, last, dest);
	...
	namespace Internal
	{
		...
		{
			return stl::copy(first, last, dest); // The copy() in turn will use memcpy for POD types.
		}
			...
			ForwardIterator currentDest(dest);
				...
				try
				{
					...
					for(; first != last; ++first, ++currentDest)
						::new(static_cast<void*>(stl::addressof(*currentDest))) value_type(*first);
			#if EASTL_EXCEPTIONS_ENABLED
				}
				catch(...)
				{
	...
	inline ForwardIterator uninitialized_copy(InputIterator first, InputIterator last, ForwardIterator result)
	{
		...
		return Internal::uninitialized_copy_impl(first, last, result, stl::is_trivial<value_type>());
	}
	...
	namespace Internal
	{
				...
				ForwardIterator currentDest(dest);
					...
					try
					{
						...
						for(; n > 0; --n, ++first, ++currentDest)
							::new((void*)(stl::addressof(*currentDest))) value_type(*first);
				#if EASTL_EXCEPTIONS_ENABLED
					}
					catch(...)
					{
		...
		struct uninitialized_copy_n_impl<InputIterator, Count, ForwardIterator, EASTL_ITC_NS::random_access_iterator_tag>
		{
			...
			{
				return stl::uninitialized_copy(first, first + n, dest);
			}
		};
	}
	...
	/// internally it uses generic_iterator to make pointers act like regular stl::iterator.
	///
	...
	{
		typedef typename stl::iterator_traits<generic_iterator<Result, void> >::value_type value_type;
		const generic_iterator<Result, void> i(Internal::uninitialized_copy_impl(stl::generic_iterator<First, void>(first), // generic_iterator makes a pointer act like an iterator.
																				 stl::generic_iterator<Last, void>(last), 
																				 stl::generic_iterator<Result, void>(result), 
																				 stl::is_trivially_copy_assignable<value_type>()));
		return i.base();
	}
	...
	///
		...
		namespace Internal
		{
			...
			{
				return stl::copy(first, last, dest); // The copy() in turn will use memcpy for is_trivially_copy_assignable (e.g. POD) types.
			}
			...
			inline ForwardIterator uninitialized_move_impl(InputIterator first, InputIterator last, ForwardIterator dest, false_type)
			{
				...
				#endif
						for(; first != last; ++first, ++currentDest)
							::new((void*)stl::addressof(*currentDest)) value_type(stl::move(*first)); // If value_type has a move constructor then it will be used here.
				#if EASTL_EXCEPTIONS_ENABLED
					}
					catch(...)
					{
		...
		{
			typedef typename stl::iterator_traits<generic_iterator<Result, void> >::value_type value_type;
			const generic_iterator<Result, void> i(Internal::uninitialized_move_impl(stl::generic_iterator<First, void>(first), // generic_iterator makes a pointer act like an iterator.
																					 stl::generic_iterator<Last, void>(last), 
																					 stl::generic_iterator<Result, void>(dest), 
																					 stl::is_trivially_copy_assignable<value_type>())); // is_trivially_copy_assignable identifies if copy assignment would be as valid as move assignment, which means we have the opportunity to memcpy/memmove optimization.
			return i.base();
		}
	#else
		template <typename First, typename Last, typename Result>
	...
	{
		#if EASTL_MOVE_SEMANTICS_ENABLED
			return stl::uninitialized_copy(stl::make_move_iterator(first), stl::make_move_iterator(last), dest);
		#else
			return stl::uninitialized_copy(first, last, dest);
		#endif
	}
	...
	{
		#if EASTL_MOVE_SEMANTICS_ENABLED
			return stl::uninitialized_copy(stl::make_move_if_noexcept_iterator(first), stl::make_move_if_noexcept_iterator(last), dest);
		#else
			return stl::uninitialized_copy(first, last, dest);
		#endif
	}
	...
	{
		#if EASTL_EXCEPTIONS_ENABLED
			return stl::uninitialized_move_if_noexcept(first, last, dest);
		#else
			return stl::uninitialized_move_ptr(first, last, dest);
		#endif
	}
	...
	{
		#if EASTL_MOVE_SEMANTICS_ENABLED
			return stl::uninitialized_copy_n(stl::make_move_iterator(first), n, dest);
		#else
			return stl::uninitialized_copy_n(first, n, dest);
		#endif
	}
	...
	inline void uninitialized_default_fill(ForwardIterator first, ForwardIterator last)
	{
		...
		try
		{
			...
			for (; currentDest != last; ++currentDest)
				::new (stl::addressof(*currentDest)) value_type();
	#if EASTL_EXCEPTIONS_ENABLED
		}
		catch (...)
		{
	...
	namespace Internal
	{
		...
		void uninitialized_fill_impl(ForwardIterator first, ForwardIterator last, const T& value, false_type)
		{
			...
			#endif
					for(; currentDest != last; ++currentDest)
						::new((void*)stl::addressof(*currentDest)) value_type(value);
			#if EASTL_EXCEPTIONS_ENABLED
				}
				catch(...)
				{
	...
	{
		typedef typename stl::iterator_traits<ForwardIterator>::value_type value_type;
		Internal::uninitialized_fill_impl(first, last, value, stl::is_trivially_copy_assignable<value_type>());
	}
	...
	{
		typedef typename stl::iterator_traits<stl::generic_iterator<T*, void> >::value_type value_type;
		Internal::uninitialized_fill_impl(stl::generic_iterator<T*, void>(first),
		                                  stl::generic_iterator<T*, void>(last), value,
		                                  stl::is_trivially_copy_assignable<value_type>());
	}
	...
	{
		typedef typename stl::iterator_traits<ForwardIterator>::value_type value_type;
		Internal::uninitialized_fill_n_impl(first, n, value, stl::is_trivially_copy_assignable<value_type>());
	}
	...
	{
		typedef typename stl::iterator_traits<generic_iterator<T*, void> >::value_type value_type;
		Internal::uninitialized_fill_n_impl(stl::generic_iterator<T*, void>(first), n, value, stl::is_trivially_copy_assignable<value_type>());
	}
	...
	{
		const ForwardIterator mid(stl::uninitialized_copy(first1, last1, first2));
			...
			{
		#endif
				stl::uninitialized_fill(mid, last2, value);
		#if EASTL_EXCEPTIONS_ENABLED
			}
			catch(...)
			{
	...
	inline void uninitialized_move_fill(InputIterator first1, InputIterator last1,
										ForwardIterator first2, ForwardIterator last2, const T& value)
	{
		const ForwardIterator mid(stl::uninitialized_move(first1, last1, first2));
	...
	{
		stl::uninitialized_fill(result, mid, value);
			...
			{
		#endif
				return stl::uninitialized_copy(first, last, mid);
		#if EASTL_EXCEPTIONS_ENABLED
			}
			catch(...)
			{
	...
	uninitialized_fill_move(ForwardIterator result, ForwardIterator mid, const T& value, InputIterator first, InputIterator last)
	{
		stl::uninitialized_fill(result, mid, value);
			...
			{
		#endif
				return stl::uninitialized_move(first, last, mid);
		#if EASTL_EXCEPTIONS_ENABLED
			}
			catch(...)
			{
	...
	{
		const ForwardIterator mid(stl::uninitialized_copy(first1, last1, result));
			...
			{
		#endif
				return stl::uninitialized_copy(first2, last2, mid);
		#if EASTL_EXCEPTIONS_ENABLED
			}
			catch(...)
			{
	...
	{
		typedef typename stl::iterator_traits<ForwardIterator>::value_type value_type;
		destruct_impl(first, last, stl::has_trivial_destructor<value_type>());
	}
	...
	/// is a no-op.
	///
	/// This is the same as stl::destruct but we included for C++17 compliance.
	///
	...
	///     void*  p  = buffer;
	///     void*  p1 = stl::align(16,  3, p, space); p = (char*)p +  3; space -=  3;
	///     void*  p2 = stl::align(32, 78, p, space); p = (char*)p + 78; space -= 78;
	///     void*  p3 = stl::align(64,  9, p, space); p = (char*)p +  9; space -=  9;
	...
	///     size_t space = sizeof(buffer);
	///     void*  p  = buffer;
	///     void*  p1 = stl::align_advance(16,  3, p, space, &p, &space); // p is advanced and space reduced accordingly.
	///     void*  p2 = stl::align_advance(32, 78, p, space, &p, &space);
	///     void*  p3 = stl::align_advance(64,  9, p, space, &p, &space);
	///     void*  p4 = stl::align_advance(16, 33, p, space);
	...
	private:
		template <typename>
		static stl::no_type test(...);

		template <typename U>
		static stl::yes_type test(typename U::allocator_type* = NULL);
	...
	struct uses_allocator_impl
		: public integral_constant<bool, stl::is_convertible<Allocator, typename T::allocator_type>::value>
	{
	};
	...
	namespace Internal
	{
		...
		{
		private:
			template <typename U> static stl::no_type  test(...);
			template <typename U> static stl::yes_type test(typename U::element_type* = 0);
		public:
			static const bool value = sizeof(test<Pointer>(0)) == sizeof(stl::yes_type);
		};
		...
		private:
			template <typename U> static stl::no_type  test(...);
			template <typename U> static stl::yes_type test(typename U::difference_type* = 0);
		public:
			static const bool value = sizeof((test<Pointer>(0))) == sizeof(stl::yes_type);
		};
		...
		// its own type), but we don't currently use rebind in stl (as we have a different allocator
	...
	public:
		static pointer pointer_to(typename stl::conditional<stl::is_void<element_type>::value, void, element_type>::type& r) // 20.6.3.2: if element_type is (possibly cv-qualified) void, the type of r is unspecified; otherwise, it is T&.
	...
	public:
		static pointer pointer_to(typename stl::conditional<stl::is_void<element_type>::value, void, element_type>::type& r) EA_NOEXCEPT
			{ return stl::addressof(r); } // 20.6.3.2: if element_type is (possibly cv-qualified) void, the type of r is unspecified; otherwise, it is T&.
	};

} // namespace stl

#include <stl/internal/allocator_traits.h>
