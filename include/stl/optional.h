...
#include <stl/memory.h> // stl::addressof
#include <stl/internal/in_place_t.h> // stl::in_place_t
...
namespace stl
{
	...
	/// nullopt_t
	///
	/// nullopt_t is class type used to indicate stl::optional type with uninitialized state.
	///
	...
	///
		...
		{
			bad_optional_access() : std::logic_error("stl::bad_optional_access exception") {}
	...
	namespace Internal
	{
		...
		/// optional_storage
		///
		template<typename T, bool IsTriviallyDestructible = stl::is_trivially_destructible_v<T>>
		struct optional_storage
		{
			typedef typename stl::remove_const<T>::type value_type;

			inline optional_storage() EA_NOEXCEPT : empty_val('\0') {}

			template<typename TT = T, typename = stl::enable_if_t<stl::is_copy_constructible_v<TT>>>
			inline optional_storage(const optional_storage& other) : engaged(other.engaged) 
			{
				auto* pOtherValue = reinterpret_cast<const T*>(stl::addressof(other.val));
				::new (stl::addressof(val)) value_type(*pOtherValue);
			}

			inline optional_storage(const value_type& v) : engaged(true)
			{
				::new (stl::addressof(val)) value_type(v);
			}

			inline optional_storage(value_type&& v) : engaged(true)
			{
				::new (stl::addressof(val)) value_type(stl::move(v));
			}
			...
			inline explicit optional_storage(in_place_t, Args&&... args)
			    : engaged(true)
			{
				::new (stl::addressof(val)) T{std::forward<Args>(args)...};
			}

			template <typename U,
			          typename... Args,
			          typename = stl::enable_if_t<stl::is_constructible_v<T, std::initializer_list<U>&, Args&&...>>>
			inline explicit optional_storage(in_place_t, std::initializer_list<U> ilist, Args&&... args)
			    : engaged(true)
			{
				::new (stl::addressof(val)) value_type{ilist, stl::forward<Args>(args)...};
			}

			inline void destruct_value() { (*(value_type*)stl::addressof(val)).~value_type(); }


			// This union exists to support trivial types that do not require constructors/destructors to be called.
			// The stl::optional<T> type will set the empty_val in this case to "initialize" its member data.
			union
			{
				stl::aligned_storage_t<sizeof(value_type), stl::alignment_of_v<value_type>> val;
				char empty_val;
			};
			bool engaged = false;
		};
		...
		{
			typedef stl::remove_const_t<T> value_type;
			...
			inline explicit optional_storage(in_place_t, Args&&... args)
			    : engaged(true)
			{
				new (stl::addressof(val)) value_type{stl::forward<Args>(args)...};
			}
	...
	public:
	    ...
	    static_assert(!stl::is_reference<value_type>::value, "stl::optional of a reference type is ill-formed");
		static_assert(!stl::is_same<value_type, in_place_t>::value, "stl::optional of a in_place_t type is ill-formed");
		static_assert(!stl::is_same<value_type, nullopt_t>::value, "stl::optional of a nullopt_t type is ill-formed");
		...
		inline EA_CONSTEXPR optional(value_type&& value) EA_NOEXCEPT_IF(stl::is_nothrow_move_constructible_v<T>)
		    : base_type(stl::move(value))
		{
		}
		...
		optional(optional&& other)
		{
			...
			auto* pOtherValue = reinterpret_cast<T*>(stl::addressof(other.val));
			::new (stl::addressof(val)) value_type(stl::move(*pOtherValue));
		}

		template <typename... Args>
		inline EA_CONSTEXPR explicit optional(in_place_t, Args&&... args)
		    : base_type(in_place, stl::forward<Args>(args)...)
		{
		}
		...
		inline explicit optional(in_place_t, std::initializer_list<U> ilist, Args&&... args)
		    : base_type(in_place, ilist, stl::forward<Args>(args)...)
		{
		}

		template <typename U = value_type,
		          typename = stl::enable_if_t<stl::is_constructible_v<T, U&&> &&
		                                        !stl::is_same_v<stl::remove_cvref_t<U>, stl::in_place_t> &&
		                                        !stl::is_same_v<stl::remove_cvref_t<U>, optional>>>
		inline explicit EA_CONSTEXPR optional(U&& value)
		    : base_type(in_place, stl::forward<U>(value))
		{
		}
	    ...
	    inline optional& operator=(optional&& other)
	        EA_NOEXCEPT_IF(EA_NOEXCEPT(stl::is_nothrow_move_assignable<value_type>::value &&
	                                       stl::is_nothrow_move_constructible<value_type>::value))
	    {
			auto* pOtherValue = reinterpret_cast<T*>(stl::addressof(other.val));
			if (engaged == other.engaged)
			{
				if (engaged)
					*get_value_address() = stl::move(*pOtherValue);
			}
			else
			{
				if (engaged)
				...
				{
					construct_value(stl::move(*pOtherValue));
					engaged = true;
				}
			}
		    return *this;
	    ...
	    template <class U, typename = typename stl::enable_if<stl::is_same<stl::decay_t<U>, T>::value>::type>
	    inline optional& operator=(U&& u)
	    {
			if(engaged)
			{
				*get_value_address() = stl::forward<U>(u);
			}
			else
			{
				engaged = true;
				construct_value(stl::forward<U>(u));
			}
	    ...
	    inline value_type value_or(U&& default_value) const
			{ return engaged ? *get_value_address() : static_cast<value_type>(stl::forward<U>(default_value)); }
		...
		{
			construct_value(stl::move(T(stl::forward<Args>(args)...)));
			engaged = true;
		}
		...
		{
			construct_value(stl::move(T(ilist, stl::forward<Args>(args)...)));
			engaged = true;
		}

	    inline void swap(optional& other)
	        EA_NOEXCEPT_IF(stl::is_nothrow_move_constructible<T>::value&& stl::is_nothrow_swappable<T>::value)
	    {
		    using stl::swap;
		    if (engaged == other.engaged)
		    {
		    ...
		    else
		    {
			    ...
			    if (engaged)
			    {
					other.construct_value(stl::move(*(value_type*)stl::addressof(val)));
					destruct_value();
			    }
			    else
			    {
					construct_value(stl::move(*((value_type*)stl::addressof(other.val))));
	...
	private:
	    ...
	    inline T* get_value_address() EASTL_OPTIONAL_NOEXCEPT
	    {
			...
			#endif
			return reinterpret_cast<T*>(stl::addressof(val));
	    }
	    ...
	    inline const T* get_value_address() const EASTL_OPTIONAL_NOEXCEPT
	    {
				...
				EASTL_ASSERT_MSG(engaged, "no value to retrieve");
			#endif
			return reinterpret_cast<const T*>(stl::addressof(val));
	    }
	    ...
	    inline value_type& get_value_ref() EASTL_OPTIONAL_NOEXCEPT
	    {
				...
				EASTL_ASSERT_MSG(engaged, "no value to retrieve");
			#endif
		    return *(value_type*)stl::addressof(val);
	    }
	    ...
	    inline value_type&& get_rvalue_ref() EASTL_OPTIONAL_NOEXCEPT
	    {
				...
				EASTL_ASSERT_MSG(engaged, "no value to retrieve");
			#endif
		    return stl::move(*((value_type*)stl::addressof(val)));
	    }
    }; // class optional
	...
	//
    template <class T>
    inline EA_CONSTEXPR bool operator==(const optional<T>& opt, stl::nullopt_t) EA_NOEXCEPT
		{ return !opt; }

    template <class T>
    inline EA_CONSTEXPR bool operator==(stl::nullopt_t, const optional<T>& opt) EA_NOEXCEPT
		{ return !opt; }
	...
	///
	template <typename T>
	struct hash<stl::optional<T>>
	{
		typedef stl::optional<T> argument_type;
		typedef size_t result_type;

	    result_type operator()(const argument_type& opt) const EA_NOEXCEPT
	    {
		    if (opt)
			    return stl::hash<T>()(*opt);
		    else
	...
	{
		return optional<decay_t<T>>(stl::forward<T>(value));
	}

	template <class T, class... Args>
	inline EA_CONSTEXPR optional<T> make_optional(Args&&... args)
	{
		return optional<T>(stl::in_place, stl::forward<Args>(args)...);
	}

	template <class T, class U, class... Args>
	inline EA_CONSTEXPR optional<T> make_optional(std::initializer_list<U> il, Args&&... args)
	{
		return stl::optional<T>(stl::in_place, il, stl::forward<Args>(args)...);
	}
