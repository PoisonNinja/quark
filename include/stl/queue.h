...
// There are no significant differences between stl/queue and std::queue.
...
namespace stl
{
	...
	///
		...
		#define EASTL_QUEUE_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " queue" // Unless the user overrides something, this is "stl queue".
	#endif
	...
	///
	template <typename T, typename Container = stl::deque<T, EASTLAllocatorType, DEQUE_DEFAULT_SUBARRAY_SIZE(T)> >
	class queue
	{
	public:
	...
	public:
		...
		explicit queue(const Allocator& allocator, typename stl::enable_if<stl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
		  : c(allocator)
		{
		}    

		template <class Allocator>
		queue(const this_type& x, const Allocator& allocator, typename stl::enable_if<stl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
		  : c(x.c, allocator)
		{
		}
			...
			template <class Allocator>
			queue(this_type&& x, const Allocator& allocator, typename stl::enable_if<stl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
			  : c(stl::move(x.c), allocator)
			{
			}
		#endif
		...
		void swap(this_type& x) EA_NOEXCEPT_IF((stl::is_nothrow_swappable<this_type::container_type>::value));
	...
	inline void queue<T, Container>::push(const value_type& value)
	{
		...
		{
			c.emplace_back(stl::forward<Args>(args)...);
		}
	#else
		#if EASTL_MOVE_SEMANTICS_ENABLED
			template <typename T, typename Container>
	...
	template <typename T, typename Container>
	void queue<T, Container>::swap(this_type& x) EA_NOEXCEPT_IF((stl::is_nothrow_swappable<this_type::container_type>::value))
	{
		using stl::swap;
		swap(c, x.c);
	}
	...
	template <typename T, typename Container>
	inline void swap(queue<T, Container>& a, queue<T, Container>& b) EA_NOEXCEPT_IF((stl::is_nothrow_swappable<typename queue<T, Container>::container_type>::value)) // EDG has a bug and won't let us use Container in this noexcept statement
	{
		a.swap(b);
	}
