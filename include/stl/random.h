...
namespace stl
{
	...
	{
		static_assert(stl::is_integral<IntType>::value, "uniform_int_distribution: IntType must be integral.");
	...
	public:
		...
		struct param_type
		{
			explicit param_type(IntType a = 0, IntType b = stl::numeric_limits<IntType>::max());
		...
		uniform_int_distribution(IntType a = 0, IntType b = stl::numeric_limits<IntType>::max());
	...
	uniform_int_distribution<IntType>::operator()(Generator& g, const param_type& params)
	{
		...
		static_assert(stl::is_unsigned<result_type>::value && (sizeof(result_type) <= 4), "uniform_int_distribution currently supports only uint8_t, uint16_t, uint32_t.");
	...
	// stl doesn't currently implement IO stream-related functionality.
	...
	// stl::basic_ostream<CharT, Traits>& operator<<(stl::basic_ostream<CharT, Traits>& os, const uniform_int_distribution& uid);
	//
	// template<class CharT, class Traits, class ResultType>
	// stl::basic_istream<CharT, Traits>& operator>>(stl::basic_istream<CharT, Traits>& is, uniform_int_distribution& uid);
