...
inline stl::safe_object::safe_object()
  :  mpSafePtrList(0)
{
}


inline bool stl::safe_object::is_unreferenced() const
{
	return (mpSafePtrList == NULL);
}


inline void stl::safe_object::clear_references()
{
...
inline void stl::safe_object::add(safe_ptr_base* pBase) const
{
...
inline void stl::safe_object::remove(safe_ptr_base* pBase) const
{
...
inline stl::safe_ptr_base::safe_ptr_base(const safe_ptr_base& safePtrBase)
...
inline stl::safe_ptr_base::safe_ptr_base()
  : mpObject(NULL),
	mpNext(NULL)
{
}
...
inline stl::safe_ptr_base::safe_ptr_base(const safe_object* pObject)
  : mpObject(pObject),
	mpNext(NULL)
{
	if(mpObject)
...
inline void stl::safe_ptr_base::reset()
{
	if(mpObject)
	{
		mpObject->remove(this);
...
inline bool stl::safe_ptr_base::empty() const
{
	return (mpObject == NULL);
}


inline void stl::safe_ptr_base::reset(const safe_object* pNewObject)
{
...
inline bool stl::safe_ptr_base::unique() const
{
...
inline bool stl::safe_object::has_unique_reference() const
{
	return mpSafePtrList ? mpSafePtrList->unique() : false;
}

// Deprecated:
inline bool stl::safe_object::has_references() const
{
	return mpSafePtrList ? mpSafePtrList->unique() : false;
}
...
template<class T>
inline stl::safe_ptr<T>::safe_ptr() 
  : safe_ptr_base()
{
}
...
template<class T>
inline stl::safe_ptr<T>::safe_ptr(T* pObject)
...
template<class T>
inline stl::safe_ptr<T>::safe_ptr(const this_type& safePtr)
...
template<class T>
inline typename stl::safe_ptr<T>::this_type& stl::safe_ptr<T>::operator=(const this_type& safePtr)
{
	if(this != &safePtr)
...
template<class T>
inline typename stl::safe_ptr<T>::this_type& stl::safe_ptr<T>::operator=(T* const pObject)
{
	reset(pObject);
	return *this;
}
...
template<class T>
inline bool stl::safe_ptr<T>::operator==(const this_type& rhs) const
{
...
template<class T>
inline T* stl::safe_ptr<T>::get() const
{
...
template<class T>
inline stl::safe_ptr<T>::operator T*() const
{
	return static_cast<T*>(const_cast<safe_object*>(mpObject));
}
...
template<class T>
inline T* stl::safe_ptr<T>::operator->() const
{
	return static_cast<T*>(const_cast<safe_object*>(mpObject));
}
...
template<class T>
inline bool stl::safe_ptr<T>::operator!() const
{
	return (mpObject == NULL);
}
...
// template<class T>
// inline stl::safe_ptr<T>::operator bool_() const
// {
//     if(mpObject)
//         return &this_type::get;
//     return NULL;
...
template<class T>
inline bool operator==(const stl::safe_ptr<T>& safePtr, const T* pObject)
{
...
template<class T>
inline bool operator<(const stl::safe_ptr<T>& safePtrA, const stl::safe_ptr<T>& safePtrB)
{
