...
namespace stl
{
	...
	///
		...
		#define EASTL_SLIST_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " slist" // Unless the user overrides something, this is "stl slist".
	#endif
	...
	slist<T, Allocator>::front() const
	{
		...
		{
			DoInsertValueAfter((SListNodeBase*)&mNode, stl::forward<Args>(args)...);
		}
	#else
		#if EASTL_MOVE_SEMANTICS_ENABLED
			template <typename T, typename Allocator>
			...
			{
				DoInsertValueAfter((SListNodeBase*)&mNode, stl::move(value));
			}
		#endif
	...
	slist<T, Allocator>::push_front()
	{
		...
		{
			emplace_after(before_begin(), stl::move(value));
		}
	#endif
	...
	slist<T, Allocator>::insert_after(const_iterator position, InputIterator first, InputIterator last)
	{
		...
		{
			return emplace_after(position, stl::move(value));
		}
	#endif
		...
		{
			return iterator((SListNodeBase*)DoInsertValueAfter(position.mpNode, stl::forward<Args>(args)...));
		}
	#else
		#if EASTL_MOVE_SEMANTICS_ENABLED
			template <typename T, typename Allocator>
			...
			{
				return iterator((SListNodeBase*)DoInsertValueAfter(position.mpNode, stl::move(value)));
			}
		#endif
	...
	inline void slist<T, Allocator>::splice_after(const_iterator position, const_iterator before_first, const_iterator before_last)
	{
		if(before_first != before_last) // If there is anything to splice...
		{
				...
				if(i == iEnd) // If the input came from an external range...
					mSize += (size_type)stl::distance(before_first, before_last); // Note that we have no way of knowing how to decrementing the size from the external container, assuming it came from one.
				else
	...
	{
		// To do: look at using a merge sort, which may well be faster. 
		stl::comb_sort(begin(), end(), compare);
	}
	...
	inline void slist<T, Allocator>::reverse() EA_NOEXCEPT
	{
		...
		slist<T, Allocator>::DoCreateNode(Args&&... args)
		{
			node_type* const pNode = DoAllocateNode();  // pNode is of type node_type, but it's uninitialized memory.
				...
				try
				{
					::new((void*)&pNode->mValue) value_type(stl::forward<Args>(args)...);
				}
				catch(...)
				{
					DoFreeNode(pNode);
			...
			slist<T, Allocator>::DoCreateNode(value_type&& value)
			{
				node_type* const pNode = DoAllocateNode();
					...
					try
					{
						::new((void*)&pNode->mValue) value_type(stl::move(value));
					}
					catch(...)
					{
						DoFreeNode(pNode);
	...
	slist<T, Allocator>::DoInsertValueAfter(SListNodeBase* pNode)
	{
		...
		{
			SListNodeBase* pNodeNew = (SListNodeBase*)DoCreateNode(stl::forward<Args>(args)...);
			...
			{
				SListNodeBase* pNodeNew = (SListNodeBase*)DoCreateNode(stl::move(value));
				pNode = SListNodeInsertAfter(pNode, pNodeNew);
				#if EASTL_LIST_SIZE_CACHE
					++mSize; // Increment the size after the node creation because we need to assume an exception can occur in the creation.
				#endif
	...
	{
		stl::swap(mNode.mpNext, x.mNode.mpNext);
