...
namespace stl
{
	...
	{
		typedef stl::less<typename stl::iterator_traits<ForwardIterator>::value_type> Less;

		return stl::is_sorted<ForwardIterator, Less>(first, last, Less());
	}
	...
	/// Example usage:
	///     vector<int> intArray;
	///     vector<int>::iterator unsorted_element = is_sorted_until(stl::end(intArray), stl::end(intArray));
	///     vector<int>::iterator unsorted_element_with_user_compare = is_sorted_until(stl::end(intArray), stl::end(intArray), stl::less<int>());
	///
	template<typename ForwardIterator>
	...
	{
		typedef stl::less<typename stl::iterator_traits<InputIterator1>::value_type> Less;

		return stl::merge<InputIterator1, InputIterator2, OutputIterator, Less>
	...
	{
		typedef typename stl::iterator_traits<BidirectionalIterator>::value_type value_type;
		...
		if(first != last) // if the range is non-empty...
		{
			...
			{
				value_type temp(stl::forward<value_type>(*iSorted));
				...
				// a problem for stl containers and ranges, as they support a single decrement of first,
				...
				for(--iCurrent; (iNext != first) && compare(temp, *iCurrent); --iNext, --iCurrent)
				{
					...
					*iNext = stl::forward<value_type>(*iCurrent);
				}

				*iNext = stl::forward<value_type>(temp);
			}
		}
	} // insertion_sort
	...
	{
		typedef typename stl::iterator_traits<RandomAccessIterator>::difference_type difference_type;
	...
	{
		typedef stl::less<typename stl::iterator_traits<RandomAccessIterator>::value_type> Less;

		stl::shell_sort<RandomAccessIterator, Less>(first, last, Less());
	}
	...
	void heap_sort(RandomAccessIterator first, RandomAccessIterator last, StrictWeakOrdering compare)
	{
		...
		stl::make_heap<RandomAccessIterator, StrictWeakOrdering>(first, last, compare);
		stl::sort_heap<RandomAccessIterator, StrictWeakOrdering>(first, last, compare);
	}
	...
	inline void heap_sort(RandomAccessIterator first, RandomAccessIterator last)
	{
		...
		stl::heap_sort<RandomAccessIterator, Less>(first, last, Less());
	}
	...
	public:
		static void sort(RandomAccessIterator first, RandomAccessIterator last, T* pBuffer, StrictWeakOrdering compare)
		{
			if (sort_impl(first, last, pBuffer, difference_type(0), compare) == RL_Buffer)
			{
				...
				stl::copy<T*, RandomAccessIterator>(pBuffer, pBuffer + nCount, first);
			}
			EASTL_DEV_ASSERT((stl::is_sorted<RandomAccessIterator, StrictWeakOrdering>(first, last, compare)));
		}
	...
	private:
		...
		static ResultLocation sort_impl(RandomAccessIterator first, RandomAccessIterator last, T* pBuffer, difference_type lastSortedEnd, StrictWeakOrdering compare)
		{
			...
			if (lastSortedEnd < nCount)
			{
				...
				{
					stl::Internal::insertion_sort_already_started<RandomAccessIterator, StrictWeakOrdering>(first, last, first + lastSortedEnd, compare);
		...
		static ResultLocation merge_halves(RandomAccessIterator first, RandomAccessIterator last, difference_type nMid, T* pBuffer, bool firstHalfLocation, bool secondHalfLocation, StrictWeakOrdering compare)
		{
			...
			if (firstHalfLocation == RL_SourceRange)
			{
				...
				{
					stl::merge<RandomAccessIterator, RandomAccessIterator, T*, StrictWeakOrdering>(first, first + nMid, first + nMid, last, pBuffer, compare);
					EASTL_DEV_ASSERT((stl::is_sorted<T*, StrictWeakOrdering>(pBuffer, pBuffer + nCount, compare)));
					return RL_Buffer;
				}
				else
				{
					stl::copy(first, first + nMid, pBuffer);
					stl::merge<T*, T*, RandomAccessIterator, StrictWeakOrdering>(pBuffer, pBuffer + nMid, pBuffer + nMid, pBuffer + nCount, first, compare);
					EASTL_DEV_ASSERT((stl::is_sorted<RandomAccessIterator, StrictWeakOrdering>(first, last, compare)));
					return RL_SourceRange;
				}
			}
			else
			{
				if (secondHalfLocation == RL_SourceRange)
				{
					stl::copy(first + nMid, last, pBuffer + nMid);
					stl::merge<T*, T*, RandomAccessIterator, StrictWeakOrdering>(pBuffer, pBuffer + nMid, pBuffer + nMid, pBuffer + nCount, first, compare);
					EASTL_DEV_ASSERT((stl::is_sorted<RandomAccessIterator, StrictWeakOrdering>(first, last, compare)));
					return RL_SourceRange;
				}
	...
	inline void merge_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T* pBuffer)
	{
		...
		stl::merge_sort_buffer<RandomAccessIterator, T, Less>(first, last, pBuffer, Less());
	}
	...
	{
		typedef typename stl::iterator_traits<RandomAccessIterator>::difference_type difference_type;
		typedef typename stl::iterator_traits<RandomAccessIterator>::value_type      value_type;
		...
		if(nCount > 1)
		{
			...
			stl::uninitialized_fill(pBuffer, pBuffer + nCount, value_type());

			stl::merge_sort_buffer<RandomAccessIterator, value_type, StrictWeakOrdering>
									(first, last, pBuffer, compare);

			stl::destruct(pBuffer, pBuffer + nCount);
	...
	inline void merge_sort(RandomAccessIterator first, RandomAccessIterator last, Allocator& allocator)
	{
		...
		stl::merge_sort<RandomAccessIterator, Allocator, Less>(first, last, allocator, Less());
	}
	...
	inline RandomAccessIterator get_partition(RandomAccessIterator first, RandomAccessIterator last, T&& pivotValue)
	{
		...
		return get_partition_impl<RandomAccessIterator, T&&>(first, last, stl::move(pivotValue));
	}
	...
	{
		// Note: unlike the copy-constructible variant of get_partition... we can't create a temporary const move-constructible object
		return get_partition_impl<RandomAccessIterator, T&&, Compare>(first, last, stl::forward<T>(pivotValue), compare);
	}
	...
	inline void partial_sort(RandomAccessIterator first, RandomAccessIterator middle, RandomAccessIterator last)
	{
		...
		stl::make_heap<RandomAccessIterator>(first, middle);
		...
		for(RandomAccessIterator i = middle; i < last; ++i)
		{
			if(*i < *first)
			{
				...
				value_type temp(stl::forward<value_type>(*i));
				*i = stl::forward<value_type>(*first);
				stl::adjust_heap<RandomAccessIterator, difference_type, value_type>
								  (first, difference_type(0), difference_type(middle - first), difference_type(0), stl::forward<value_type>(temp));
			}
		}

		stl::sort_heap<RandomAccessIterator>(first, middle);
	}
	...
	inline void partial_sort(RandomAccessIterator first, RandomAccessIterator middle, RandomAccessIterator last, Compare compare)
	{
		...
		stl::make_heap<RandomAccessIterator, Compare>(first, middle, compare);
		...
		for(RandomAccessIterator i = middle; i < last; ++i)
		{
			if(compare(*i, *first))
			{
				...
				value_type temp(stl::forward<value_type>(*i));
				*i = stl::forward<value_type>(*first);
				stl::adjust_heap<RandomAccessIterator, difference_type, value_type, Compare>
								  (first, difference_type(0), difference_type(middle - first), difference_type(0), stl::forward<value_type>(temp), compare);
			}
		}

		stl::sort_heap<RandomAccessIterator, Compare>(first, middle, compare);
	}
	...
	inline void nth_element(RandomAccessIterator first, RandomAccessIterator nth, RandomAccessIterator last)
	{
		...
		while((last - first) > 5)
		{
			const value_type           midValue(stl::median<value_type>(*first, *(first + (last - first) / 2), *(last - 1)));
			const RandomAccessIterator midPos(stl::get_partition<RandomAccessIterator, value_type>(first, last, midValue));
		...
		stl::insertion_sort<RandomAccessIterator>(first, last);
	}
	...
	inline void nth_element(RandomAccessIterator first, RandomAccessIterator nth, RandomAccessIterator last, Compare compare)
	{
		...
		while((last - first) > 5)
		{
			const value_type           midValue(stl::median<value_type, Compare>(*first, *(first + (last - first) / 2), *(last - 1), compare));
			const RandomAccessIterator midPos(stl::get_partition<RandomAccessIterator, value_type, Compare>(first, last, midValue, compare));
		...
		stl::insertion_sort<RandomAccessIterator, Compare>(first, last, compare);
	}
	...
	namespace Internal
	{
		...
		inline void quick_sort_impl_helper(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount)
		{
			...
			{
				const RandomAccessIterator position(stl::get_partition<RandomAccessIterator, value_type>(first, last,
					stl::forward<PivotValueType>(stl::median<value_type>(stl::forward<value_type>(*first), stl::forward<value_type>(*(first + (last - first) / 2)), stl::forward<value_type>(*(last - 1))))));

				stl::Internal::quick_sort_impl_helper<RandomAccessIterator, Size, PivotValueType>(position, last, --kRecursionCount);
				last = position;
			}

			if(kRecursionCount == 0)
				stl::partial_sort<RandomAccessIterator>(first, last, last);
		}
		...
		inline void quick_sort_impl_helper(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount, Compare compare)
		{
			...
			while(((last - first) > kQuickSortLimit) && (kRecursionCount > 0))
			{
				const RandomAccessIterator position(stl::get_partition<RandomAccessIterator, value_type, Compare>(first, last,
					stl::forward<PivotValueType>(stl::median<value_type, Compare>(stl::forward<value_type>(*first), stl::forward<value_type>(*(first + (last - first) / 2)), stl::forward<value_type>(*(last - 1)), compare)), compare));

				stl::Internal::quick_sort_impl_helper<RandomAccessIterator, Size, Compare, PivotValueType>(position, last, --kRecursionCount, compare);
				last = position;
			}

			if(kRecursionCount == 0)
				stl::partial_sort<RandomAccessIterator, Compare>(first, last, last, compare);
		}
		EA_RESTORE_VC_WARNING()

		template <typename RandomAccessIterator, typename Size>
		inline void quick_sort_impl(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount,
			typename stl::enable_if<stl::is_copy_constructible<typename iterator_traits<RandomAccessIterator>::value_type>::value>::type* = 0)
		{
			typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
		...
		template <typename RandomAccessIterator, typename Size>
		inline void quick_sort_impl(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount,
			typename stl::enable_if<stl::is_move_constructible<typename iterator_traits<RandomAccessIterator>::value_type>::value
			&& !stl::is_copy_constructible<typename iterator_traits<RandomAccessIterator>::value_type>::value>::type* = 0)
		{
			typedef typename iterator_traits<RandomAccessIterator>::value_type value_type;
	...
	void quick_sort(RandomAccessIterator first, RandomAccessIterator last)
	{
		...
		if(first != last)
		{
			stl::Internal::quick_sort_impl<RandomAccessIterator, difference_type>(first, last, 2 * Internal::Log2(last - first));

			if((last - first) > (difference_type)kQuickSortLimit)
			{
				stl::insertion_sort<RandomAccessIterator>(first, first + kQuickSortLimit);
				stl::Internal::insertion_sort_simple<RandomAccessIterator>(first + kQuickSortLimit, last);
			}
			else
				stl::insertion_sort<RandomAccessIterator>(first, last);
		}
	...
	void quick_sort(RandomAccessIterator first, RandomAccessIterator last, Compare compare)
	{
		...
		if(first != last)
		{
			stl::Internal::quick_sort_impl<RandomAccessIterator, difference_type, Compare>(first, last, 2 * Internal::Log2(last - first), compare);

			if((last - first) > (difference_type)kQuickSortLimit)
			{
				stl::insertion_sort<RandomAccessIterator, Compare>(first, first + kQuickSortLimit, compare);
				stl::Internal::insertion_sort_simple<RandomAccessIterator, Compare>(first + kQuickSortLimit, last, compare);
			}
			else
				stl::insertion_sort<RandomAccessIterator, Compare>(first, last, compare);
		}
	...
	namespace Internal
	{
		...
		// To consider: Use void stl::reverse(BidirectionalIterator first, BidirectionalIterator last);
		//
		template <typename RandomAccessIterator>
		void reverse_elements(RandomAccessIterator first, intptr_t start, intptr_t end)
		{
			while(start < end)
			{
				stl::swap(*(first + start), *(first + end));
				++start;
				--end;
			}
		}
							...
							T* pBuffer, StrictWeakOrdering compare)
		{
			...
			if(A < B) // If the first run is shorter than the second run... merge left.
			{
				...
				stl::copy(first + curr, first + curr + A, pBuffer);
			...
			{
				stl::copy(first + curr + A, first + curr + A + B, pBuffer);
	...
	// the case that at at least half your data is already sorted. Otherwise, stl::quick_sort is about 10% 
	...
	{
		typedef stl::less<T> Less;

		stl::tim_sort_buffer<RandomAccessIterator, T, Less>(first, last, pBuffer, Less());
	}
	...
	void radix_sort(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator buffer)
	{
		...
		stl::Internal::radix_sort_impl<RandomAccessIterator, ExtractKey, DigitBits>(first, last, buffer, ExtractKey(), typename ExtractKey::radix_type());
	}
	...
	{
		typedef typename stl::iterator_traits<ForwardIterator>::difference_type difference_type;

		ForwardIterator iCurrent, iNext;
		difference_type length = stl::distance(first, last);
	...
	inline void comb_sort(ForwardIterator first, ForwardIterator last)
	{
		...
		stl::comb_sort<ForwardIterator, Less>(first, last, Less());
	}
	...
	{
		typedef typename stl::iterator_traits<ForwardIterator>::iterator_category IC;

		stl::Internal::bubble_sort_impl<ForwardIterator, StrictWeakOrdering>(first, last, compare, IC());
	}
	...
	inline void bubble_sort(ForwardIterator first, ForwardIterator last)
	{
		...
		stl::Internal::bubble_sort_impl<ForwardIterator, Less>(first, last, Less(), IC());
	}
	...
	/// If a default sort function is specified then call it, otherwise use stl's default quick_sort.
	/// EASTL_DEFAULT_SORT_FUNCTION must be namespace-qualified and include any necessary template
	/// parameters (e.g. stl::comb_sort instead of just comb_sort), and it must be visible to this code. 
	...
	inline void sort(RandomAccessIterator first, RandomAccessIterator last)
	{
		...
		#else
			stl::quick_sort<RandomAccessIterator>(first, last);
		#endif
	}
	...
	inline void sort(RandomAccessIterator first, RandomAccessIterator last, Compare compare)
	{
		...
		#else
			stl::quick_sort<RandomAccessIterator, Compare>(first, last, compare);
		#endif
	}
	...
	/// If a default sort function is specified then call it, otherwise use stl's default merge_sort.
	/// EASTL_DEFAULT_STABLE_SORT_FUNCTION must be namespace-qualified and include any necessary template
	/// parameters (e.g. stl::tim_sort instead of just tim_sort), and it must be visible to this code. 
	...
	void stable_sort(RandomAccessIterator first, RandomAccessIterator last, StrictWeakOrdering compare)
	{
		...
		#else
			stl::merge_sort<RandomAccessIterator, EASTLAllocatorType, StrictWeakOrdering>
	...
	void stable_sort(RandomAccessIterator first, RandomAccessIterator last)
	{
		...
		#else
			stl::merge_sort<RandomAccessIterator, EASTLAllocatorType>
	...
	void stable_sort(RandomAccessIterator first, RandomAccessIterator last, Allocator& allocator, StrictWeakOrdering compare)
	{
		...
		#else
			stl::merge_sort<RandomAccessIterator, Allocator, StrictWeakOrdering>(first, last, allocator, compare);
		#endif
	}
	...
	//    #else
	//        stl::merge_sort<RandomAccessIterator, Allocator>(first, last, allocator);
	//    #endif
	//}
	...
	/* 
	// Something to consider adding: An stl sort which uses qsort underneath. 
	// The primary purpose of this is to have an stl interface for sorting which
	...
	{
		typedef typename stl::iterator_traits<ContiguousIterator>::value_type value_type;

		qsort(first, (size_t)stl::distance(first, last), sizeof(value_type), small_footprint_sort_func<value_type>);
	}
	*/
