...
namespace stl
{
	...
	///
		...
		#define EASTL_STACK_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " stack" // Unless the user overrides something, this is "stl stack".
	#endif
	...
	public:
		...
		explicit stack(const Allocator& allocator, typename stl::enable_if<stl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
		  : c(allocator)
		{
		}    

		template <class Allocator>
		stack(const this_type& x, const Allocator& allocator, typename stl::enable_if<stl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
		  : c(x.c, allocator)
		{
		}
			...
			template <class Allocator>
			stack(this_type&& x, const Allocator& allocator, typename stl::enable_if<stl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
	...
	template <typename T, typename Container>
	void stack<T, Container>::swap(this_type& x) EA_NOEXCEPT_IF(stl::is_nothrow_swappable<this_type::container_type>::value)
	{
		using stl::swap;
		swap(c, x.c);
	}
	...
	template <typename T, typename Container>
	inline void swap(stack<T, Container>& a, stack<T, Container>& b) EA_NOEXCEPT_IF((stl::is_nothrow_swappable<typename stack<T, Container>::container_type>::value))
	{
		a.swap(b);
	}
