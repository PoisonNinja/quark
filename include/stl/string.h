...
//    - basic_string has a few extension functions that allow for increased
//    performance.
...
//    - basic_string internally manually expands basic functions such as
//    begin(),
//      size(), etc. in order to improve debug performance and optimizer
//      success.
//    - basic_string is savvy to an environment that doesn't have exception
//    handling,
//      as is sometimes the case with console or embedded environments.
//    - basic_string has less deeply nested function calls and allows the user
//    to
//      enable forced inlining in debug builds in order to reduce bloat.
//    - basic_string doesn't use char traits. As a result, stl assumes that
...
//    - basic_string data is guaranteed to be 0-terminated, and the c_str()
//    function
//      is guaranteed to return the same pointer as the data() which is
//      guaranteed to be the same value as &string[0].
//    - basic_string has a set_capacity() function which frees excess capacity.
//      The only way to do this with std::basic_string is via the cryptic
//      non-obvious trick of using: basic_string<char>(x).swap(x);
//    - basic_string has a force_size() function, which unilaterally moves the
//    string
//      end position (mpEnd) to the given location. Useful for when the user
//      writes into the string via some extenal means such as C strcpy or
//      sprintf.
//    - basic_string substr() deviates from the standard and returns a string
//    with
...
// Copy on Write (cow)
//
// This string implementation does not do copy on write (cow). This is by
// design, as cow penalizes 95% of string uses for the benefit of only 5% of the
// uses (these percentages are qualitative, not quantitative). The primary
// benefit of cow is that it allows for the sharing of string data between two
// string objects. Thus if you say this:
//    string a("hello");
//    string b(a);
// the "hello" will be shared between a and b. If you then say this:
//    a = "world";
// then a will release its reference to "hello" and leave b with the only
// reference to it. Normally this functionality is accomplished via reference
// counting and with atomic operations or mutexes.
//
...
// a cow string. The discussion of these issues will not be rehashed here, as
// you can read the references below for better detail than can be provided in
// the space we have here. However, we can say that the C++ standard is sensible
// and that anything we try to do here to allow for an efficient cow
// implementation would result in a generally unacceptable string interface.
//
// The disadvantages of cow strings are:
//    - A reference count needs to exist with the string, which increases string
//    memory usage.
//    - With thread safety, atomic operations and mutex locks are expensive,
//    especially
//      on weaker memory systems such as console gaming platforms.
//    - All non-const string accessor functions need to do a sharing check the
//    the
//      first such check needs to detach the string. Similarly, all string
//      assignments need to do a sharing check as well. If you access the string
//      before doing an assignment, the assignment doesn't result in a shared
//      string, because the string has already been detached.
//    - String sharing doesn't happen the large majority of the time. In some
//    cases,
//      the total sum of the reference count memory can exceed any memory
//      savings gained by the strings that share representations.
//
// The addition of a string_cow class is under consideration for this library.
// There are conceivably some systems which have string usage patterns which
// would benefit from cow sharing. Such functionality is best saved for a
// separate string implementation so that the other string uses aren't
// penalized.
//
// References:
...
#include <stl/bonus/compressed_pair.h>
...
#include <lib/printf.h> // snprintf, etc.
...
#pragma warning(                                                               \
    disable : 4267) // 'argument' : conversion from 'size_t' to 'const
                    // uint32_t', possible loss of data. This is a bogus warning
                    // resulting from a bug in VC++.
#pragma warning(disable : 4480) // nonstandard extension used: specifying
                                // underlying type for enum
#pragma warning(                                                               \
    disable : 4571) // catch(...) semantics changed since Visual C++ 7.1;
...
#include <stl/internal/char_traits.h>
...
// These functions can deal with C99 standard return values or Microsoft
// non-standard return values but act more efficiently if implemented via the
// C99 style.
//
...
EASTL_EASTDC_API int Vsnprintf(char8_t* EA_RESTRICT pDestination, size_t n,
                               const char8_t* EA_RESTRICT pFormat,
                               va_list arguments);
EASTL_EASTDC_API int Vsnprintf(char16_t* EA_RESTRICT pDestination, size_t n,
                               const char16_t* EA_RESTRICT pFormat,
                               va_list arguments);
EASTL_EASTDC_API int Vsnprintf(char32_t* EA_RESTRICT pDestination, size_t n,
                               const char32_t* EA_RESTRICT pFormat,
                               va_list arguments);
#if defined(EA_WCHAR_UNIQUE) && EA_WCHAR_UNIQUE
EASTL_EASTDC_API int Vsnprintf(wchar_t* EA_RESTRICT pDestination, size_t n,
...
namespace stl
{
inline int Vsnprintf(char8_t* EA_RESTRICT pDestination, size_t n,
                     const char8_t* EA_RESTRICT pFormat, va_list arguments)
{
    return EA::StdC::Vsnprintf(pDestination, n, pFormat, arguments);
}

inline int Vsnprintf(char16_t* EA_RESTRICT pDestination, size_t n,
                     const char16_t* EA_RESTRICT pFormat, va_list arguments)
{
    return EA::StdC::Vsnprintf(pDestination, n, pFormat, arguments);
}

inline int Vsnprintf(char32_t* EA_RESTRICT pDestination, size_t n,
                     const char32_t* EA_RESTRICT pFormat, va_list arguments)
{
    return EA::StdC::Vsnprintf(pDestination, n, pFormat, arguments);
}

#if defined(EA_WCHAR_UNIQUE) && EA_WCHAR_UNIQUE
inline int Vsnprintf(wchar_t* EA_RESTRICT pDestination, size_t n,
                     const wchar_t* EA_RESTRICT pFormat, va_list arguments)
{
    return EA::StdC::Vsnprintf(pDestination, n, pFormat, arguments);
}
#endif
...
extern int Vsnprintf8(char8_t* pDestination, size_t n, const char8_t* pFormat,
                      va_list arguments);
extern int Vsnprintf16(char16_t* pDestination, size_t n,
                       const char16_t* pFormat, va_list arguments);
extern int Vsnprintf32(char32_t* pDestination, size_t n,
                       const char32_t* pFormat, va_list arguments);
#if defined(EA_WCHAR_UNIQUE) && EA_WCHAR_UNIQUE
extern int VsnprintfW(wchar_t* pDestination, size_t n, const wchar_t* pFormat,
                      va_list arguments);
#endif

namespace stl
{
inline int Vsnprintf(char8_t* pDestination, size_t n, const char8_t* pFormat,
                     va_list arguments)
{
    return Vsnprintf8(pDestination, n, pFormat, arguments);
}

inline int Vsnprintf(char16_t* pDestination, size_t n, const char16_t* pFormat,
                     va_list arguments)
{
    return Vsnprintf16(pDestination, n, pFormat, arguments);
}

inline int Vsnprintf(char32_t* pDestination, size_t n, const char32_t* pFormat,
                     va_list arguments)
{
...
#if defined(EA_WCHAR_UNIQUE) && EA_WCHAR_UNIQUE
inline int Vsnprintf(wchar_t* pDestination, size_t n, const wchar_t* pFormat,
                     va_list arguments)
{
...
#define EASTL_BASIC_STRING_DEFAULT_NAME                                        \
    EASTL_DEFAULT_NAME_PREFIX                                                  \
        " basic_string" // Unless the user overrides something, this is "stl
                        // basic_string".
#endif
...
#define EASTL_BASIC_STRING_DEFAULT_ALLOCATOR                                   \
    allocator_type(EASTL_BASIC_STRING_DEFAULT_NAME)
#endif
...
public:
    ...
    typedef T*
        iterator; // Maintainer note: We want to leave iterator defined as T* --
                  // at least in release builds -- as this gives some algorithms
                  // an advantage that optimizers cannot get around.
    ...
    typedef eastl_size_t size_type; // See config.h for the definition of
                                    // eastl_size_t, which defaults to uint32_t.
    typedef ptrdiff_t difference_type;
    typedef Allocator allocator_type;

    static const size_type npos =
        (size_type)-1; /// 'npos' means non-valid position or simply
                       /// non-position.

public:
    // CtorDoNotInitialize exists so that we can create a constructor that
    // allocates but doesn't initialize and also doesn't collide with any other
    // constructor declaration.
    struct CtorDoNotInitialize {
    };

    // CtorSprintf exists so that we can create a constructor that accepts
    // printf-style arguments but also doesn't collide with any other
    // constructor declaration.
    struct CtorSprintf {
    };

    // CtorConvert exists so that we can have a constructor that implements
    // string encoding conversion, such as between UCS2 char16_t and UTF8
    // char8_t.
    struct CtorConvert {
    };
...
// Masks used to determine if we are in SSO or Heap
    ...
    // Big Endian use LSB, unless we want to reorder struct layouts on
    // endianness, Bit is set when we are in Heap
...
protected:
    ...
    struct HeapLayout {
        ...
        size_type mnSize; // Size of the string. Number of characters currently
                          // in the string, not including the trailing '0'
        size_type mnCapacity; // Capacity of the string. Number of characters
                              // string can hold, not including the trailing '0'
    };
    ...
    struct SSOPadding<CharT, 1> {
        // template specialization to remove the padding structure to avoid
        // warnings on zero length arrays also, this allows us to take advantage
        // of the empty-base-class optimization.
    };

    // The view of memory when the string data is able to store the string data
    // locally (without a heap allocation).
    struct SSOLayout {
        enum : size_type {
            SSO_CAPACITY =
        ...
        // mnSize must correspond to the last byte of HeapLayout.mnCapacity, so
        // we don't want the compiler to insert padding after mnSize if
        // sizeof(value_type) != 1; Also ensures both layouts are the same size.
    ...
    // This view of memory is a utility structure for easy copying of the string
    // data.
    struct RawLayout {
    ...
    static_assert(sizeof(SSOLayout) == sizeof(HeapLayout),
                  "heap and sso layout structures must be the same size");
    static_assert(sizeof(HeapLayout) == sizeof(RawLayout),
                  "heap and raw layout structures must be the same size");

    // This implements the 'short string optimization' or SSO. SSO reuses the
    // existing storage of string class to hold string data short enough to fit
    // therefore avoiding a heap allocation. The number of characters stored in
    // the string SSO buffer is variable and depends on the string character
    // width. This implementation favors a consistent string size than
    // increasing the size of the string local data to accommodate a consistent
    // number of characters despite character width.
    struct Layout {
        union {
            HeapLayout heap;
            SSOLayout sso;
        ...
        // We are using Heap when the bit is set, easier to conceptualize
        // checking IsHeap instead of IsSSO
        ...
        // Largest value for SSO.mnSize == 23, which has two LSB bits set, but
        // on big-endian (BE) use least significant bit (LSB) to denote heap so
        // shift.
        inline size_type GetSSOSize() const EA_NOEXCEPT
        {
#ifdef EA_SYSTEM_BIG_ENDIAN
            return SSOLayout::SSO_CAPACITY -
                   (sso.mRemainingSizeField.mnRemainingSize >> 2);
#else
            return (SSOLayout::SSO_CAPACITY -
        ...
        {
            return IsHeap() ? GetHeapSize() : GetSSOSize();
        }
...
#ifdef EA_SYSTEM_BIG_ENDIAN
            sso.mRemainingSizeField.mnRemainingSize =
                (char)((SSOLayout::SSO_CAPACITY - size) << 2);
#else
            sso.mRemainingSizeField.mnRemainingSize =
        ...
        {
            IsHeap() ? SetHeapSize(size) : SetSSOSize(size);
        }

        inline size_type GetRemainingCapacity() const EA_NOEXCEPT
        {
            return size_type(CapacityPtr() - EndPtr());
        }
        ...
        inline value_type* SSOCapcityPtr() EA_NOEXCEPT
        {
            return sso.mData + SSOLayout::SSO_CAPACITY;
        }
        inline const value_type* SSOCapcityPtr() const EA_NOEXCEPT
        {
            return sso.mData + SSOLayout::SSO_CAPACITY;
        }

        // Points to end of the buffer at the terminating '0', *ptr == '0' <-
        // not true for SSO
        inline value_type* CapacityPtr() EA_NOEXCEPT
        {
            return IsHeap() ? HeapCapacityPtr() : SSOCapcityPtr();
        }
        ...
        {
            stl::swap(dst.raw, src.raw);
        }
        inline void Swap(Layout& a, Layout& b) EA_NOEXCEPT
        {
            stl::swap(a.raw, b.raw);
        }

        inline void ResetToSSO() EA_NOEXCEPT
        {
            memset(&raw, 0, sizeof(RawLayout));
        }
    };

    stl::compressed_pair<Layout, allocator_type> mPair;
...
public:
    ...
    basic_string()
        EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR(EASTL_BASIC_STRING_DEFAULT_ALLOCATOR));
    ...
    basic_string(
        ...
        const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
    basic_string(
        CtorDoNotInitialize, size_type n,
        const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
    ...
    explicit basic_string(
        ...
        const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);
    basic_string(
        const view_type& sv, size_type position, size_type n,
        const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);

    template <typename OtherCharType>
    basic_string(
        CtorConvert, const OtherCharType* p,
        const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);

    template <typename OtherCharType>
    basic_string(
        CtorConvert, const OtherCharType* p, size_type n,
        const allocator_type& allocator = EASTL_BASIC_STRING_DEFAULT_ALLOCATOR);

    template <typename OtherStringType> // Unfortunately we need the CtorConvert
                                        // here because otherwise this function
                                        // would collide with the value_type*
                                        // constructor.
    ...
    } // We need this because otherwise the const value_type* version can
      // collide with the const OtherStringType& version below.
    ...
    this_type& assign(const this_type& x, size_type position,
                      size_type n = npos);
    ...
    const_iterator
    begin() const EA_NOEXCEPT; // Expanded in source code as: mpBegin
    ...
    void set_capacity(
        size_type n =
            npos); // Revises the capacity to the user-specified value. Resizes
                   // the container to match the capacity if the requested
                   // capacity n is less than the current size. If n == npos
                   // then the capacity is reallocated (if necessary) such that
                   // capacity == size.
    void force_size(
        size_type n); // Unilaterally moves the string end position (mpEnd) to
                      // the given location. Useful for when the user writes
                      // into the string via some extenal means such as C strcpy
                      // or sprintf. This allows for more efficient use than
                      // using resize to achieve this.
    void shrink_to_fit();
    ...
    this_type& append(const this_type& x, size_type position,
                      size_type n = npos);
    ...
    this_type& append_sprintf_va_list(const value_type* pFormat,
                                      va_list arguments);
    ...
    this_type& insert(size_type position, const this_type& x, size_type beg,
                      size_type n);
    ...
    iterator insert(const_iterator p, const value_type* pBegin,
    ...
    this_type& replace(size_type position, size_type n1, const value_type* p,
                       size_type n2);
    this_type& replace(size_type position, size_type n1, const value_type* p);
    this_type& replace(size_type position, size_type n1, size_type n2,
                       value_type c);
    ...
    this_type& replace(const_iterator first, const_iterator last,
                       const value_type* p);
    this_type& replace(const_iterator first, const_iterator last, size_type n,
                       value_type c);
    this_type& replace(const_iterator first, const_iterator last,
                       const value_type* pBegin, const value_type* pEnd);
    ...
    // Find operations
    size_type find(const this_type& x,
                   size_type position = 0) const EA_NOEXCEPT;
    ...
    size_type rfind(const this_type& x,
                    size_type position = npos) const EA_NOEXCEPT;
    ...
    size_type find_first_of(const this_type& x,
                            size_type position = 0) const EA_NOEXCEPT;
    size_type find_first_of(const value_type* p, size_type position = 0) const;
    size_type find_first_of(const value_type* p, size_type position,
                            size_type n) const;
    size_type find_first_of(value_type c,
                            size_type position = 0) const EA_NOEXCEPT;

    // Find last-of operations
    size_type find_last_of(const this_type& x,
                           size_type position = npos) const EA_NOEXCEPT;
    size_type find_last_of(const value_type* p,
                           size_type position = npos) const;
    size_type find_last_of(const value_type* p, size_type position,
                           size_type n) const;
    size_type find_last_of(value_type c,
                           size_type position = npos) const EA_NOEXCEPT;

    // Find first not-of operations
    size_type find_first_not_of(const this_type& x,
                                size_type position = 0) const EA_NOEXCEPT;
    size_type find_first_not_of(const value_type* p,
                                size_type position = 0) const;
    size_type find_first_not_of(const value_type* p, size_type position,
                                size_type n) const;
    size_type find_first_not_of(value_type c,
                                size_type position = 0) const EA_NOEXCEPT;

    // Find last not-of operations
    size_type find_last_not_of(const this_type& x,
                               size_type position = npos) const EA_NOEXCEPT;
    size_type find_last_not_of(const value_type* p,
                               size_type position = npos) const;
    size_type find_last_not_of(const value_type* p, size_type position,
                               size_type n) const;
    ...
    int compare(size_type pos1, size_type n1, const this_type& x,
                size_type pos2, size_type n2) const;
    int compare(const value_type* p) const;
    int compare(size_type pos1, size_type n1, const value_type* p) const;
    int compare(size_type pos1, size_type n1, const value_type* p,
                size_type n2) const;
    static int compare(const value_type* pBegin1, const value_type* pEnd1,
                       const value_type* pBegin2, const value_type* pEnd2);

    // Case-insensitive comparison functions. Not part of C++ this_type. Only
    // ASCII-level locale functionality is supported. Thus this is not suitable
    // for localization purposes.
    int comparei(const this_type& x) const EA_NOEXCEPT;
    int comparei(const value_type* p) const;
    static int comparei(const value_type* pBegin1, const value_type* pEnd1,
                        const value_type* pBegin2, const value_type* pEnd2);
...
protected:
    ...
    static const value_type* CharTypeStringFindEnd(const value_type* pBegin,
                                                   const value_type* pEnd,
                                                   value_type c);
    ...
    static const value_type* CharTypeStringRSearch(const value_type* p1Begin,
                                                   ...
                                                   const value_type* p2Begin,
                                                   const value_type* p2End);
    static const value_type* CharTypeStringFindFirstOf(
        const value_type* p1Begin, const value_type* p1End,
        const value_type* p2Begin, const value_type* p2End);
    static const value_type* CharTypeStringRFindFirstOf(
        const value_type* p1RBegin, const value_type* p1REnd,
        const value_type* p2Begin, const value_type* p2End);
    static const value_type* CharTypeStringFindFirstNotOf(
        const value_type* p1Begin, const value_type* p1End,
        const value_type* p2Begin, const value_type* p2End);
    static const value_type* CharTypeStringRFindFirstNotOf(
        const value_type* p1RBegin, const value_type* p1REnd,
        const value_type* p2Begin, const value_type* p2End);
...
template <typename T, typename Allocator>
basic_string<T, Allocator>::basic_string(const this_type& x,
...
template <typename T, typename Allocator>
basic_string<T, Allocator>::basic_string(const this_type& x, size_type position,
                                         size_type n)
    : mPair(x.get_allocator())
{
    ...
    if (EASTL_UNLIKELY(position > x.internalLayout().GetSize())) // 21.4.2 p4
    {
        ...
        RangeInitialize(
            ...
            x.internalLayout().BeginPtr() + position +
                stl::min_alt(n, x.internalLayout().GetSize() - position));
#else
    RangeInitialize(
        x.internalLayout().BeginPtr() + position,
        x.internalLayout().BeginPtr() + position +
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>::basic_string(const value_type* p,
                                                size_type n,
                                                const allocator_type& allocator)
    : mPair(allocator)
{
...
inline basic_string<T, Allocator>::basic_string(CtorConvert,
    ...
    : mPair(allocator)
{
    AllocateSelf(); // In this case we are converting from one string encoding
                    // to another, and we
    append_convert(p); // implement this in the simplest way, by simply
                       // default-constructing and calling assign.
}
    ...
    : mPair(allocator)
{
    AllocateSelf(); // In this case we are converting from one string encoding
                    // to another, and we
    append_convert(p, n); // implement this in the simplest way, by simply
                          // default-constructing and calling assign.
}
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>::basic_string(size_type n, value_type c,
                                                const allocator_type& allocator)
    : mPair(allocator)
{
    SizeInitialize(n, c);
...
// CtorDoNotInitialize exists so that we can create a version that allocates but
// doesn't initialize but also doesn't collide with any other constructor
// declaration.
template <typename T, typename Allocator>
...
// CtorSprintf exists so that we can create a version that does a variable
// argument sprintf but also doesn't collide with any other constructor
// declaration.
template <typename T, typename Allocator>
basic_string<T, Allocator>::basic_string(CtorSprintf /*unused*/,
...
template <typename T, typename Allocator>
basic_string<T, Allocator>::basic_string(std::initializer_list<value_type> init,
                                         const allocator_type& allocator)
    : mPair(allocator)
{
...
basic_string<T, Allocator>::basic_string(this_type&& x) EA_NOEXCEPT
    : mPair(x.get_allocator())
{
    internalLayout() = stl::move(x.internalLayout());
    x.AllocateSelf();
}
...
template <typename T, typename Allocator>
inline void
basic_string<T, Allocator>::set_allocator(const allocator_type& allocator)
{
...
{
#if EASTL_ASSERT_ENABLED // We allow the user to reference the trailing 0 char
                         // without asserting. Perhaps we shouldn't.
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::this_type&
basic_string<T, Allocator>::operator=(const this_type& x)
{
    if (&x != this) {
        ...
        if (bSlowerPathwayRequired) {
            set_capacity(0); // Must use set_capacity instead of clear because
                             // set_capacity frees our memory, unlike clear.
...
{
    assign_convert(&c, 1); // Call this version of append because it will result
                           // in the encoding-converting append being used.
}

template <typename T, typename Allocator>
template <typename StringType>
inline void basic_string<T, Allocator>::DoAssignConvert(const StringType& x,
                                                        false_type)
{
...
template <typename OtherStringType>
inline typename basic_string<T, Allocator>::this_type&
basic_string<T, Allocator>::operator=(const OtherStringType& x)
{
    clear();
...
template <typename OtherCharType>
inline typename basic_string<T, Allocator>::this_type&
basic_string<T, Allocator>::operator=(const OtherCharType* p)
{
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::this_type&
basic_string<T, Allocator>::operator=(const value_type* p)
{
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::this_type&
basic_string<T, Allocator>::operator=(value_type c)
{
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::this_type&
basic_string<T, Allocator>::operator=(this_type&& x)
{
    return assign(stl::move(x));
}

template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::this_type&
basic_string<T, Allocator>::operator=(std::initializer_list<value_type> ilist)
{
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::this_type&
basic_string<T, Allocator>::operator=(view_type v)
{
...
{
    // C++ basic_string specifies that resize(n) is equivalent to resize(n,
    // value_type()). For built-in types, value_type() is the same as zero
    // (value_type(0)). We can improve the efficiency (especially for long
    // strings) of this string class by resizing without assigning to anything.
...
void basic_string<T, Allocator>::reserve(size_type n)
{
    ...
    // C++20 says if the passed in capacity is less than the current capacity we
    // do not shrink If new_cap is less than or equal to the current capacity(),
    // there is no effect.
    // http://en.cppreference.com/w/cpp/string/basic_string/reserve

    n = stl::max_alt(
        n, internalLayout().GetSize()); // Calculate the new capacity, which
                                        // needs to be >= container size.
...
inline void basic_string<T, Allocator>::set_capacity(size_type n)
{
    if (n == npos)
        ...
        // '-1' because we pretend that we didn't allocate memory for the
        // terminating 0.
    ...
    if ((n < capacity() && internalLayout().IsHeap()) || (n > capacity())) {
        // In here the string is transition from heap->heap, heap->sso or
        // sso->heap
        ...
        if (EASTL_LIKELY(n)) {
            ...
            if (n <= SSOLayout::SSO_CAPACITY) {
                // heap->sso
                // A heap based layout wants to reduce its size to within sso
                // capacity An sso layout wanting to reduce its capacity will
                // not get in here
                ...
                CharStringUninitializedCopy(pOldBegin, pOldBegin + n,
            ...
            pointer pNewBegin = DoAllocate(
                n + 1); // We need the + 1 to accomodate the trailing 0.
            size_type nSavedSize =
                internalLayout().GetSize(); // save the size in case we
                                            // transition from sso->heap

            pointer pNewEnd = CharStringUninitializedCopy(
                internalLayout().BeginPtr(), internalLayout().EndPtr(),
                pNewBegin);
            *pNewEnd = 0;
...
basic_string<T, Allocator>::detach() EA_NOEXCEPT
{
    ...
    if (internalLayout().IsSSO()) {
        const size_type n =
            internalLayout().GetSize() +
            1; // +1' so that we have room for the terminating 0.
        pDetached       = DoAllocate(n);
        pointer pNewEnd = CharStringUninitializedCopy(
            internalLayout().BeginPtr(), internalLayout().EndPtr(), pDetached);
        *pNewEnd = 0;
    } else {
...
basic_string<T, Allocator>::at(size_type n) const
{
    ...
    if (EASTL_UNLIKELY(n >= internalLayout().GetSize()))
        ThrowRangeException();
#elif EASTL_ASSERT_ENABLED // We assert if the user references the trailing 0
                           // char.
    if (EASTL_UNLIKELY(n >= internalLayout().GetSize()))
...
basic_string<T, Allocator>::front()
{
    ...
    if (EASTL_UNLIKELY(
            internalLayout().GetSize() <=
            0)) // We assert if the user references the trailing 0 char.
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>& basic_string<T, Allocator>::
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::append(const this_type& x)
{
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::append(const this_type& x, size_type position,
                                   size_type n)
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(
            position >=
            x.internalLayout().GetSize())) // position must be < x.mpEnd, but
                                           // position + n may be > mpEnd.
        ThrowRangeException();
#endif
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::append(const value_type* p, size_type n)
{
    return append(p, p + n);
}
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::append(const value_type* p)
{
...
basic_string<T, Allocator>::append_convert(const OtherCharType* pOther,
                                           size_type n)
{
    // Question: What do we do in the case that we have an illegally encoded
    // source string? This can happen with UTF8 strings. Do we throw an
    // exception or do we ignore the input? One argument is that it's not a
    // string class' job to handle the security aspects of a program and the
    // higher level application code should be verifying UTF8 string validity,
    // and thus we should do the friendly thing and ignore the invalid
    // characters as opposed to making the user of this function handle
    // exceptions that are easily forgotten.

    const size_t kBufferSize = 512;
    value_type
        selfBuffer[kBufferSize]; // This assumes that value_type is one of
                                 // char8_t, char16_t, char32_t, or wchar_t. Or
                                 // more importantly, a type with a trivial
    ...
    while (pOther != pOtherEnd) {
        ...
        DecodePart(pOther, pOtherEnd, pSelfBufferCurrent,
                   selfBufferEnd); // Write pOther to pSelfBuffer, converting
                                   // encoding as we go. We currently ignore the
                                   // return value, as we don't yet have a plan
...
template <typename T, typename Allocator>
basic_string<T, Allocator>& basic_string<T, Allocator>::append(size_type n,
                                                               value_type c)
{
    ...
    if ((nSize + n) > nCapacity)
        reserve(stl::max_alt(GetNewCapacity(nCapacity), (nSize + n)));

    if (n > 0) {
        pointer pNewEnd =
            CharStringUninitializedFillN(internalLayout().EndPtr(), n, c);
        *pNewEnd = 0;
...
template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::append(const value_type* pBegin,
                                   const value_type* pEnd)
{
    if (pBegin != pEnd) {
        ...
        if ((nOldSize + n) > nCapacity) {
            const size_type nLength =
                stl::max_alt(GetNewCapacity(nCapacity), (nOldSize + n));
            ...
            *pNewEnd = 0;
            ...
            pointer pNewEnd = CharStringUninitializedCopy(
                pBegin, pEnd, internalLayout().EndPtr());
            *pNewEnd = 0;
...
template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::append_sprintf_va_list(const value_type* pFormat,
                                                   va_list arguments)
{
    ...
    // C99 vsnprintf states that a buffer size of zero returns the number of
    // characters that would be written to the buffer irrelevant of whether the
    // buffer is a nullptr But C99 vswprintf for wchar_t changes the behaviour
    // of the return to instead say that it "will fail if n or more wide
    // characters were requested to be written", so calling vswprintf with a
    // buffer size of zero always returns -1 unless... you are MSVC where they
    // deviate from the std and say if the buffer is NULL and the size is zero
    // it will return the number of characters written or if we are using EAStdC
    // which also does the sane behaviour.
    ...
    nReturnValue = stl::Vsnprintf(nullptr, 0, pFormat, arguments);
    ...
    if (nReturnValue > 0) {
        ...
        nReturnValue =
            stl::Vsnprintf(internalLayout().BeginPtr() + nInitialSize,
...
#else
    size_type nInitialSize = internalLayout().GetSize();
    size_type nInitialRemainingCapacity =
        internalLayout().GetRemainingCapacity();
    int nReturnValue;
    ...
    nReturnValue = stl::Vsnprintf(internalLayout().EndPtr(),
                                    (size_t)nInitialRemainingCapacity + 1,
                                    pFormat, arguments);

    if (nReturnValue >= (int)(nInitialRemainingCapacity +
                              1)) // If there wasn't enough capacity...
    {
    ...
    } else if (nReturnValue < 0) // If vsnprintf is non-C99-standard
    {
        ...
        size_type n = stl::max_alt((size_type)(SSOLayout::SSO_CAPACITY - 1),
                                     (size_type)(nInitialSize * 2));
        ...
        for (; (nReturnValue < 0) && (n < 1000000); n *= 2) {
            ...
            nReturnValue =
                stl::Vsnprintf(internalLayout().BeginPtr() + nInitialSize,
                                 nCapacity + 1, pFormat, arguments);

            if (nReturnValue == (int)(unsigned)nCapacity) {
                resize(++n);
                nReturnValue =
                    stl::Vsnprintf(internalLayout().BeginPtr() + nInitialSize,
                                     nCapacity + 2, pFormat, arguments);
            }
        }
    }
...
template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::append_sprintf(const value_type* pFormat, ...)
{
    va_list arguments;
    va_start(arguments, pFormat);
    append_sprintf_va_list(pFormat, arguments);
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::assign(const this_type& x)
{
    // The C++11 Standard 21.4.6.3 p6 specifies that assign from this_type
    // assigns contents only and not the allocator.
    return assign(x.internalLayout().BeginPtr(), x.internalLayout().EndPtr());
}

template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::assign(const this_type& x, size_type position,
                                   size_type n)
{
#if EASTL_STRING_OPT_RANGE_ERRORS
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::assign(const value_type* p, size_type n)
{
    return assign(p, p + n);
}
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::assign(const value_type* p)
{
    return assign(p, p + CharStrlen(p));
}

template <typename T, typename Allocator>
basic_string<T, Allocator>& basic_string<T, Allocator>::assign(size_type n,
                                                               value_type c)
{
    if (n <= internalLayout().GetSize()) {
        ...
        CharTypeAssignN(internalLayout().BeginPtr(), internalLayout().GetSize(),
                        c);
...
template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::assign(const value_type* pBegin,
                                   const value_type* pEnd)
{
    const size_type n = (size_type)(pEnd - pBegin);
    if (n <= internalLayout().GetSize()) {
        memmove(internalLayout().BeginPtr(), pBegin,
                (size_t)n * sizeof(value_type));
        erase(internalLayout().BeginPtr() + n, internalLayout().EndPtr());
    } else {
        memmove(internalLayout().BeginPtr(), pBegin,
                (size_t)(internalLayout().GetSize()) * sizeof(value_type));
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::assign(std::initializer_list<value_type> ilist)
{
    return assign(ilist.begin(), ilist.end());
}
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::assign(this_type&& x)
{
    if (get_allocator() == x.get_allocator()) {
        stl::swap(internalLayout(), x.internalLayout());
    } else
        assign(x.internalLayout().BeginPtr(), x.internalLayout().EndPtr());
...
template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::insert(size_type position, const this_type& x)
{
#if EASTL_STRING_OPT_RANGE_ERRORS
...
#if EASTL_STRING_OPT_LENGTH_ERRORS
    if (EASTL_UNLIKELY(internalLayout().GetSize() >
                       (max_size() - x.internalLayout().GetSize())))
        ThrowLengthException();
#endif

    insert(internalLayout().BeginPtr() + position,
...
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY((position > internalLayout().GetSize()) ||
                       (beg > x.internalLayout().GetSize())))
        ThrowRangeException();
#endif

    size_type nLength = stl::min_alt(n, x.internalLayout().GetSize() - beg);
...
template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::insert(size_type position, const value_type* p,
                                   size_type n)
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(position > internalLayout().GetSize()))
...
template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::insert(size_type position, size_type n,
                                   value_type c)
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(position > internalLayout().GetSize()))
...
{
    const difference_type nPosition =
        (p -
         internalLayout().BeginPtr()); // Save this because we might reallocate.

#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((p < internalLayout().BeginPtr()) ||
                       (p > internalLayout().EndPtr())))
        EASTL_FAIL_MSG("basic_string::insert -- invalid position");
#endif

    if (n) // If there is anything to insert...
    {
        if (internalLayout().GetRemainingCapacity() >=
            n) // If we have enough capacity...
        {
            const size_type nElementsAfter =
                (size_type)(internalLayout().EndPtr() - p);

            if (nElementsAfter >=
                n) // If there's enough space for the new chars between the
                   // insert position and the end...
            {
                // Ensure we save the size before we do the copy, as we might
                // overwrite the size field with the NULL terminator in the edge
                // case where we are inserting enough characters to equal our
                // capacity
                const size_type nSavedSize = internalLayout().GetSize();
                CharStringUninitializedCopy((internalLayout().EndPtr() - n) + 1,
                                            internalLayout().EndPtr() + 1,
                                            internalLayout().EndPtr() + 1);
                internalLayout().SetSize(nSavedSize + n);
                memmove(const_cast<value_type*>(p) + n, p,
                ...
                const size_type nOldSize = internalLayout().GetSize();
#endif
                CharStringUninitializedFillN(internalLayout().EndPtr() + 1,
                                             n - nElementsAfter - 1, c);
                internalLayout().SetSize(internalLayout().GetSize() +
                                         (n - nElementsAfter));
                ...
                try {
                    ...
                    const size_type nSavedSize = internalLayout().GetSize();
                    CharStringUninitializedCopy(p, pOldEnd + 1,
                                                internalLayout().EndPtr());
                ...
                CharTypeAssignN(const_cast<value_type*>(p), nElementsAfter + 1,
                                c);
            }
        } else {
            const size_type nOldSize = internalLayout().GetSize();
            ...
            const size_type nLength =
                stl::max_alt(GetNewCapacity(nOldCap), nOldSize + n);

            iterator pNewBegin = DoAllocate(nLength + 1);

            iterator pNewEnd = CharStringUninitializedCopy(
                internalLayout().BeginPtr(), p, pNewBegin);
            pNewEnd  = CharStringUninitializedFillN(pNewEnd, n, c);
            pNewEnd  = CharStringUninitializedCopy(p, internalLayout().EndPtr(),
                                                  pNewEnd);
            *pNewEnd = 0;
...
template <typename T, typename Allocator>
typename basic_string<T, Allocator>::iterator
basic_string<T, Allocator>::insert(const_iterator p, const value_type* pBegin,
                                   const value_type* pEnd)
{
    const difference_type nPosition =
        (p -
    ...
    if (n) {
        const bool bCapacityIsSufficient =
            (internalLayout().GetRemainingCapacity() >= n);
        const bool bSourceIsFromSelf = ((pEnd >= internalLayout().BeginPtr()) &&
                                        (pBegin <= internalLayout().EndPtr()));

        if (bSourceIsFromSelf && internalLayout().IsSSO()) {
            // pBegin to pEnd will be <= this->GetSize(), so stackTemp will
            // guaranteed be an SSO String If we are inserting ourself into
            // ourself and we are SSO, then on the recursive call we can
            // guarantee 0 or 1 allocation depending if we need to realloc
            // We don't do this for Heap strings as then this path may do 1 or 2
            // allocations instead of only 1 allocation when we fall through to
            // the last else case below
            const this_type stackTemp(pBegin, pEnd, get_allocator());
            return insert(p, stackTemp.data(),
                          stackTemp.data() + stackTemp.size());
        }

        // If bSourceIsFromSelf is true, then we reallocate. This is because we
        // are inserting ourself into ourself and thus both the source and
        // destination be modified, making it rather tricky to attempt to do in
        // place. The simplest resolution is to reallocate. To consider: there
        // may be a way to implement this whereby we don't need to reallocate or
        // can often avoid reallocating.
        if (bCapacityIsSufficient && !bSourceIsFromSelf) {
            const size_type nElementsAfter =
                (size_type)(internalLayout().EndPtr() - p);

            if (nElementsAfter >=
                n) // If there are enough characters between insert pos and end
            {
                // Ensure we save the size before we do the copy, as we might
                // overwrite the size field with the NULL terminator in the edge
                // case where we are inserting enough characters to equal our
                ...
                memmove(const_cast<value_type*>(p) + n, p,
                        (size_t)((nElementsAfter - n) + 1) *
                            sizeof(value_type));
                memmove(const_cast<value_type*>(p), pBegin,
                        (size_t)(n) * sizeof(value_type));
            } else {
                pointer pOldEnd = internalLayout().EndPtr();
#if EASTL_EXCEPTIONS_ENABLED
                ...
                CharStringUninitializedCopy(pMid, pEnd,
                                            internalLayout().EndPtr() + 1);
                internalLayout().SetSize(internalLayout().GetSize() +
                                         (n - nElementsAfter));
                ...
                CharStringUninitializedCopy(pBegin, pMid,
        ...
        } else // Else we need to reallocate to implement this.
        {
            ...
            if (bCapacityIsSufficient) // If bCapacityIsSufficient is true, then
                                       // bSourceIsFromSelf must be true.
                nLength = nOldSize + n;
            else
                nLength =
                    stl::max_alt(GetNewCapacity(nOldCap), (nOldSize + n));
...
template <typename T, typename Allocator>
typename basic_string<T, Allocator>::iterator
basic_string<T, Allocator>::insert(const_iterator p,
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::erase(size_type position, size_type n)
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(position > internalLayout().GetSize()))
        ThrowRangeException();
    ...
    erase(internalLayout().BeginPtr() + position,
          internalLayout().BeginPtr() + position +
              stl::min_alt(n, internalLayout().GetSize() - position));
...
basic_string<T, Allocator>::erase(const_iterator p)
{
    ...
    memmove(const_cast<value_type*>(p), p + 1,
            (size_t)(internalLayout().EndPtr() - p) * sizeof(value_type));
    internalLayout().SetSize(internalLayout().GetSize() - 1);
...
{
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((pBegin < internalLayout().BeginPtr()) ||
                       (pBegin > internalLayout().EndPtr()) ||
                       (pEnd < internalLayout().BeginPtr()) ||
                       (pEnd > internalLayout().EndPtr()) || (pEnd < pBegin)))
        EASTL_FAIL_MSG("basic_string::erase -- invalid position");
#endif

    if (pBegin != pEnd) {
        memmove(const_cast<value_type*>(pBegin), pEnd,
                (size_t)((internalLayout().EndPtr() - pEnd) + 1) *
                    sizeof(value_type));
        const size_type n = (size_type)(pEnd - pBegin);
...
template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::replace(size_type position, size_type n,
                                    const this_type& x)
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(position > internalLayout().GetSize()))
...
#if EASTL_STRING_OPT_LENGTH_ERRORS
    if (EASTL_UNLIKELY((internalLayout().GetSize() - nLength) >=
                       (max_size() - x.internalLayout().GetSize())))
        ThrowLengthException();
#endif

    return replace(internalLayout().BeginPtr() + position,
                   internalLayout().BeginPtr() + position + nLength,
                   x.internalLayout().BeginPtr(), x.internalLayout().EndPtr());
}

template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::replace(size_type pos1, size_type n1,
                                    const this_type& x, size_type pos2,
                                    size_type n2)
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY((pos1 > internalLayout().GetSize()) ||
                       (pos2 > x.internalLayout().GetSize())))
        ThrowRangeException();
#endif

    const size_type nLength1 =
        stl::min_alt(n1, internalLayout().GetSize() - pos1);
    const size_type nLength2 =
        stl::min_alt(n2, x.internalLayout().GetSize() - pos2);

#if EASTL_STRING_OPT_LENGTH_ERRORS
    if (EASTL_UNLIKELY((internalLayout().GetSize() - nLength1) >=
                       (max_size() - nLength2)))
        ThrowLengthException();
#endif

    return replace(internalLayout().BeginPtr() + pos1,
                   internalLayout().BeginPtr() + pos1 + nLength1,
                   x.internalLayout().BeginPtr() + pos2,
                   x.internalLayout().BeginPtr() + pos2 + nLength2);
}

template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::replace(size_type position, size_type n1,
                                    const value_type* p, size_type n2)
{
    ...
    const size_type nLength =
        stl::min_alt(n1, internalLayout().GetSize() - position);

#if EASTL_STRING_OPT_LENGTH_ERRORS
    if (EASTL_UNLIKELY((n2 > max_size()) || ((internalLayout().GetSize() -
                                              nLength) >= (max_size() - n2))))
        ThrowLengthException();
#endif

    return replace(internalLayout().BeginPtr() + position,
                   internalLayout().BeginPtr() + position + nLength, p, p + n2);
}
                                    ...
                                    size_type n2, value_type c)
{
    ...
    return replace(internalLayout().BeginPtr() + position,
                   internalLayout().BeginPtr() + position + nLength, n2, c);
}

template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::replace(const_iterator pBegin, const_iterator pEnd,
                                    const this_type& x)
{
...
template <typename T, typename Allocator>
basic_string<T, Allocator>& basic_string<T, Allocator>::replace(
    const_iterator pBegin1, const_iterator pEnd1, const value_type* pBegin2,
    const value_type* pEnd2)
{
#if EASTL_ASSERT_ENABLED
    if (EASTL_UNLIKELY((pBegin1 < internalLayout().BeginPtr()) ||
                       (pBegin1 > internalLayout().EndPtr()) ||
                       (pEnd1 < internalLayout().BeginPtr()) ||
                       (pEnd1 > internalLayout().EndPtr()) ||
                       (pEnd1 < pBegin1)))
        EASTL_FAIL_MSG("basic_string::replace -- invalid position");
#endif
    ...
    {
        if ((pBegin2 > pEnd1) ||
            (pEnd2 <= pBegin1)) // If we have a non-overlapping operation...
            memcpy(const_cast<value_type*>(pBegin1), pBegin2,
                   (size_t)(pEnd2 - pBegin2) * sizeof(value_type));
        else
            memmove(const_cast<value_type*>(pBegin1), pBegin2,
                    (size_t)(pEnd2 - pBegin2) * sizeof(value_type));
    ...
    } else // Else we are expanding.
    {
            ...
            (pEnd2 <= pBegin1)) // If we have a non-overlapping operation...
        {
            ...
            if ((pEnd2 <= pBegin1) || (pBegin2 > pEnd1))
                memcpy(const_cast<value_type*>(pBegin1), pBegin2,
                       (size_t)(pMid2 - pBegin2) * sizeof(value_type));
            else
                memmove(const_cast<value_type*>(pBegin1), pBegin2,
                        (size_t)(pMid2 - pBegin2) * sizeof(value_type));
        ...
        {
            // I can't think of any easy way of doing this without allocating
            // temporary memory.
            const size_type nOldSize     = internalLayout().GetSize();
            const size_type nOldCap      = capacity();
            const size_type nNewCapacity = stl::max_alt(
                GetNewCapacity(nOldCap), (nOldSize + (nLength2 - nLength1)));

            pointer pNewBegin = DoAllocate(nNewCapacity + 1);

            pointer pNewEnd = CharStringUninitializedCopy(
                internalLayout().BeginPtr(), pBegin1, pNewBegin);
            pNewEnd = CharStringUninitializedCopy(pBegin2, pEnd2, pNewEnd);
            pNewEnd = CharStringUninitializedCopy(
                pEnd1, internalLayout().EndPtr(), pNewEnd);
            *pNewEnd = 0;
...
basic_string<T, Allocator>::copy(value_type* p, size_type n,
                                 size_type position) const
{
    ...
    // C++ std says the effects of this function are as if calling
    // char_traits::copy() thus the 'p' must not overlap *this string, so we can
    // use memcpy
    const size_type nLength =
        stl::min_alt(n, internalLayout().GetSize() - position);
...
{
    if (get_allocator() == x.get_allocator() ||
        (internalLayout().IsSSO() &&
         x.internalLayout().IsSSO())) // If allocators are equivalent...
    {
        // We leave mAllocator as-is.
        stl::swap(internalLayout(), x.internalLayout());
    } else // else swap the contents.
    {
        const this_type temp(
            *this); // Can't call stl::swap because that would
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find(const this_type& x,
                                 size_type position) const EA_NOEXCEPT
{
    return find(x.internalLayout().BeginPtr(), position,
...
template <typename T, typename Allocator>
typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find(const value_type* p, size_type position,
                                 size_type n) const
{
    // It is not clear what the requirements are for position, but since the C++
    // standard appears to be silent it is assumed for now that position can be
    // any value.
    //#if EASTL_ASSERT_ENABLED
    ...
    if (EASTL_LIKELY(
            ((npos - n) >= position) &&
            (position + n) <=
                internalLayout().GetSize())) // If the range is valid...
    {
        const value_type* const pTemp =
            stl::search(internalLayout().BeginPtr() + position,
...
template <typename T, typename Allocator>
typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find(value_type c,
                                 size_type position) const EA_NOEXCEPT
{
    // It is not clear what the requirements are for position, but since the C++
    // standard appears to be silent it is assumed for now that position can be
    ...
    if (EASTL_LIKELY(position <
                     internalLayout().GetSize())) // If the position is valid...
    {
        const const_iterator pResult =
            stl::find(internalLayout().BeginPtr() + position,
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::rfind(const this_type& x,
                                  size_type position) const EA_NOEXCEPT
{
    return rfind(x.internalLayout().BeginPtr(), position,
                 x.internalLayout().GetSize());
}
...
template <typename T, typename Allocator>
typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::rfind(const value_type* p, size_type position,
                                  size_type n) const
{
    ...
    //#if EASTL_ASSERT_ENABLED
    //    if(EASTL_UNLIKELY((position != npos) && (position > (size_type)(mpEnd
    //    - mpBegin))))
    //        EASTL_FAIL_MSG("basic_string::rfind -- invalid position");
    //#endif

    // Note that a search for a zero length string starting at position = end()
    // returns end() and not npos. Note by Paul Pedriana: I am not sure how this
    // should behave in the case of n == 0 and position > size. The standard
    // seems to suggest that rfind doesn't act exactly the same as find in that
    // input position can be > size and the return value can still be other than
    // npos. Thus, if n == 0 then you can never return npos, unlike the case
    // with find.
    const size_type nLength = internalLayout().GetSize();

    if (EASTL_LIKELY(n <= nLength)) {
        if (EASTL_LIKELY(n)) {
            const const_iterator pEnd = internalLayout().BeginPtr() +
                                        stl::min_alt(nLength - n, position) +
                                        n;
            const const_iterator pResult = CharTypeStringRSearch(
                internalLayout().BeginPtr(), pEnd, p, p + n);

            if (pResult != pEnd)
                return (size_type)(pResult - internalLayout().BeginPtr());
        } else
            return stl::min_alt(nLength, position);
    }
    return npos;
}
...
template <typename T, typename Allocator>
typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::rfind(value_type c,
                                  size_type position) const EA_NOEXCEPT
{
    ...
    if (EASTL_LIKELY(nLength)) {
        const value_type* const pEnd = internalLayout().BeginPtr() +
                                       stl::min_alt(nLength - 1, position) +
                                       1;
        const value_type* const pResult =
            CharTypeStringRFind(pEnd, internalLayout().BeginPtr(), c);
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_first_of(const this_type& x,
                                          size_type position) const EA_NOEXCEPT
{
    return find_first_of(x.internalLayout().BeginPtr(), position,
                         x.internalLayout().GetSize());
}

template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_first_of(const value_type* p,
                                          size_type position) const
{
                                          ...
                                          size_type position, size_type n) const
{
    ...
    if (EASTL_LIKELY((position < internalLayout().GetSize()))) {
        ...
        const const_iterator pResult   = CharTypeStringFindFirstOf(
            pBegin, internalLayout().EndPtr(), p, p + n);
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_first_of(value_type c,
                                          size_type position) const EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_last_of(const this_type& x,
                                         size_type position) const EA_NOEXCEPT
{
    return find_last_of(x.internalLayout().BeginPtr(), position,
                        x.internalLayout().GetSize());
}

template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_last_of(const value_type* p,
                                         size_type position) const
{
                                         ...
                                         size_type position, size_type n) const
{
    ...
    if (EASTL_LIKELY(nLength)) {
        const value_type* const pEnd = internalLayout().BeginPtr() +
                                       stl::min_alt(nLength - 1, position) +
                                       1;
        const value_type* const pResult = CharTypeStringRFindFirstOf(
            pEnd, internalLayout().BeginPtr(), p, p + n);
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_last_of(value_type c,
                                         size_type position) const EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_first_not_of(
    const this_type& x, size_type position) const EA_NOEXCEPT
{
    return find_first_not_of(x.internalLayout().BeginPtr(), position,
                             x.internalLayout().GetSize());
}

template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_first_not_of(const value_type* p,
                                              size_type position) const
{
                                              ...
                                              size_type n) const
{
    if (EASTL_LIKELY(position <= internalLayout().GetSize())) {
        const const_iterator pResult =
            CharTypeStringFindFirstNotOf(internalLayout().BeginPtr() + position,
                                         internalLayout().EndPtr(), p, p + n);
...
typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_first_not_of(
    value_type c, size_type position) const EA_NOEXCEPT
{
    if (EASTL_LIKELY(position <= internalLayout().GetSize())) {
        // Todo: Possibly make a specialized version of
        // CharTypeStringFindFirstNotOf(pBegin, pEnd, c).
        const const_iterator pResult =
            CharTypeStringFindFirstNotOf(internalLayout().BeginPtr() + position,
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_last_not_of(
    const this_type& x, size_type position) const EA_NOEXCEPT
{
    return find_last_not_of(x.internalLayout().BeginPtr(), position,
                            x.internalLayout().GetSize());
}

template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::find_last_not_of(const value_type* p,
                                             size_type position) const
{
                                             ...
                                             size_type n) const
{
    ...
    if (EASTL_LIKELY(nLength)) {
        const value_type* const pEnd = internalLayout().BeginPtr() +
                                       stl::min_alt(nLength - 1, position) +
                                       1;
        const value_type* const pResult = CharTypeStringRFindFirstNotOf(
            pEnd, internalLayout().BeginPtr(), p, p + n);
    ...
    value_type c, size_type position) const EA_NOEXCEPT
{
    ...
    if (EASTL_LIKELY(nLength)) {
        // Todo: Possibly make a specialized version of
        // CharTypeStringRFindFirstNotOf(pBegin, pEnd, c).
        const value_type* const pEnd = internalLayout().BeginPtr() +
                                       stl::min_alt(nLength - 1, position) +
                                       1;
        const value_type* const pResult = CharTypeStringRFindFirstNotOf(
            pEnd, internalLayout().BeginPtr(), &c, &c + 1);
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>
basic_string<T, Allocator>::substr(size_type position, size_type n) const
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(position > internalLayout().GetSize()))
        ThrowRangeException();
    ...
    // C++ std says the return string allocator must be default constructed, not
    // a copy of this->get_allocator()
    return basic_string(
        internalLayout().BeginPtr() + position,
        internalLayout().BeginPtr() + position +
...
template <typename T, typename Allocator>
inline int
basic_string<T, Allocator>::compare(const this_type& x) const EA_NOEXCEPT
{
    return compare(internalLayout().BeginPtr(), internalLayout().EndPtr(),
                   x.internalLayout().BeginPtr(), x.internalLayout().EndPtr());
}

template <typename T, typename Allocator>
inline int basic_string<T, Allocator>::compare(size_type pos1, size_type n1,
                                               const this_type& x) const
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY(pos1 > internalLayout().GetSize()))
        ThrowRangeException();
...
{
#if EASTL_STRING_OPT_RANGE_ERRORS
    if (EASTL_UNLIKELY((pos1 > (size_type)(internalLayout().EndPtr() -
                                           internalLayout().BeginPtr())) ||
                       (pos2 > (size_type)(x.internalLayout().EndPtr() -
...
{
    return compare(internalLayout().BeginPtr(), internalLayout().EndPtr(), p,
                   p + CharStrlen(p));
}
...
{
    for (pointer p = internalLayout().BeginPtr(); p < internalLayout().EndPtr();
         ++p)
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>
basic_string<T, Allocator>::left(size_type n) const
{
    const size_type nLength = length();
    if (n < nLength)
        return substr(0, n);
    // C++ std says that substr must return default constructed allocated, but
    // we do not. Instead it is much more practical to provide the copy of the
    // current allocator
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>
basic_string<T, Allocator>::right(size_type n) const
{
    const size_type nLength = length();
    if (n < nLength)
...
template <typename T, typename Allocator>
inline basic_string<T, Allocator>&
basic_string<T, Allocator>::sprintf(const value_type* pFormat, ...)
{
    va_list arguments;
    va_start(arguments, pFormat);
...
template <typename T, typename Allocator>
basic_string<T, Allocator>&
basic_string<T, Allocator>::sprintf_va_list(const value_type* pFormat,
                                            va_list arguments)
{
    internalLayout().SetSize(0); // Fast truncate to zero length.
                                        ...
                                        const value_type* pEnd2)
{
    ...
    const difference_type nMin = stl::min_alt(n1, n2);
...
template <typename T, typename Allocator>
inline int
basic_string<T, Allocator>::comparei(const this_type& x) const EA_NOEXCEPT
{
    return comparei(internalLayout().BeginPtr(), internalLayout().EndPtr(),
                    x.internalLayout().BeginPtr(), x.internalLayout().EndPtr());
}
...
{
    return comparei(internalLayout().BeginPtr(), internalLayout().EndPtr(), p,
                    p + CharStrlen(p));
}
...
basic_string<T, Allocator>::InsertInternal(const_iterator p, value_type c)
{
    ...
    if ((internalLayout().EndPtr() + 1) <= internalLayout().CapacityPtr()) {
        const size_type nSavedSize = internalLayout().GetSize();
        memmove(const_cast<value_type*>(p) + 1, p,
                (size_t)(internalLayout().EndPtr() - p) * sizeof(value_type));
        ...
        *pNewPosition                    = c;
        ...
        const size_type nOldCap  = capacity();
        const size_type nLength =
            stl::max_alt(GetNewCapacity(nOldCap), (nOldSize + 1));

        iterator pNewBegin = DoAllocate(nLength + 1);

        pNewPosition  = CharStringUninitializedCopy(internalLayout().BeginPtr(),
                                                   p, pNewBegin);
        *pNewPosition = c;

        iterator pNewEnd = pNewPosition + 1;
        pNewEnd =
            CharStringUninitializedCopy(p, internalLayout().EndPtr(), pNewEnd);
        *pNewEnd = 0;
...
template <typename T, typename Allocator>
void basic_string<T, Allocator>::RangeInitialize(const value_type* pBegin,
                                                 const value_type* pEnd)
{
...
template <typename T, typename Allocator>
inline void
basic_string<T, Allocator>::RangeInitialize(const value_type* pBegin)
{
#if EASTL_STRING_OPT_ARGUMENT_ERRORS
...
template <typename T, typename Allocator>
inline typename basic_string<T, Allocator>::size_type
basic_string<T, Allocator>::GetNewCapacity(
    size_type currentCapacity) // This needs to return a value of at least
                               // currentCapacity and at least 1.
{
    return (currentCapacity <= SSOLayout::SSO_CAPACITY)
...
inline void basic_string<T, Allocator>::DeallocateSelf()
{
    if (internalLayout().IsHeap()) {
        DoFree(internalLayout().BeginPtr(),
               internalLayout().GetHeapCapacity() + 1);
    }
}
...
basic_string<T, Allocator>::CharTypeStringFindEnd(const value_type* pBegin,
                                                  const value_type* pEnd,
                                                  value_type c)
{
...
template <typename T, typename Allocator>
const typename basic_string<T, Allocator>::value_type*
basic_string<T, Allocator>::CharTypeStringRFind(const value_type* pRBegin,
                                                const value_type* pREnd,
                                                const value_type c)
{
    while (pRBegin > pREnd) {
...
// Purpose: find p2 within p1. Return p1End if not found or if either string is
// zero length.
template <typename T, typename Allocator>
const typename basic_string<T, Allocator>::value_type*
basic_string<T, Allocator>::CharTypeStringSearch(const value_type* p1Begin,
                                                 const value_type* p1End,
                                                 const value_type* p2Begin,
                                                 const value_type* p2End)
{
    ...
    if ((p2Begin + 1) == p2End)
        return stl::find(p1Begin, p1End, *p2Begin);
    ...
    while (p1Begin != p1End) {
        p1Begin = stl::find(p1Begin, p1End, *p2Begin);
        if (p1Begin == p1End)
            return p1End;
...
// Specialized value_type version of STL find_end() function (which really is a
// reverse search function). Purpose: find last instance of p2 within p1. Return
// p1End if not found or if either string is zero length.
template <typename T, typename Allocator>
const typename basic_string<T, Allocator>::value_type*
basic_string<T, Allocator>::CharTypeStringRSearch(const value_type* p1Begin,
                                                  const value_type* p1End,
                                                  const value_type* p2Begin,
                                                  const value_type* p2End)
{
...
// This function is much like the C runtime strtok function, except the strings
// aren't null-terminated.
template <typename T, typename Allocator>
const typename basic_string<T, Allocator>::value_type*
basic_string<T, Allocator>::CharTypeStringFindFirstOf(const value_type* p1Begin,
                                                      const value_type* p1End,
                                                      const value_type* p2Begin,
                                                      const value_type* p2End)
{
...
template <typename T, typename Allocator>
const typename basic_string<T, Allocator>::value_type*
basic_string<T, Allocator>::CharTypeStringRFindFirstOf(
    const value_type* p1RBegin, const value_type* p1REnd,
    const value_type* p2Begin, const value_type* p2End)
{
...
template <typename T, typename Allocator>
const typename basic_string<T, Allocator>::value_type*
basic_string<T, Allocator>::CharTypeStringFindFirstNotOf(
    const value_type* p1Begin, const value_type* p1End,
    const value_type* p2Begin, const value_type* p2End)
{
    for (; p1Begin != p1End; ++p1Begin) {
...
// Specialized value_type version of STL find_first_not_of() function in
// reverse.
template <typename T, typename Allocator>
const typename basic_string<T, Allocator>::value_type*
basic_string<T, Allocator>::CharTypeStringRFindFirstNotOf(
    const value_type* p1RBegin, const value_type* p1REnd,
    const value_type* p2Begin, const value_type* p2End)
{
    for (; p1RBegin != p1REnd; --p1RBegin) {
...
template <typename T, typename Allocator>
inline bool
operator==(const typename basic_string<T, Allocator>::reverse_iterator& r1,
...
{
    typedef typename basic_string<T, Allocator>::CtorDoNotInitialize
        CtorDoNotInitialize;
    CtorDoNotInitialize
        cDNI; // GCC 2.x forces us to declare a named temporary like this.
    basic_string<T, Allocator> result(
        cDNI, a.size() + b.size(),
        const_cast<basic_string<T, Allocator>&>(a)
            .get_allocator()); // Note that we choose to assign a's allocator.
    result.append(a);
    result.append(b);
    return result;
}
...
template <typename T, typename Allocator>
basic_string<T, Allocator>
operator+(const typename basic_string<T, Allocator>::value_type* p,
          const basic_string<T, Allocator>& b)
{
    typedef typename basic_string<T, Allocator>::CtorDoNotInitialize
        CtorDoNotInitialize;
...
template <typename T, typename Allocator>
basic_string<T, Allocator>
operator+(typename basic_string<T, Allocator>::value_type c,
          const basic_string<T, Allocator>& b)
{
    typedef typename basic_string<T, Allocator>::CtorDoNotInitialize
        CtorDoNotInitialize;
...
template <typename T, typename Allocator>
basic_string<T, Allocator>
operator+(const basic_string<T, Allocator>& a,
...
template <typename T, typename Allocator>
basic_string<T, Allocator>
operator+(basic_string<T, Allocator>&& a,
          const typename basic_string<T, Allocator>::value_type* p)
{
    a.append(p);
    return stl::move(a);
...
{
    if ((internalLayout().BeginPtr() == nullptr) ||
        (internalLayout().EndPtr() == nullptr))
        return false;
                       ...
                       const basic_string<T, Allocator>& b)
{
    return (
        ...
        (memcmp(a.data(), b.data(),
                (size_t)a.size() *
                    sizeof(typename basic_string<T, Allocator>::value_type)) ==
         0));
}
                       ...
                       const basic_string<T, Allocator>& b)
{
    ...
    return ((n == b.size()) &&
            (memcmp(p, b.data(), (size_t)n * sizeof(*p)) == 0));
}
                       ...
                       const typename basic_string<T, Allocator>::value_type* p)
{
    ...
    const size_type n = (size_type)CharStrlen(p);
    return ((a.size() == n) &&
            (memcmp(a.data(), p, (size_t)n * sizeof(*p)) == 0));
}
...
inline bool operator<(const basic_string<T, Allocator>& a,
                      const basic_string<T, Allocator>& b)
{
    return basic_string<T, Allocator>::compare(a.begin(), a.end(), b.begin(),
                                               b.end()) < 0;
}
                      ...
                      const basic_string<T, Allocator>& b)
{
    typedef typename basic_string<T, Allocator>::size_type size_type;
    const size_type n = (size_type)CharStrlen(p);
    return basic_string<T, Allocator>::compare(p, p + n, b.begin(), b.end()) <
           0;
}
                      ...
                      const typename basic_string<T, Allocator>::value_type* p)
{
    typedef typename basic_string<T, Allocator>::size_type size_type;
    const size_type n = (size_type)CharStrlen(p);
    return basic_string<T, Allocator>::compare(a.begin(), a.end(), p, p + n) <
           0;
}
...
typedef basic_string<char8_t>
    u8string; // Actually not a C++11 type, but added for consistency.
...
struct hash<string> {
    size_t operator()(const string& x) const
    {
        const unsigned char* p =
            (const unsigned char*)
                x.c_str(); // To consider: limit p to at most 256 chars.
        unsigned int c,
            result = 2166136261U; // We implement an FNV-like string hash.
...
/// to_string
///
/// Converts integral types to an stl::string with the same content that
/// sprintf produces.  The following implementation provides a type safe
/// conversion mechanism which avoids the common bugs associated with sprintf
...
/// to_wstring
///
/// Converts integral types to an stl::wstring with the same content that
/// sprintf produces.  The following implementation provides a type safe
/// conversion mechanism which avoids the common bugs associated with sprintf
/// style format strings.
///
...
EA_DISABLE_VC_WARNING(4455) // disable warning C4455: literal suffix identifiers
                            // that do not start with an underscore are reserved
