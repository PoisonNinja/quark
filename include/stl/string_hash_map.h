...
class string_hash_map : public stl::hash_map<const char*, T, Hash, Predicate, Allocator>
{
public:
	typedef stl::hash_map<const char*, T, Hash, Predicate, Allocator> base;
...
string_hash_map<T, Hash, Predicate, Allocator>::string_hash_map(const string_hash_map& src, const allocator_type& allocator) : base(allocator)
{
	for (const_iterator i=src.begin(), e=src.end(); i!=e; ++i)
		base::base_type::insert(stl::make_pair(strduplicate(i->first), i->second));
}
...
string_hash_map<T, Hash, Predicate, Allocator>::insert(const char* key, const T& value)
{
	...
	return base::base_type::insert(stl::make_pair(strduplicate(key), value));
}
