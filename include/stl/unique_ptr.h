...
namespace stl
{
	...
	public:
		...
		EA_CPP14_CONSTEXPR unique_ptr() EA_NOEXCEPT
			: mPair(pointer())
		{
			static_assert(!stl::is_pointer<deleter_type>::value, "unique_ptr deleter default-constructed with null pointer. Use a different constructor or change your deleter to a class.");
		}
		...
		unique_ptr(pointer pValue, typename stl::conditional<stl::is_reference<deleter_type>::value, deleter_type, typename stl::add_lvalue_reference<const deleter_type>::type>::type deleter) EA_NOEXCEPT
		...
		unique_ptr(pointer pValue, typename stl::remove_reference<deleter_type>::type&& deleter) EA_NOEXCEPT
			: mPair(pValue, stl::move(deleter))
		{
			static_assert(!stl::is_reference<deleter_type>::value, "deleter_type reference refers to an rvalue deleter. The reference will probably become invalid before used. Change the deleter_type to not be a reference or construct with permanent deleter.");
		}
		...
		///     ptr = stl::move(newPtr);  // Deletes int(3) and assigns mpValue to int(4)
	...
	public:
		...
		explicit unique_ptr(P pArray, typename stl::enable_if<Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0) EA_NOEXCEPT
			: mPair(pArray)
		{
			static_assert(!stl::is_pointer<deleter_type>::value, "unique_ptr deleter default-constructed with null pointer. Use a different constructor or change your deleter to a class.");
		}
		...
		template <typename P>
		unique_ptr(P pArray, typename stl::conditional<stl::is_reference<deleter_type>::value, deleter_type,
														typename stl::add_lvalue_reference<const deleter_type>::type>::type deleter,
														typename stl::enable_if<Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0) EA_NOEXCEPT
			: mPair(pArray, deleter) {}

		template <typename P>
		unique_ptr(P pArray, typename stl::remove_reference<deleter_type>::type&& deleter, typename stl::enable_if<Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0) EA_NOEXCEPT
			: mPair(pArray, stl::move(deleter))
		{
			static_assert(!stl::is_reference<deleter_type>::value, "deleter_type reference refers to an rvalue deleter. The reference will probably become invalid before used. Change the deleter_type to not be a reference or construct with permanent deleter.");
		}
		...
		unique_ptr(unique_ptr<U, E>&& u, typename enable_if<Internal::is_safe_array_conversion<T, pointer, U, typename unique_ptr<U, E>::pointer>::value && 
														   ...
														   (!stl::is_reference<deleter_type>::value || stl::is_same<E, deleter_type>::value)>::type* = 0) EA_NOEXCEPT
			: mPair(u.release(), stl::forward<E>(u.get_deleter())) {}
	...
	/// It simply returns stl::hash(x.get()). If your unique_ptr pointer type (the return value of unique_ptr<T>::get) is 
	...
	inline bool operator<(const unique_ptr<T1, D1>& a, const unique_ptr<T2, D2>& b)
	{
		//typedef typename stl::unique_ptr<T1, D1>::pointer P1;       // We currently need to make these temporary variables, as otherwise clang complains about CPointer being int*&&&.
		//typedef typename stl::unique_ptr<T2, D2>::pointer P2;       // I think there's something wrong with our common_type type trait implementation.
		//typedef typename stl::common_type<P1, P2>::type   PCommon;  // "in instantiation of function template specialization 'stl::operator<<int, int>, no known conversion from 'element_type *' (aka 'int *') to 'int *&&&' for 1st argument"
