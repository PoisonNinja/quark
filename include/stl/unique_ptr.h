...
#include <stddef.h>
#include <stl/EABase/nullptr.h>
#include <stl/bonus/compressed_pair.h>
#include <stl/functional.h>
#include <stl/internal/config.h>
#include <stl/internal/move_help.h> // Defines EASTL_MOVE
#include <stl/internal/smart_ptr.h> // Defines smart_ptr_deleter
...
/// a deleter which frees memory in a way that matches how it was originally
/// allocated. Deleters have instance information and are moved between
/// containers the same way the allocated pointers are. Thus you can allocate
/// memory via some heap and provide a deleter which contains a pointer to that
/// same heap, and regardless of what you do with the unique_ptr, including
/// moving it to another unique_ptr, the deletion will use the originally
/// provided heap.
///
/// Example usage:
...
/// Here is a table which documents several members of shared_ptr and unique_ptr
/// with respect to completeness requirements. If the member requires a complete
/// type, the entry has a "C", otherwise the table entry is filled with "I".
///
...
///     +------------------------+---------------+---------------+
///
template <typename T, typename Deleter = stl::default_delete<T>>
class unique_ptr
{
public:
    ...
    typedef
        typename Internal::unique_pointer_type<element_type, deleter_type>::type
            pointer;
...
public:
    ...
    {
        static_assert(
            !stl::is_pointer<deleter_type>::value,
            "unique_ptr deleter default-constructed with null pointer. Use a "
            "different constructor or change your deleter to a class.");
    }
    ...
    /// Example usage:
    ///     stl::smart_ptr_deleter<int> del;
    ///     unique_ptr<int> ptr(new int(3), del);
    unique_ptr(
        pointer pValue,
        typename stl::conditional<
            stl::is_reference<deleter_type>::value, deleter_type,
            typename stl::add_lvalue_reference<const deleter_type>::type>::type
            deleter) EA_NOEXCEPT : mPair(pValue, deleter)
    {
    }

    /// unique_ptr
    /// Constructs a unique_ptr with the owned pointer and deleter specified
    /// (rvalue) Example usage:
    ///     unique_ptr<int> ptr(new int(3), stl::smart_ptr_deleter<int>());
    unique_ptr(pointer pValue,
               typename stl::remove_reference<deleter_type>::type&& deleter)
        EA_NOEXCEPT : mPair(pValue, stl::move(deleter))
    {
        static_assert(
            !stl::is_reference<deleter_type>::value,
            "deleter_type reference refers to an rvalue deleter. The reference "
            "will probably become invalid before used. Change the deleter_type "
            "to not be a reference or construct with permanent deleter.");
    }
    ...
    /// Example usage:
    ///     unique_ptr<int> ptr(new int(3));
    ///     unique_ptr<int> newPtr = stl::move(ptr);
    unique_ptr(this_type&& x) EA_NOEXCEPT
        : mPair(x.release(), stl::forward<deleter_type>(x.get_deleter()))
    {
    }
    ...
    unique_ptr(
        unique_ptr<U, E>&& u,
        typename enable_if<!is_array<U>::value &&
                           is_convertible<typename unique_ptr<U, E>::pointer,
                                          pointer>::value &&
                           is_convertible<E, deleter_type>::value &&
                           (is_same<deleter_type, E>::value ||
                            !is_reference<deleter_type>::value)>::type* = 0)
        EA_NOEXCEPT : mPair(u.release(), stl::forward<E>(u.get_deleter()))
    {
    }
    ...
    ///     ptr = stl::move(newPtr);  // Deletes int(3) and assigns mpValue to
    ///     int(4)
    this_type& operator=(this_type&& x) EA_NOEXCEPT
    {
        reset(x.release());
        mPair.second() = stl::move(stl::forward<deleter_type>(x.get_deleter()));
        return *this;
    }
    ...
    {
        reset(u.release());
        mPair.second() = stl::move(stl::forward<E>(u.get_deleter()));
        return *this;
    }
    ...
    operator*() const // Not noexcept, because the pointer may be NULL.
    {
        return *mPair.first();
    }
    ...
    /// Example usage:
    /// unique_ptr<int> ptr(new int(3));
    /// stl::smart_ptr_deleter<int>& del = ptr.get_deleter();
...
protected:
    stl::compressed_pair<pointer, deleter_type> mPair;
...
///
/// It's not possible to create a unique_ptr for arrays of a known bound (e.g.
/// int[4] as opposed to int[]).
///
/// Example usage:
...
public:
    ...
    template <typename P> // Pointers to types derived from T are rejected by
                          ...
                          explicit unique_ptr(
                              P pArray, typename stl::enable_if<
                                            Internal::is_array_cv_convertible<
                                                P, pointer>::value>::type* = 0)
                              EA_NOEXCEPT : mPair(pArray)
    {
        static_assert(
            !stl::is_pointer<deleter_type>::value,
            "unique_ptr deleter default-constructed with null pointer. Use a "
    ...
    unique_ptr(
        ...
        typename stl::conditional<
            ...
            typename stl::add_lvalue_reference<const deleter_type>::type>::type
            deleter,
        typename stl::enable_if<
            Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0)
        EA_NOEXCEPT : mPair(pArray, deleter)
    {
    }

    template <typename P>
    unique_ptr(
        P pArray, typename stl::remove_reference<deleter_type>::type&& deleter,
        typename stl::enable_if<
            Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0)
        EA_NOEXCEPT : mPair(pArray, stl::move(deleter))
    {
        static_assert(
            !stl::is_reference<deleter_type>::value,
            "deleter_type reference refers to an rvalue deleter. The reference "
    ...
    unique_ptr(
        unique_ptr<U, E>&& u,
        typename enable_if<
            Internal::is_safe_array_conversion<
                T, pointer, U, typename unique_ptr<U, E>::pointer>::value &&
            stl::is_convertible<E, deleter_type>::value &&
            (!stl::is_reference<deleter_type>::value ||
             stl::is_same<E, deleter_type>::value)>::type* = 0) EA_NOEXCEPT
        : mPair(u.release(), stl::forward<E>(u.get_deleter()))
    {
    }
...
/// make_unique
///
/// The C++11 Standard doesn't have make_unique, but there's no agreed reason as
/// to why.
/// http://stackoverflow.com/questions/12580432/why-does-c11-have-make-shared-but-not-make-unique
/// http://herbsutter.com/2013/05/29/gotw-89-solution-smart-pointers/
/// Herb's solution is OK but doesn't support unique_ptr<[]> (array version). We
/// do the same thing libc++ does and make a specialization of make_unique for
/// arrays.
///
/// make_unique has two cases where you can't use it and need to directly use
/// unique_ptr:
///     - You need to construct the unique_ptr with a raw pointer.
///     - You need to specify a custom deleter.
///
/// Note: This function uses global new T by default to create the ptr instance,
/// as per the C++11 Standard make_shared_ptr.
///
/// Example usage:
...
inline typename Internal::unique_type<T>::unique_type_single
...
template <typename T>
inline typename Internal::unique_type<T>::unique_type_unbounded_array
make_unique(size_t n)
{
    typedef typename stl::remove_extent<T>::type TBase;
...
/// It simply returns stl::hash(x.get()). If your unique_ptr pointer type (the
/// return value of unique_ptr<T>::get) is a custom type and not a built-in
/// pointer type then you will need to independently define stl::hash for that
/// type.
template <typename T, typename D>
struct hash<unique_ptr<T, D>> {
    size_t operator()(const unique_ptr<T, D>& x) const EA_NOEXCEPT
    {
        return stl::hash<typename unique_ptr<T, D>::pointer>()(x.get());
    }
};
...
inline bool operator<(const unique_ptr<T1, D1>& a, const unique_ptr<T2, D2>& b)
{
    // typedef typename stl::unique_ptr<T1, D1>::pointer P1;       // We
    // currently need to make these temporary variables, as otherwise clang
    // complains about CPointer being int*&&&. typedef typename
    // stl::unique_ptr<T2, D2>::pointer P2;       // I think there's something
    // wrong with our common_type type trait implementation. typedef typename
    // stl::common_type<P1, P2>::type   PCommon;  // "in instantiation of
    // function template specialization 'stl::operator<<int, int>, no known
    // conversion from 'element_type *' (aka 'int *') to 'int *&&&' for 1st
    // argument" return less<PCommon>()(a.get(), b.get());                     //
    // It looks like common_type is making CPointer be (e.g.) int*&& instead of
    // int*, though the problem may be in how less<> deals with that.

    typedef typename stl::unique_ptr<T1, D1>::pointer P1;
    typedef typename stl::unique_ptr<T2, D2>::pointer P2;
    typedef typename stl::common_type<P1, P2>::type PCommon;
    PCommon pT1 = a.get();
    PCommon pT2 = b.get();
