...
//
// Given defect 2901, the stl::variant implementation does not provide the
...
#include <stl/tuple.h>
	...
	static_assert(false, "stl::variant requires a C++14 compatible compiler (at least) ");
#endif
...
namespace stl
{
	...
	//
	template<typename T, bool = stl::is_default_constructible_v<T>>
	...
	//
		...
		{
			bad_variant_access() : std::logic_error("stl::bad_variant_access exception") {}
	...
	inline void CheckVariantCondition(bool b)
	{
		...
		EASTL_ASSERT_MSG(b, "stl::bad_variant_access assert");
	#endif
	}
	...
	public:
		...
		{
			DoOp(StorageOp::MOVE, stl::move(other)); 
			return *this;
		}
		...
		{
			// NOTE(rparolin): If this assert fires there is an stl problem picking the size of the local buffer which
			...
			new (&mBuffer) RT(stl::forward<Args>(args)...);
		...
		void set_as(std::initializer_list<U> il, Args&&... args)
		{
			...
			new (&mBuffer) RT(il, stl::forward<Args>(args)...);
		...
		T get_as()
		{
			static_assert(stl::is_pointer_v<T>, "T must be a pointer type");
	...
	EA_CONSTEXPR variant_alternative_t<I, variant<Types...>>&& get(variant<Types...>&& v)
	{
		...
		EASTL_ASSERT(v.index() == I);
		return stl::move(*v.mStorage.template get_as<return_type>());
	}
	...
	public:
			...
			: mIndex(variant_npos), mStorage()
		{
			...
			{
				mIndex = other.mIndex;
				mStorage = stl::move(other.mStorage);
			}
		}
		   ...
		   : mIndex(variant_npos), mStorage()
		{
			...
			mStorage.template set_as<T_j>(stl::forward<T_j>(t));
		}
		...
		{
			return emplace<I>(stl::forward<Args>(args)...);
		}
		...
		{
			return emplace<I>(il, stl::forward<T>(args)...);
		}
		...
		variant_alternative_t<I, variant>& emplace(Args&&... args)
		{
			...
			mIndex = static_cast<variant_index_t>(I);
			mStorage.template set_as<T>(stl::forward<Args>(args)...);
		...
		variant_alternative_t<I, variant>& emplace(std::initializer_list<U> il, Args&&... args)
		{
			...
			mIndex = static_cast<variant_index_t>(I);
			mStorage.template set_as<T>(il, stl::forward<Args>(args)...);
			return *reinterpret_cast<T*>(&mStorage.mBuffer);
		}
		...
		template <class T,
		          ...
		          typename = enable_if_t<!stl::is_same_v<decay_t<T>, variant> && stl::is_assignable_v<T_j&, T> &&
		                                 stl::is_constructible_v<T_j, T>>>
		...
		{
			stl::swap(mIndex, other.mIndex);
			stl::swap(mStorage, other.mStorage);
		}
	...
	EA_CONSTEXPR decltype(auto) visit(Visitor&& visitor, Variants&&... variants)
	{
		...
		static_assert(conjunction_v<is_same<variant_type, remove_reference_t<Variants>>...>,
		              "all variants passed to stl::visit() must have the same type");
