...
namespace stl
{
	...
	///
		...
		#define EASTL_VECTOR_MAP_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " vector_map" // Unless the user overrides something, this is "stl vector_map".
	#endif
	...
	///
	template <typename Key, typename T, typename Compare = stl::less<Key>, 
			  typename Allocator = EASTLAllocatorType, 
			  typename RandomAccessContainer = stl::vector<stl::pair<Key, T>, Allocator> >
	...
	public:
		...
		typedef stl::pair<iterator, bool>                                   insert_return_type;
	...
	public:
		...
		template <typename P, typename = stl::enable_if_t<stl::is_constructible_v<value_type, P&&>>>
		...
		template <typename U, typename BinaryPredicate> 
		stl::pair<iterator, iterator>             equal_range(const U& u, BinaryPredicate predicate);

		template <typename U, typename BinaryPredicate> 
		stl::pair<const_iterator, const_iterator> equal_range(const U& u, BinaryPredicate) const;
	...
	inline vector_map<K, T, C, A, RAC>::vector_map(this_type&& x)
		: base_type(stl::move(x)), mValueCompare(x.mValueCompare)
	{
	...
	inline vector_map<K, T, C, A, RAC>::vector_map(this_type&& x, const allocator_type& allocator)
		: base_type(stl::move(x), allocator), mValueCompare(x.mValueCompare)
	{
		// Empty. Note: x is left with empty contents but its original mValueCompare instead of the default one. 
	}
	...
	{
		base_type::operator=(stl::move(x));
		stl::swap(mValueCompare, x.mValueCompare);
		return *this;
	}
	...
	template <typename K, typename T, typename C, typename A, typename RAC>
	template <class... Args>
	inline stl::pair<typename vector_map<K, T, C, A, RAC>::iterator, bool>
	vector_map<K, T, C, A, RAC>::emplace(Args&&... args)
	{
		...
		#else
			value_type  value(stl::forward<Args>(args)...);
		#endif
		return insert(stl::move(value));
	}
	...
	vector_map<K, T, C, A, RAC>::emplace_hint(const_iterator position, Args&&... args)
	{
		...
		return insert(position, stl::move(value));
	}
	...
	vector_map<K, T, C, A, RAC>::insert(const value_type& value)
	{
		...
		if((itLB != end()) && !mValueCompare(value, *itLB))
			return stl::pair<iterator, bool>(itLB, false);

		return stl::pair<iterator, bool>(base_type::insert(itLB, value), true);
	}
	...
	{
		value_type value(stl::forward<P>(otherValue));
		const iterator itLB(lower_bound(value.first));

		if((itLB != end()) && !mValueCompare(value, *itLB))
			return stl::pair<iterator, bool>(itLB, false);

		return stl::pair<iterator, bool>(base_type::insert(itLB, stl::move(value)), true);
	}
	...
	{
		value_type value(stl::pair_first_construct, otherValue);
		const iterator itLB(lower_bound(value.first));
	...
	{
		value_type value(stl::pair_first_construct, stl::move(otherValue));
		const iterator itLB(lower_bound(value.first));
	...
	vector_map<K, T, C, A, RAC>::insert(const_iterator position, const value_type& value)
	{
		...
		const stl::pair<typename vector_map<K, T, C, A, RAC>::iterator, bool> result = insert(value);
	...
	vector_map<K, T, C, A, RAC>::insert(const_iterator position, value_type&& value)
	{
		...
		{
			if((position == begin()) || mValueCompare(*(position - 1), value)) // If the element before position is less than value...
				return base_type::insert(position, stl::move(value));
		}

		const stl::pair<typename vector_map<K, T, C, A, RAC>::iterator, bool> result = insert(stl::move(value));
	...
	{
		const stl::pair<iterator, iterator> pairIts(equal_range(k));
	...
	{
		const stl::pair<const_iterator, const_iterator> pairIts(equal_range(k));
		return (pairIts.first != pairIts.second) ? pairIts.first : end();
	}
	...
	{
		const stl::pair<iterator, iterator> pairIts(equal_range(u, predicate));
		return (pairIts.first != pairIts.second) ? pairIts.first : end();
	}
	...
	{
		const stl::pair<const_iterator, const_iterator> pairIts(equal_range(u, predicate));
		return (pairIts.first != pairIts.second) ? pairIts.first : end();
	}
	...
	{
		return stl::lower_bound(begin(), end(), k, mValueCompare);
	}
	...
	{
		return stl::upper_bound(begin(), end(), k, mValueCompare);
	}
	...
	template <typename K, typename T, typename C, typename A, typename RAC>
	inline stl::pair<typename vector_map<K, T, C, A, RAC>::iterator, typename vector_map<K, T, C, A, RAC>::iterator>
	vector_map<K, T, C, A, RAC>::equal_range(const key_type& k)
	{
		...
		if((itLower == end()) || mValueCompare(k, *itLower)) // If at the end or if (k is < itLower)...
			return stl::pair<iterator, iterator>(itLower, itLower);

		iterator itUpper(itLower);
		return stl::pair<iterator, iterator>(itLower, ++itUpper);
	}


	template <typename K, typename T, typename C, typename A, typename RAC>
	inline stl::pair<typename vector_map<K, T, C, A, RAC>::const_iterator, typename vector_map<K, T, C, A, RAC>::const_iterator>
	vector_map<K, T, C, A, RAC>::equal_range(const key_type& k) const
	{
		...
		if((itLower == end()) || mValueCompare(k, *itLower)) // If at the end or if (k is < itLower)...
			return stl::pair<const_iterator, const_iterator>(itLower, itLower);

		const_iterator itUpper(itLower);
		return stl::pair<const_iterator, const_iterator>(itLower, ++itUpper);
	}
	...
	vector_map<K, T, C, A, RAC>::equal_range(const U& u, BinaryPredicate predicate)
	{
		...
		const iterator itLower(stl::lower_bound(begin(), end(), u, predicate_cmp));
	...
	vector_map<K, T, C, A, RAC>::equal_range(const U& u, BinaryPredicate predicate) const
	{
		...
		const const_iterator itLower(stl::lower_bound(begin(), end(), u, predicate_cmp));
	...
	vector_map<K, T, C, A, RAC>::operator[](key_type&& k)
	{
		...
		if((itLB == end()) || key_comp()(k, (*itLB).first))
			itLB = insert(itLB, value_type(stl::move(k), mapped_type()));
		return (*itLB).second;
	}
