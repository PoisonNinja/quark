...
namespace stl
{
	...
	///
		...
		#define EASTL_VECTOR_MULTIMAP_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " vector_multimap" // Unless the user overrides something, this is "stl vector_multimap".
	#endif
	...
	public:
		...
		iterator insert(const value_type& value);   // The signature of this function was change in stl v2.05.00 from (the mistaken) pair<iterator, bool> to (the correct) iterator.
		...
		stl::pair<iterator, iterator>             equal_range_small(const key_type& k)
		{
		...
		stl::pair<const_iterator, const_iterator> equal_range_small(const key_type& k) const;
	...
	vector_multimap<K, T, C, A, RAC>::insert(P&& otherValue)
	{
		...
		const iterator itLB(lower_bound(value.first));
		return base_type::insert(itLB, stl::move(value));
	}
	...
	vector_multimap<K, T, C, A, RAC>::erase(const key_type& k)
	{
		...
		return (size_type)stl::distance(pairIts.first, pairIts.second); // This can result in any value >= 0.
	}
	...
	vector_multimap<K, T, C, A, RAC>::find_as(const U& u, BinaryPredicate predicate) const
	{
		...
		const stl::pair<const_iterator, const_iterator> pairIts(stl::equal_range(begin(), end(), u, predicate_cmp));
	...
	{
		multimap_value_compare<U, value_type, BinaryPredicate> predicate_cmp(predicate);
		const stl::pair<iterator, iterator> pairIts(stl::equal_range(begin(), end(), u, predicate_cmp));
		return (pairIts.first != pairIts.second) ? pairIts.first : end();
	}
	...
	{
		const stl::pair<const_iterator, const_iterator> pairIts(equal_range(k));
		return (size_type)stl::distance(pairIts.first, pairIts.second); 
	}
	...
	template <typename K, typename T, typename C, typename A, typename RAC>
	inline stl::pair<typename vector_multimap<K, T, C, A, RAC>::iterator, typename vector_multimap<K, T, C, A, RAC>::iterator>
	vector_multimap<K, T, C, A, RAC>::equal_range(const key_type& k)
	{
		return stl::equal_range(begin(), end(), k, mValueCompare);
	}


	template <typename K, typename T, typename C, typename A, typename RAC>
	inline stl::pair<typename vector_multimap<K, T, C, A, RAC>::const_iterator, typename vector_multimap<K, T, C, A, RAC>::const_iterator>
