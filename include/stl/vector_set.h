...
namespace stl
{
	...
	///
		...
		#define EASTL_VECTOR_SET_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " vector_set" // Unless the user overrides something, this is "stl vector_set".
	#endif
	...
	///
	template <typename Key, typename Compare = stl::less<Key>, typename Allocator = EASTLAllocatorType, 
			  typename RandomAccessContainer = stl::vector<Key, Allocator> >
	...
	public:
		...
		#else
			#if EASTL_MOVE_SEMANTICS_ENABLED
				stl::pair<iterator, bool> emplace(value_type&& value);
				iterator emplace_hint(const_iterator position, value_type&& value);
			#endif

			stl::pair<iterator, bool> emplace(const value_type& value);
		...
		: base_type(x), mCompare(x.mCompare)
	{
		...
		inline vector_set<K, C, A, RAC>::vector_set(this_type&& x)
			: base_type(stl::move(x)), mCompare(x.mCompare)
		{
		...
		inline vector_set<K, C, A, RAC>::vector_set(this_type&& x, const allocator_type& allocator)
			: base_type(stl::move(x), allocator), mCompare(x.mCompare)
		{
			// Empty. Note: x is left with empty contents but its original mValueCompare instead of the default one. 
		}
	#endif
	...
	vector_set<K, C, A, RAC>::value_comp()
	{
		...
		template <typename K, typename C, typename A, typename RAC>
		template <class... Args>
		inline stl::pair<typename vector_set<K, C, A, RAC>::iterator, bool>
	...
	vector_set<K, C, A, RAC>::insert(const_iterator position, const value_type& value)
	{
		...
		const stl::pair<iterator, bool> result = insert(value);
		...
		vector_set<K, C, A, RAC>::insert(const_iterator position, value_type&& value)
		{
			...
			const stl::pair<iterator, bool> result = insert(stl::move(value));
	...
	{
		return stl::lower_bound(begin(), end(), k, mCompare);
	}
	...
	{
		return stl::upper_bound(begin(), end(), k, mCompare);
	}
	...
	template <typename K, typename C, typename A, typename RAC>
	inline stl::pair<typename vector_set<K, C, A, RAC>::iterator, typename vector_set<K, C, A, RAC>::iterator>
	...
	template <typename K, typename C, typename A, typename RAC>
	inline stl::pair<typename vector_set<K, C, A, RAC>::const_iterator, typename vector_set<K, C, A, RAC>::const_iterator>
	...
	vector_set<K, C, A, RAC>::equal_range(const U& u, BinaryPredicate predicate)
	{
		...
		// one for upper_bound), we do just lower_bound and see if the 
		// result is a range of size zero or one.
		const iterator itLower(stl::lower_bound(begin(), end(), u, predicate));
	...
	vector_set<K, C, A, RAC>::equal_range(const U& u, BinaryPredicate predicate) const
	{
		...
		// one for upper_bound), we do just lower_bound and see if the 
		// result is a range of size zero or one.
		const const_iterator itLower(stl::lower_bound(begin(), end(), u, predicate));
