...
// clang-format off
// aligned_buffer must go before hashtable_policy
...
#include "../shared/hashtable-aux.cc"
    ...
    std::size_t _Prime_rehash_policy::_M_next_bkt(std::size_t __n) const
    {
        ...
        static const unsigned char __fast_bkt[] = {2, 2,  2,  3,  5,  5,  7,
                                                   7, 11, 11, 11, 11, 13, 13};
        ...
        if (__next_bkt == __last_prime)
            // Set next resize to the max value so that we never try to rehash
            // again as we already reach the biggest possible bucket number.
    ...
    std::pair<bool, std::size_t> _Prime_rehash_policy::_M_need_rehash(
        std::size_t __n_bkt, std::size_t __n_elt, std::size_t __n_ins) const
    {
        if (__n_elt + __n_ins > _M_next_resize) {
            std::size_t __min_bkts = (__n_elt + __n_ins) / _M_max_load_factor;
