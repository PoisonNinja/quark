...
namespace
{
std::shared_ptr<Filesystem::Descriptor> get_start(const char* path)
{
    std::shared_ptr<Filesystem::Descriptor> start(nullptr);
    if (*path == '/') {
...
static long sys_open(const char* path, int flags, mode_t mode)
{
    Log::printk(Log::LogLevel::DEBUG, "[sys_open] = %s, %X, %X\n", path, flags,
                mode);
    auto file = get_start(path)->open(path, flags, mode);
    if (!file) {
        return -ENOENT;
    }
...
static long sys_stat(const char* path, struct Filesystem::stat* st)
{
    ...
    auto file = get_start(path)->open(path, 0, 0);
    return file->stat(st);
}
